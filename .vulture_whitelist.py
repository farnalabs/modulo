...
__all__ = [
    ...
    "VllmBackend",
    "WatsonXBackend",
    # --- assistant route dynamic backend dispatch (assistant.py _BACKEND_IMPORTS, resolved via importlib + getattr) ---
    "Ai21Backend",
    "AnthropicBackend",
    "DeepSeekBackend",
    "FireworksBackend",
    ...
    # api/mcp_server.py._validate_sandbox_nodes, plus exercised by
    # tests/unit/core/pipeline_engine/test_sandbox_managed_inputs.py.  Vulture
    # cannot see the call sites because all three imports are lazy (function-local).
    ...
    "dispatch_hitl_email_alerts",  # FAR-602 HITL email compose
    # exercised by tests/unit/core/test_hitl_email_alerts.py; the production
    # path (schedule_hitl_email_dispatch -> _run_hitl_email_dispatch) splits
    # resolve/send, so vulture cannot see a prod call site
    ...
    "delete_node",
    "reset_store",
    # --- FAR-811: streaming artifact writer public API properties. Consumed
    #     by node_runner.py (follow-up wiring) and tests/unit/core/artifacts/.
    "bytes_written",
    "finalized",
