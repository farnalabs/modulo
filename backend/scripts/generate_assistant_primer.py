"""Auto-generate the product primer Markdown for Assistant's system prompt.
...
Usage:
    python backend/scripts/generate_assistant_primer.py
    python backend/scripts/generate_assistant_primer.py --org-id <uuid> --user-id <uuid>
    python backend/scripts/generate_assistant_primer.py --org-id <uuid> --user-id <uuid> --output primer.md
"""
...
# Resolved at module import (sync context) so async functions never perform
# blocking pathlib operations (ruff ASYNC240).
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the Assistant product primer Markdown.",
    )
    parser.add_argument(
        "--org-id",
        type=str,
