"""Register the mobile_sidebar_rail feature flag as inactive (default OFF)

Revision ID: 0080_add_mobile_sidebar_rail_flag
Revises: 0079_schema_folders
Create Date: 2026-08-11

The icon-rail mobile sidebar (FAR-137 rescope) ships as a feature-flagged
...
This migration UPSERTS ``mobile_sidebar_rail`` into ``feature_flag_catalog``
with ``is_active=false``. ``FeatureFlagRegistry._load_catalog`` keeps inactive
rows listed (in ``_inactive_flags``) instead of dropping them, and
``_refresh`` leaves them ``currently_active=False`` unless an org/system
override exists — so the admin Feature Flags UI can list the flag and enable it
per-org, while every tier sees it OFF by default.

It also seeds the ``community``/``team`` rows into ``tier_catalog`` (ON
...
revision: str = "0080_add_mobile_sidebar_rail_flag"
down_revision: str | None = "0079_schema_folders"
...
_FLAGS: dict[str, tuple[str, str]] = {
    "mobile_sidebar_rail": ("community", "Mobile icon-rail sidebar (experimental)"),
}
