...
This migration drops the signature requirement from the registry arm while
keeping ``owner_team_id IS NULL`` (registry entries stay org-owned). All other
clauses (``source_url``/``checksum``/``verified``/``download_count``/
``average_rating``/``review_count`` NOT NULL, ``forked_from IS NULL``,
