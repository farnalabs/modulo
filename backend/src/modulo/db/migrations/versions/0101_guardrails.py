...
Revision ID: 0101_guardrails
Revises: 0100_run_classification
...
revision: str = "0101_guardrails"
down_revision: str | None = "0100_run_classification"
...
def _audit_pre_existing_eval_types() -> None:
    ...
    if offending:
        raise RuntimeError(
            "0101_guardrails: pre-existing eval_type values outside the allowed set: "
            + ", ".join(offending)
            + " (expected one of "
