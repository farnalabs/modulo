...
All indexes are created CONCURRENTLY where supported (Postgres) to avoid
blocking writes. Downgrade drops every index created here.
"""
...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX CONCURRENTLY IF NOT EXISTS "
        "ix_eval_definitions_eval_suite_id "
        "ON eval_definitions (eval_suite_id)"
    )
    ...
    op.execute(
        ...
        "ON eval_results (organisation_id, evaluated_at)"
    )
    ...
    op.execute(
        "CREATE INDEX CONCURRENTLY IF NOT EXISTS "
        "ix_suite_runs_org_created "
        "ON suite_runs (organisation_id, created_at)"
    )
    ...
    op.execute(
        "CREATE INDEX CONCURRENTLY IF NOT EXISTS "
        "ix_eval_cases_org_dataset "
        "ON eval_cases (organisation_id, dataset_id)"
    )
...
def downgrade() -> None:
    ...
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS ix_eval_cases_org_dataset")
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS ix_suite_runs_active_check")
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS ix_suite_runs_org_created")
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS ix_suite_runs_org_suite_notified")
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS ix_eval_results_org_evaluated_at")
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS ix_eval_definitions_eval_suite_id")
    op.execute("DROP INDEX CONCURRENTLY IF EXISTS ix_eval_definitions_org_pipeline_type_active")
