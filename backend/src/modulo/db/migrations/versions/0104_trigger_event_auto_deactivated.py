"""Widen ``trigger_events.ck_trigger_events_validation_result`` with ``auto_deactivated``.

Revision ID: 0104_trigger_event_auto_deactivated
Revises: 0103_lifecycle_map_version_actor
Create Date: 2026-08-15

FAR-192 vocabulary widening (0069-pattern): the FAR-190 streak engine writes a
TriggerEvent row with ``validation_result='auto_deactivated'`` via
``_log_ongoing_event`` (called from ``record_ongoing_deactivation_lifecycle``
inside the deactivation transaction). The 19-value constraint created by 0069
does NOT include ``auto_deactivated``, so on a real Postgres the CHECK
rejects the insert and the whole deactivation transaction rolls back — the
engine silently fails to deactivate on production.

This migration drops the 19-value constraint and re-adds the FULL 20-value
vocabulary exactly as 0069 did:
...
* A defensive ``SELECT`` runs before ``VALIDATE CONSTRAINT`` and raises
  ...
  there can be none in practice because the 19-value constraint was enforced,
...
The downgrade drops the 20-value constraint and re-adds the OLD 19-value one.
A defensive ``SELECT`` runs first and raises if any row carries
``auto_deactivated`` (the value the old constraint cannot express) — fail
...
revision: str = "0104_trigger_event_auto_deactivated"
down_revision: str | None = "0103_lifecycle_map_version_actor"
...
# The vocabulary BEFORE this migration (0069's 19-value set). Kept as a
# literal here so the downgrade can restore exactly what 0069 created.
_OLD_VALIDATION_RESULT_VALUES = tuple(v for v in _VALIDATION_RESULT_VALUES if v != "auto_deactivated")
...
def downgrade() -> None:
    ...
    if _is_postgres():
        # Fail loudly if any row carries auto_deactivated — the old 19-value
