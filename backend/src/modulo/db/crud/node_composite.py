...
async def get_child_nodes(session: AsyncSession, parent_id: uuid.UUID) -> list[Node]:
    ...
    result = await session.execute(select(Node).where(Node.parent_node_id == parent_id).order_by(Node.created_at))
...
async def set_parent_node(
    ...
    node = (await session.execute(select(Node).where(Node.id == node_id).with_for_update())).scalar_one_or_none()
    if node is None:
        return None
