    ...
    *,
    ...
    try:
        ...
        if parameter_schema_ids:
            ...
            if set_ids:
                set_rows = (
                    (await session.execute(select(ParameterSet).where(ParameterSet.id.in_(set_ids)))).scalars().all()
                )
        ...
        if connector_ids:
            connectors = list(
                (
                    await session.execute(select(ConnectorInstance).where(ConnectorInstance.id.in_(connector_ids)))
                ).scalars()
            )
        ...
        if backend_ids:
            backends = list(
                (await session.execute(select(ModelBackend).where(ModelBackend.id.in_(backend_ids)))).scalars()
            )
