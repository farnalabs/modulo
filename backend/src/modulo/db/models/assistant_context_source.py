...
class AssistantContextSource(Base):
    __tablename__ = "assistant_context_sources"
    __table_args__ = (
        CheckConstraint(
            "(organisation_id IS NOT NULL AND account_id IS NULL) OR "
            "(organisation_id IS NULL AND account_id IS NOT NULL)",
            name="ck_assistant_context_sources_owner",
        ),
        CheckConstraint(
            "source_mode IN ('always_on', 'tool', 'off')",
            name="ck_assistant_context_sources_mode",
        ),
        UniqueConstraint(
            "organisation_id",
            "account_id",
            "source_key",
            name="uq_assistant_context_sources_key",
        ),
    )
