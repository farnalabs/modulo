"""Admin Assistant configuration and skills management."""
...
@router.get("/config")
@handle_db_errors("admin.assistant.get_assistant_config")
async def get_assistant_config(
    session: AsyncSession = Depends(get_db_session),
    principal: TenantPrincipal = require_permission(_CODE_ADMIN_ASSISTANT_MANAGE),
) -> AssistantConfigResponse:
    try:
        async with session.begin():
            result = await session.execute(
                select(SystemConfig).where(SystemConfig.key == f"assistant_config:{principal.organisation_id}")
            )
        ...
        logger.exception("Unexpected error in get_assistant_config")
        raise HTTPException(
...
@handle_db_errors("admin.assistant.get_available_providers")
async def get_available_providers(
    _principal: TenantPrincipal = require_permission(_CODE_ADMIN_ASSISTANT_MANAGE),
) -> AvailableProvidersResponse:
    try:
        from modulo.api.routes.assistant import SUPPORTED_PROVIDERS
...
async def _apply_assistant_config_patch(
    session: AsyncSession,
    req: AssistantConfigUpdate,
    principal: TenantPrincipal,
) -> dict[str, Any]:
    """Load-or-create the org's ``assistant_config`` entry and apply non-None fields.
    ...
    entry = result.scalar_one_or_none()
    if entry is None:
        entry = SystemConfig(key=f"assistant_config:{principal.organisation_id}", value={})
        session.add(entry)
...
def _assistant_config_response(current: dict[str, Any]) -> AssistantConfigResponse:
    """The merged Assistant config rendered as a response, with documented defaults."""
    return AssistantConfigResponse(
...
async def update_assistant_config(
    ...
    try:
        async with session.begin():
            current = await _apply_assistant_config_patch(session, req, principal)
        ...
        logger.exception("Unexpected error in update_assistant_config")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_INTERNAL_SERVER_ERROR,
        ) from None
...
async def _get_org_skill(
    ...
    skill = await session.get(AssistantSkill, skill_id)
...
def _skill_to_response(skill: AssistantSkill) -> SkillResponse:
...
@router.get("/skills")
@handle_db_errors("admin.assistant.list_org_skills")
    ...
    try:
        async with session.begin():
            ...
            result = await session.execute(
                ...
                .where(
                    AssistantSkill.organisation_id == principal.organisation_id,
                    AssistantSkill.account_id.is_(None),
                )
...
@handle_db_errors("admin.assistant.get_org_context_sources")
async def get_org_context_sources(
    ...
    try:
        async with session.begin():
            ...
            builtin_defaults = AssistantConfig().context_sources
        return {
...
async def reset_org_context_sources(
    ...
    try:
        async with session.begin():
            ...
            result = await session.execute(
                select(AssistantContextSource).where(
                    AssistantContextSource.organisation_id == principal.organisation_id,
...
async def get_user_skills(session: AsyncSession, user_id: uuid.UUID, org_id: uuid.UUID) -> list[AssistantSkill]:
    result = await session.execute(
        select(AssistantSkill)
        .where(
            or_(
                AssistantSkill.account_id == user_id,
                AssistantSkill.organisation_id == org_id,
            )
        )
        .order_by(AssistantSkill.created_at.desc())
    )
...
async def get_user_skill_or_404(session: AsyncSession, user_id: uuid.UUID, skill_id: uuid.UUID) -> AssistantSkill:
    skill = await session.get(AssistantSkill, skill_id)
