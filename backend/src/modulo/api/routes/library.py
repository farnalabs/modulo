...
async def create_pipeline_from_template_endpoint(
    ...
    import sys; sys.stderr.write(f"FULL_IMPL {primitive_id}\n"); sys.stderr.flush()
    try:
        async with session.begin():
            await set_rls_org(session, principal.organisation_id)
            await set_rls_user_context(session, principal.account_id, principal.org_role)
        ...
        raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="Feature is not available. Run database migrations to enable it.") from None
    if primitive is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Primitive {primitive_id} not found")
    if primitive.primitive_type != "pipeline_template":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Primitive type must be 'pipeline_template', got '{primitive.primitive_type}'")
        ...
        async with session.begin():
            ...
            for edge_data in edges:
                ...
                edge = PipelineEdge(
                    ...
                    organisation_id=principal.organisation_id, pipeline_id=pipeline.id,
