...
from modulo.db.crud.schema_folder import (
    create_folder,
    delete_folder,
    list_folders,
    update_folder,
...
router = APIRouter(prefix="/api/v1/schema-folders", tags=["schema-folders"])
...
@handle_db_errors("schema_folders.list")
async def list_folders_endpoint(
    ...
    try:
        ...
        logger.exception("schema_folders.list")
        raise HTTPException(
...
@handle_db_errors("schema_folders.create")
...
@handle_db_errors("schema_folders.update")
...
@handle_db_errors("schema_folders.delete")
async def delete_folder_endpoint(
    ...
    try:
        ...
        logger.exception("schema_folders.delete(%s)", folder_id)
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail="This feature is not available. Run database migrations to enable it.",
        ) from None
...
@handle_db_errors("schema_folders.move")
async def reorder_folder_endpoint(
    ...
    try:
        ...
        logger.exception("schema_folders.reorder(%s)", folder_id)
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail="This feature is not available. Run database migrations to enable it.",
        ) from None
