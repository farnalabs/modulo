"""Assistant Chat API — CRUD sessions, messages, and SSE streaming for LLM chat.

Endpoints:
    Sessions:
        GET    /api/v1/assistant/sessions             — list user's sessions
        POST   /api/v1/assistant/sessions             — create new session
        GET    /api/v1/assistant/sessions/{id}        — get session with message count
        PATCH  /api/v1/assistant/sessions/{id}        — rename session
        DELETE /api/v1/assistant/sessions/{id}        — delete session + messages

    Messages:
        GET    /api/v1/assistant/sessions/{id}/messages   — list messages for session
        POST   /api/v1/assistant/sessions/{id}/messages   — append a message

    Streaming:
        POST   /api/v1/assistant/sessions/{id}/stream     — SSE stream of LLM response

    UI Commands:
        POST   /api/v1/assistant/sessions/{id}/permission-response
        POST   /api/v1/assistant/sessions/{id}/ui-command-results
        POST   /api/v1/assistant/sessions/{id}/reset-permissions
"""
...
_CODE_ASSISTANT_DATABASE_ERROR = "assistant.database_error"
...
def _get_registry() -> Any | None:
    ...
    if _redis_registry is None:
        ...
        if not redis_url:
            return None
        from modulo.core.assistant.redis_registry import AssistantRedisRegistry

        _redis_registry = AssistantRedisRegistry(redis_url)
...
class StreamRequest(BaseModel):
    ...
    page_context: str | None = Field(None, description="Current page context for Assistant's context-awareness.")
...
async def _is_ui_driving_enabled(org_id: uuid.UUID) -> bool:
    """Check if the assistant_ui_driving feature flag is enabled for the given org.
    ...
    try:
        ...
        return await registry.resolve_flag("assistant_ui_driving", org_id=org_id)
    except Exception:
...
def _tool_allowlist_disabled(config: AssistantConfig, tool_name: str, args: dict[str, Any]) -> bool:
...
def _default_tool_permission(config: AssistantConfig, tool_name: str, args: dict[str, Any]) -> str:
...
def _resolve_tool_permission(
    config: AssistantConfig, tool_name: str, args: dict[str, Any], page_context: str = ""
) -> str:
...
def _build_stream_backend(req: StreamRequest, api_key: str) -> tuple[ModelBackendBase | None, str | None]:
    ...
    try:
        ...
        logger.exception("assistant.backend_construction_failed")
...
async def _auto_name_stream_session(
    ...
    if auto_name:
        try:
            ...
            logger.exception("assistant.auto_naming_failed")
...
async def _stream_approved_commands(
    ...
    if len(approved_calls) != len(results):
        logger.warning(
            "assistant.ui_result_length_mismatch",
...
@handle_db_errors("assistant.list_sessions")
async def list_sessions(
    ...
    try:
        ...
        logger.exception("assistant.list_sessions")
        raise HTTPException(
        ...
        logger.exception("assistant.list_sessions.unexpected_error")
        raise HTTPException(
...
async def _resolve_provider_model(
    ...
    Assistant default provider/model, and finally to the opencode default model
    ...
    if mb:
        ...
        config = await AssistantConfigService(session).get_config(principal.organisation_id)
...
async def create_session(
    ...
    try:
        ...
        logger.exception("assistant.create_session")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.create_session.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
...
@handle_db_errors("assistant.get_session")
async def get_session(
    ...
    try:
        ...
        logger.exception("assistant.get_session")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.get_session.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
...
async def rename_session(
    ...
    try:
        ...
        logger.exception("assistant.rename_session")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.rename_session.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
    ...
    try:
        ...
        if registry is not None:
            try:
                ...
                logger.exception("assistant.redis_cleanup_failed")

        return {"status": "deleted", "id": str(session_id)}
    except ProgrammingError:
        logger.exception("assistant.delete_session")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.delete_session.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
...
@handle_db_errors("assistant.list_messages")
async def list_messages(
    ...
    try:
        ...
        logger.exception("assistant.list_messages")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.list_messages.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
...
async def append_message(
    ...
    try:
        ...
        logger.exception("assistant.append_message")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.append_message.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
...
async def _stream_event_generator(
    ...
    try:
        ...
        logger.exception("Assistant streaming error — missing DB table or schema")
        ...
        logger.exception("Assistant streaming error")
...
async def stream_chat(
    ...
    try:
        ...
        logger.exception("assistant.stream_chat")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
...
async def submit_permission_response(
    ...
    try:
        ...
        logger.exception("assistant.submit_permission_response")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.submit_permission_response.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
...
async def submit_ui_command_results(
    ...
    try:
        ...
        logger.exception("assistant.submit_ui_command_results")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.submit_ui_command_results.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
...
async def reset_session_permissions(
    ...
    try:
        ...
        logger.exception("assistant.reset_session_permissions")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.reset_session_permissions.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
...
async def resume_session(
    ...
    try:
        ...
        logger.exception("assistant.resume_session")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.resume_session.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
...
async def stop_session(
    ...
    try:
        ...
        logger.exception("assistant.stop_session")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.stop_session.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
...
@handle_db_errors("assistant.get_audit_trail")
async def get_audit_trail(
    ...
    try:
        ...
        logger.exception("assistant.get_audit_trail")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.get_audit_trail.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
...
async def undo_last_action(
    ...
    try:
        ...
        logger.exception("assistant.undo_last_action")
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail=MSG_FEATURE_NOT_AVAILABLE,
        ) from None
        ...
        logger.exception("assistant.undo_last_action.unexpected_error")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=MSG_UNEXPECTED_ERROR,
        ) from None
