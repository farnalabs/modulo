...
async def backfill_facts(session: Any, day: date) -> int:
    ...
    # native Postgres ``jsonb`` type (since migration 0147 converted it from
    # ``json``), so the ``jsonb_*`` functions apply; all three degrade to
    # defaults when the graph is malformed/absent — backfilled rows must NEVER
    # carry NULL here (NULL facts on backfilled rows are a bug).
