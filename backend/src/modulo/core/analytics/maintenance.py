...
def _reassembled_bytes_expr(node_side: Any, empty_flag: Any) -> Any:
    ...
    * the live writer (core/analytics/__init__.py, Python site):
      ...
      for the simple-ASCII shape (the LEGACY fallback is GONE with the
      dropped runs blob columns — migration 0215 — so the reader formula is
    ...
    unrepresented after the drop migration's repair; the EMPTY fallback
    term referencing the dropped runs column died with migration 0215.
    """
...
async def backfill_facts(session: Any, day: date) -> int:
    ...
    select_stmt = (
        sa.select(
            ...
            # FAR-583 drop: bytes are computed from the reassembled
