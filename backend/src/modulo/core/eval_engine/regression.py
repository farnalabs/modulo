...
def _validate_regression_inputs(
    ...
    """Validate the regression-detection inputs, raising ``ValueError`` on any violation."""
    if days < 1:
...
def _regression_window_bounds(days: int, recent_window_ratio: float) -> tuple[datetime, datetime]:
    """Resolve the (baseline_start, recent_start) clock windows for the lookback period."""
    ...
    return baseline_start, recent_start


async def _fetch_regression_rows(
    ...
    """Execute the clock-window regression query, logging and re-raising DB errors."""
    try:
        q = text("""
            SELECT
                er.eval_id,
...
def _window_regression_alerts(
    rows: Sequence[Any],
    threshold: float,
    trend: str | None,
) -> list[RegressionAlert]:
    """Build alerts from the clock-window query rows, honouring the trend filter."""
...
async def detect_regressions(
    ...
    *,
    ...
    """
    _validate_regression_inputs(days, threshold, recent_window_ratio, trend, group_by)
    ...
    baseline_start, recent_start = _regression_window_bounds(days, recent_window_ratio)
    rows = await _fetch_regression_rows(session, org_id, days, baseline_start, recent_start, pipeline_id)
    return _window_regression_alerts(rows, threshold, trend)


async def _fetch_grouped_regression_rows(
    session: AsyncSession,
    org_id: UUID,
    ...
    """Execute the SuiteRun comparison query, logging and re-raising DB errors."""
    try:
        q = text("""
            SELECT
                er.eval_id,
...
def _exceeds_regression_threshold(
    drop: float,
    prev_pass_rate: float,
    threshold: float,
    relative_threshold: float | None,
) -> bool:
    """Return ``True`` when the drop exceeds the absolute (and optional relative) threshold."""
...
def _grouped_regression_alerts(
    rows: Sequence[Any],
    threshold: float,
    relative_threshold: float | None,
    trend: str | None,
    ...
    """Build SuiteRun-path alerts from the grouped query rows (detector semantics)."""
    alerts: list[RegressionAlert] = []
    for row in rows:
        ...
        if not _exceeds_regression_threshold(drop, prev_pass_rate, threshold, relative_threshold):
            continue
...
async def _detect_regressions_grouped(
    ...
    rows = await _fetch_grouped_regression_rows(session, org_id, current_ids, baseline_ids, all_ids, eval_type)
    return _grouped_regression_alerts(rows, threshold, relative_threshold, trend, current_ids)
