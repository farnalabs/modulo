...
* count that actor's ``hitl.output_delivered`` audit events in the last
  ``SWEEP_WINDOW_SECONDS`` (org-scoped — the ``hitl_claims.account_id``
  column suggested by the original plan is NULLed by ``_decide`` at
  decision time, so the audit chain is the only per-actor decision
  record; the count joins ``hitl_claims`` on the audit ``resource_id``
  to resolve each decided gate's pipeline);
* when the count exceeds ``SWEEP_COUNT_THRESHOLD`` AND the decisions span
  more than one pipeline, emit the alarm: an audit event
  (``hitl_approve_sweep_suspected``), an in-app admin notification, and a
  fire-and-forget webhook dispatch through the shared :class:`Notifier`.
...
# Decision event counted by the detection query. One approve (plain or with
# modification) writes exactly one ``hitl.output_delivered`` audit event, so
# the count of these events == the count of committed approve decisions.
_DECISION_EVENT_TYPE = "hitl.output_delivered"
...
async def count_recent_approves(
    ...
    *,
    ...
    """Count the actor's approve decisions since *window_start* (org-scoped).

    Returns ``(decision_count, distinct_pipeline_count)``. The count reads the
    audit chain (the only per-actor decision record — ``hitl_claims.account_id``
    is NULLed at decision time) joined to ``hitl_claims`` on the audit
    ``resource_id`` so each decided gate's pipeline is resolved without a
    second query.
    """
    stmt = (
        ...
        .where(
            ...
            AuditEvent.event_type == _DECISION_EVENT_TYPE,
...
async def _create_in_app_notification(session: AsyncSession, org_id: uuid.UUID, payload: dict[str, Any]) -> None:
    """Create the in-app admin notification inside the decision transaction.

    Isolated in a helper so tests can observe it without patching the mapper
    class. Failure propagates to the caller's guard (the alarm is no-throw).
    """
    await NotificationEventMapper().create_from_event(
        session,
        org_id=org_id,
        event_type=EVENT_HITL_APPROVE_SWEEP,
        payload=payload,
    )
...
def _schedule_sweep_webhook(org_id: uuid.UUID, payload: dict[str, Any]) -> None:
    ...
    inside the decision transaction. Like ``hitl_email_alerts`` the task is
    scheduled on the running loop with a strong reference retained until it
    completes; it opens its OWN session on the shared engine. Never raises.
    """
    try:
...
async def maybe_alarm_approve_sweep(
    ...
    *,
    ...
    Runs after an approve decision (plain or with-modification) has committed
    its ``hitl.output_delivered`` audit event on *session*, so the detection
    count includes the just-made decision. No-throw by contract: every failure
    ...
    try:
        ...
        approve_count, distinct_pipelines = await count_recent_approves(
            session, org_id=org_id, actor_id=actor_id, window_start=window_start
        )
        if approve_count <= SWEEP_COUNT_THRESHOLD or distinct_pipelines < SWEEP_MIN_DISTINCT_PIPELINES:
            return False

        payload = _alarm_payload(actor_id=actor_id, approve_count=approve_count, distinct_pipelines=distinct_pipelines)
