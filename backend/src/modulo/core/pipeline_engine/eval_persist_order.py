...
async def run_evals_persist_before_decide(
    *,
    ...
    When ``session_factory`` is ``None`` or ``org_id`` is ``None``, only the
    compute and decide steps run — persistence is skipped (matching the
    legacy no-persistence path when no session is available).
    ...
    Args:
        ...
        eval_target: The resolved node output dict for eval computation.
