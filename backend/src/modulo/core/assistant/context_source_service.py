...
_BUILTIN_SOURCE_METADATA: dict[str, dict[str, str]] = {
    "page_context": {
        "name": "Page Context",
        "description": "Content and state of the current page Assistant is viewing",
    },
    "user_profile": {
        "name": "User Profile",
...
class AssistantContextSourceService:
    ...
    async def _upsert_context_source(
        ...
        if user_id is None:
            stmt = (
                select(AssistantContextSource)
                .where(
                    AssistantContextSource.organisation_id == org_id,
                    AssistantContextSource.source_key == source_key,
                    AssistantContextSource.account_id.is_(None),
                )
                .with_for_update()
            )
    ...
    async def reset_user_overrides(self, org_id: uuid.UUID, user_id: uuid.UUID) -> None:
        stmt = sa_delete(AssistantContextSource).where(
            AssistantContextSource.organisation_id == org_id,
            AssistantContextSource.account_id == user_id,
        )
