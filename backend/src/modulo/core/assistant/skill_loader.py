...
class SkillLoader:
    def __init__(
        ...
        config_service: AssistantConfigService | None = None,
        ctx_service: AssistantContextSourceService | None = None,
    ...
    async def _get_skills(self, **filters: Any) -> list[SkillEntry]:
        try:
            ...
            for k, v in filters.items():
                col = getattr(AssistantSkill, k, None)
                if col is None:
            ...
            stmt = (
                ...
                .where(*conditions, AssistantSkill.active.is_(True))
                .order_by(AssistantSkill.id)
    ...
    def _build_config_section(self, config: AssistantConfig | None, system_prompt_override: str | None) -> str | None:
        if config is None:
            return None
    ...
    def _build_guidance_section(self, config: AssistantConfig | None) -> str | None:
        if config is None:
            return None
        return config.additional_guidance or None

    def _build_overview_section(self, config: AssistantConfig | None, ctx_sources: dict[str, str]) -> str | None:
        if config is None:
            return None
    ...
    async def _load_config(self, org_id: uuid.UUID) -> AssistantConfig | None:
        config_service = self._config_service or AssistantConfigService(self._session)
        try:
            return await config_service.get_config(org_id)
        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("Failed to load Assistant config for org %s", org_id)
            return None

    async def _load_ctx_sources(self, org_id: uuid.UUID, user_id: uuid.UUID) -> dict[str, str]:
        ctx_service = self._ctx_service or AssistantContextSourceService(self._session)
        try:
    ...
    def _to_entry(self, skill: AssistantSkill) -> SkillEntry:
