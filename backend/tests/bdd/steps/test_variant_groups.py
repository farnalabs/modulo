...
# ===================================================================
#  Batch-scoped comparison (FAR-332) — step definitions
# ===================================================================
#
# These steps back a "batch comparison" feature scenario. The batch is loaded
# purely by ``batch_id`` (independent of a live variant group) and each run
# carries its frozen snapshot/override, canonical status, eval pass rate, cost,
# and tokens.


@then("every run created in the batch shares the same batch_id")
def check_batch_same_batch_id(ctx: dict[str, Any]) -> None:
    results = ctx.get("run_results")
    assert results is not None, "Expected batch run results"
    assert len(results) >= 1
    batch_ids = {r.get("batch_id") for r in results}
    assert len(batch_ids) == 1, f"Expected a single batch_id, got {batch_ids}"


@given("the batch has runs with frozen snapshot and override data")
def batch_has_frozen_runs(ctx: dict[str, Any]) -> None:
    batch_id = uuid.uuid4()
    ...
    ctx["batch_compare"] = None


@when("the batch comparison is computed")
def batch_comparison_computed(ctx: dict[str, Any]) -> None:
    import asyncio
    ...
    async def _run():
        ...
        def _rows(stmt, *args: Any, **kwargs: Any) -> Any:
            ...
            if "EvalResult" in str(stmt) or "eval_results" in str(stmt):
                return eval_mock
            return scalar_mock

        session.execute = AsyncMock(side_effect=_rows)
        return await get_batch_compare(
            session,
            org_id=uuid.UUID("00000000-0000-0000-0000-000000000001"),
            batch_id=ctx["batch_id"],
        )

    ctx["batch_compare"] = asyncio.run(_run())


@then("the comparison returns one entry per batch run")
def check_comparison_run_count(ctx: dict[str, Any]) -> None:
    compare = ctx.get("batch_compare")
    assert compare is not None, "Expected a batch comparison result"
    assert len(compare) == len(ctx["batch_runs"])


@then("each comparison entry exposes status, eval pass rate, cost, tokens and the frozen snapshot")
def check_comparison_fields(ctx: dict[str, Any]) -> None:
    compare = ctx.get("batch_compare")
    assert compare is not None, "Expected a batch comparison result"
    for entry in compare:
        for field in (
            ...
            assert field in entry, f"Comparison entry missing {field!r}"


@then("the frozen snapshot recorded at fire time is used, not the live group")
def check_frozen_snapshot_used(ctx: dict[str, Any]) -> None:
    compare = ctx.get("batch_compare")
    assert compare is not None, "Expected a batch comparison result"
    for entry, run in zip(compare, ctx["batch_runs"], strict=True):
        frozen = run["variant_config_snapshot"]
        assert entry["variant_name"] == frozen["variant_name"]
        assert str(entry["snapshot_id"]) == frozen["snapshot_id"]
