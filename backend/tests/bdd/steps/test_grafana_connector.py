...
list dashboards/dashboard-by-uid/alert-rules/datasources, and annotation writes.
"""
...
def grafana_health_check(ctx: dict) -> None:
    ...
    valid = ctx["valid"]
    response = httpx.Response(200, json={"commit": "abc123"}) if valid else httpx.Response(401, text="Unauthorized")
    with respx.mock:
...
def grafana_query_dashboard_by_uid(uid: str, ctx: dict) -> None:
    ...
    dashboard = {"dashboard": {"uid": uid, "title": "My Dashboard"}, "meta": {"slug": "my-dashboard"}}
    with respx.mock:
...
def grafana_query_alert_rules(ctx: dict) -> None:
    ...
    rules = [{"uid": "r1", "name": "High CPU Rule"}, {"uid": "r2", "name": "Memory Rule"}]
    with respx.mock:
...
def grafana_query_datasources(ctx: dict) -> None:
    ...
    datasources = [{"id": 1, "name": "Prometheus", "type": "prometheus"}, {"id": 2, "name": "Loki", "type": "loki"}]
    with respx.mock:
...
def grafana_result_contains_dashboards(ctx: dict) -> None:
    ...
    assert result.records[0]["uid"] == "d1", result.records[0]
...
def grafana_result_contains_dashboard(ctx: dict) -> None:
    ...
    assert result.records[0]["meta"]["slug"] == "my-dashboard", result.records[0]
