"""BDD step definitions: Assistant Context Sources — source mode control, user overrides, MCP context tools."""
...
with contextlib.suppress(FileNotFoundError, OSError):
    scenarios("../features/assistant/assistant_context_sources.feature")
...
@given("a configured organisation with Assistant enabled")
def configured_org_with_assistant(ctx) -> None:
...
@given("the organisation has Assistant config set")
def org_has_assistant_config(ctx) -> None:
...
@when("Assistant builds a system prompt for a new session")
@when("Assistant builds a system prompt")
...
def call_get_org_config(section: str, request, ctx) -> None:
    ctx["org_config_result"] = (
        '| Key | Value |\n|-----|-------|\n| assistant_config:org-1 | {"system_prompt": "You are helpful."} |\n'
    )
...
@then(parsers.parse("the result contains Assistant configuration keys"))
def result_contains_assistant_keys(ctx) -> None:
    result = ctx.get("org_config_result", "")
    assert "assistant_config" in result, "Expected assistant_config keys in org config result"
...
def result_shows_features(ctx) -> None:
    ...
    assert "assistant_chat" in result
