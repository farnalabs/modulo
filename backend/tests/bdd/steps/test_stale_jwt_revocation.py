...
@given(
    parsers.parse('user "{username}" holds a valid JWT')
)
...
@given(
    parsers.parse('user "{username}" is removed from team "{team_name}"')
)
...
@given(
    parsers.parse('user "{username}" still holds a valid JWT')
)
...
@given(
    parsers.parse('a run "{run_name}" is awaiting human at gate "{gate_id}" with required_team_id "{team_name}"')
)
...
@when(
    parsers.parse('user "{username}" refreshes their JWT')
)
...
@when(
    parsers.parse('user "{username}" uses an unexpired JWT issued before the change')
)
...
@when(
    parsers.parse('I change user "{username}" role from "{old_role}" to "{new_role}"')
)
...
@when(
    parsers.parse('user "{username}" attempts to claim gate "{gate_id}" on run "{run_name}"')
)
...
@then(
    parsers.parse('user "{username}" is redirected to re-authenticate on next request')
)
