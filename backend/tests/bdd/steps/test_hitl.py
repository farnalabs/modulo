...
def another_user_claimed_gate(gate_id: str, ctx):
    """A different user already holds the claim on this gate."""
    ctx["gate_claimed_by_other"] = True
    ctx["gate_claimed"] = False


@given(parsers.parse('another user has claimed gate "{gate_id}" with claim_token "{token}"'))
def another_user_claimed_gate_with_token(gate_id: str, token: str, ctx):
    """A different user holds the claim with a known claim_token."""
    ctx["gate_claimed_by_other"] = True
    ctx["gate_claimed"] = False
    ctx["other_claim_token"] = token


@when(parsers.parse("I POST /api/runs/{run_id}/claim"))
def post_claim_gate(request, run_id, ctx, client):
    """POST the real claim endpoint; an already-claimed gate returns 409."""
    from modulo.core.hitl_manager import AlreadyClaimedError

    _ = run_id  # parsed from step — use ctx["run_id"] for the actual UUID
    run_id = ctx["run_id"]
    gate_id = ctx["gate_id"]

    mock_gate = MagicMock()
    mock_gate.run_id = run_id
    mock_gate.gate_id = gate_id
    mock_gate.claim_token = ctx.get("claim_token", "valid_token_" + uuid.uuid4().hex)
    mock_gate.expires_at = datetime.now(UTC) + timedelta(minutes=15)

    mock_mgr = MagicMock()
    if ctx.get("gate_claimed_by_other"):
        mock_mgr.claim = AsyncMock(side_effect=AlreadyClaimedError(run_id, gate_id))
    else:
        mock_mgr.claim = AsyncMock(return_value=mock_gate)
    ...
    with (
        ...
        resp = client.post(f"/api/v1/runs/{run_id}/hitl/{gate_id}/claim", json={})
    request.node._resp = resp
    if not ctx.get("gate_claimed_by_other"):
        ctx["claim_token"] = mock_gate.claim_token
        ctx["gate_claimed"] = True


@then(parsers.parse('I am the claimant of gate "{gate_id}"'))
def i_am_claimant(gate_id: str, ctx):
    assert ctx.get("gate_claimed"), f"Expected user to be the claimant of gate '{gate_id}'"


@then("the response contains a claim_token")
def response_contains_claim_token(request):
    body = request.node._resp.json()
    assert body.get("claim_token"), f"Expected claim_token in response, got {body}"
...
def my_claim_expires(ctx):
    assert ctx.get("claim_expired"), "Claim was not expired"


@then("another user can claim the gate")
def another_user_can_claim(ctx):
    assert ctx.get("claim_expired"), "Another user cannot claim until the claim expires"


# ============================================================================
# HITL Approve — approve.feature
# ============================================================================


@when(parsers.parse('I POST /api/runs/{run_id}/approve with claim_token and decision "{decision}"'))
def post_approve_with_claim_token(request, run_id, decision: str, ctx, client):
    """POST the real approve endpoint with the user's valid claim_token."""
    from modulo.core.pipeline_engine.executor import PipelineExecutor as RealExecutor

    _ = run_id
    run_id = ctx["run_id"]
    gate_id = ctx["gate_id"]
    claim_token = ctx.get("claim_token", "valid_token_" + uuid.uuid4().hex)
...
def post_approve_no_claim_token(request, run_id, decision: str, ctx, client):
    """Missing claim_token fails request validation with 422."""
    ...
    mock_mgr = MagicMock()
    mock_mgr.approve = AsyncMock(side_effect=DecisionPayloadError("claim_token is required"))

    with (
        patch("modulo.api.routes.hitl.HITLManager", return_value=mock_mgr),
        patch("modulo.api.routes.hitl.PipelineExecutor"),
    ):
        resp = client.post(
            f"/api/v1/runs/{run_id}/hitl/{gate_id}/approve",
            json={},
...
def post_approve_expired_claim_token(request, run_id, decision: str, ctx, client):
    """An expired claim_token maps to 410 Gone."""
    ...
    mock_mgr = MagicMock()
    mock_mgr.approve = AsyncMock(side_effect=ClaimTokenExpiredError())
    ...
    with (
        ...
        resp = client.post(
            f"/api/v1/runs/{run_id}/hitl/{gate_id}/approve",
            json={"claim_token": ctx.get("claim_token", "expired-token")},
        )
    request.node._resp = resp
    ctx["decision"] = decision
...
def post_approve_other_users_token(request, run_id, token: str, decision: str, ctx, client):
    """A claim_token held by another user maps to 403."""
    ...
    mock_mgr = MagicMock()
    mock_mgr.approve = AsyncMock(side_effect=ClaimTokenInvalidError())
