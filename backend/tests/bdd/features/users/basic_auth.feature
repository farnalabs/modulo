Feature: Basic Auth
  ...
  Scenario: Successful login returns JWT
    ...
    When I POST /api/v1/auth/login with email "alice@example.com" and password "correct-horse-battery"
  ...
  Scenario: Wrong password is rejected
    Given a user exists with email "alice@example.com" and password "correct-horse-battery"
    When I POST /api/v1/auth/login with email "alice@example.com" and password "wrong-password"
    Then the response status is 401

  Scenario: Unknown email is rejected
    When I POST /api/v1/auth/login with email "nobody@example.com" and password "anything"
    Then the response status is 401
