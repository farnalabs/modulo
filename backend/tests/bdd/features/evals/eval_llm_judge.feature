Feature: LLM Judge Eval
  ...
  Scenario: Rubric-based scoring passes above threshold
    Given node "code-reviewer" has an llm_judge eval "quality-check"
    ...
    And the llm_judge callable returns {"passed": true, "score": 0.85, "detail": "Meets criteria"}
  ...
  Scenario: LLM judge score below threshold fails the eval
    ...
    And the eval has failure_behaviour "warn"
    When the node outputs {"code": "def foo(): pass"}
    And the llm_judge callable returns {"passed": false, "score": 0.45, "detail": "Missing error handling"}
  ...
  Scenario: LLM judge block behaviour stops the pipeline
    ...
    When the node outputs {"code": "def foo(): pass"}
    And the llm_judge callable returns {"passed": false, "score": 0.3, "detail": "Critical issue"}
