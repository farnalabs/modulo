Feature: Admin Assistant Configuration
  As an org admin
  I want to configure the Assistant AI assistant settings
  So that I can control the system prompt, access, and defaults

  Scenario: Admin can get the default Assistant config when none is set
  ...
  Scenario: Admin can set the system prompt
    Given I am authenticated as an admin in org "acme"
    When I update the Assistant config with system_prompt "You are a helpful assistant"
    Then the response status is 200
    And the config has system_prompt "You are a helpful assistant"

  Scenario: Admin can set the access list
    Given I am authenticated as an admin in org "acme"
    When I update the Assistant config access_list with user_ids ["user-1", "user-2"]
    Then the response status is 200
    And the config access_list includes user_ids ["user-1", "user-2"]

  Scenario: Admin can set the default provider and model
    Given I am authenticated as an admin in org "acme"
    When I update the Assistant config default_provider to "openai" and default_model to "gpt-4o"
    Then the response status is 200
  ...
  Scenario: Admin can allow specific providers
    Given I am authenticated as an admin in org "acme"
    When I update the Assistant config allowed_providers to ["anthropic", "openai"]
    Then the response status is 200
  ...
  Scenario: Non-admin gets 403 on update config
    Given I am authenticated as a viewer in org "acme"
    When I update the Assistant config with system_prompt "hacked"
    Then the response status is 403

  Scenario: Update config persists and returns merged fields
    Given I am authenticated as an admin in org "acme"
    When I update the Assistant config with additional_guidance "Be concise"
    And I GET the admin Assistant config
  ...
  Scenario: Admin gets 422 for unsupported allowed_provider
    Given I am authenticated as an admin in org "acme"
    When I update the Assistant config allowed_providers to ["nonexistent"]
