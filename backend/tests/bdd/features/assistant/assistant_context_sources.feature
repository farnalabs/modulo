Feature: Assistant Context Sources
  As a Assistant user
  I want Assistant to only load the knowledge I need
  So that Assistant stays token-efficient and relevant

  Background:
    Given the organisation exists
    And a configured organisation with Assistant enabled
  ...
  Scenario: get_org_config returns filtered config
    ...
    When the user calls get_org_config with section "assistant"
    Then the result contains Assistant configuration keys
