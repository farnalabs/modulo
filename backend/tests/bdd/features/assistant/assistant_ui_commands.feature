Feature: Assistant UI Commands
  Assistant can execute browser automation commands by emitting LLM tool calls
  ...
  Scenario: Assistant navigates to a page
  ...
  Scenario: Assistant clicks an element with destructive selector requires approval
  ...
  Scenario: Assistant fills a form field (auto-allowed in safe mode)
    Given permission mode is "safe"
  ...
  Scenario: Assistant extracts page content
