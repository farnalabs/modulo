Feature: Assistant Chat Messages
  ...
  Scenario: Append a user message to an existing session
    ...
    And I have a assistant session
    When I append a "user" message with content "Hello, Assistant!"
  ...
  Scenario: List messages for a session returns ascending order
    Given I am authenticated as an admin in org "acme"
    And I have a assistant session with messages in order
