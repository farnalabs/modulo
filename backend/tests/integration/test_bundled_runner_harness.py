...
network). The unit/docker suites cover provider behaviour in isolation; THIS
file verifies the topology the CI job boots:

  1. the spike (a) allow/deny matrix through the proxy;
  ...
  3. the proxy-completeness assertion: the exercise must not trigger ANY
     proxy rejection (log scan for 403 / ``PR--`` — spike criterion 3), with
     the single retry-once tolerance for the transient first-pull 500
     (spike surprise #5);
...
All tests are ``@pytest.mark.docker`` and skip cleanly (fixture level) when
no Docker engine is reachable. When the compose rig itself is absent
(plain local engine, no overlay), the topology-dependent tests skip with an
explicit reason rather than failing — the CI job boots the rig first.
"""
...
_HARNESS_WORKSPACE_NETWORK = os.environ.get("MODULO_RUNNER_HARNESS_NETWORK", "runner-ci-workspace")
...
def _run_on_local_engine_loop(factory: Callable[[], Any], timeout: float = 90.0) -> Any:
    ...
    - the integration conftest pins the ``WindowsSelectorEventLoopPolicy``
      globally (psycopg requirement), but Docker Desktop's named pipe
      (aiodocker's local-engine default) only works on a ProactorEventLoop;
...
def _engine_probe() -> bool:
    """Sync engine reachability check (Proactor bridge; the conftest-pinned
    Selector loop cannot open the Docker Desktop named pipe on win32)."""
...
async def test_proxy_allow_deny_matrix() -> None:
    ...
    are denied 403 with the PR-- proxy-reject flag in the proxy log."""
    host = _require_rig()
...
async def test_nested_roundtrip_through_proxy_and_completeness() -> None:
    ...
    been rejected by the proxy (log scan; the transient first-pull 500 gets
    retry-once tolerance per spike surprise #5, and 500s are not proxy
    rejections anyway — only 403/PR-- lines fail)."""
    host = _require_rig()
    network = _HARNESS_WORKSPACE_NETWORK

    # The rig's workspace network must exist (the holder service created it).
    if not _workspace_network_exists(network):
        pytest.skip(f"compose workspace network '{network}' not found (is the runner-ci rig up?)")
...
# ---------------------------------------------------------------------------
# Composition sanity: the overlay file itself (no engine needed)
# ---------------------------------------------------------------------------


def _overlay_path() -> Any:
