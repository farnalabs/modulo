...
_JSONB_DB_TO_JSON_ORM: dict[str, frozenset[str]] = {
    ...
    # B2b (migration 0212): the three legacy runs blob columns are GONE
    # from BOTH sides, so their parity ignore entries are removed with them.
...
class TestParityIgnoreListClassify:
    ...
    def test_listed_entries_are_ignored(self) -> None:
        ...
        assert _is_benign_migration_managed(("modify_type", None, "runs", "cost_breakdown", {}, JSONB(), JSON()))
        # B2b parity REMOVES the three expiring runs-column examples; the
        # classifier's REMOVE entries exercising hypothetical cases remains
        # valid for the remaining parity convention (new-table columns keep
        # their own table's entries; cost_breakdown stays).
        assert not _is_benign_migration_managed(("modify_type", None, "runs", "outputs_json", {}, JSONB(), JSON())), (
            "the B2b expiry for the runs blob columns must NOT persist after 0212"
        )
        # B2b (0212): the runs blob columns are gone from BOTH sides - a
        ...
        assert not _is_benign_migration_managed(("remove_column", None, "runs", Column("outputs_json", JSONB()))), (
            "the B2b remove_column expiry for the runs blob columns must NOT persist after 0212"
        )
