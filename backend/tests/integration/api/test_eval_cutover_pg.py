...
# ---------------------------------------------------------------------------
# Criterion 12 - suite-aggregator filter parity on a real schema
# ---------------------------------------------------------------------------


async def test_suite_aggregator_filter_parity_and_softdelete_delta(
    pg_env: McpEnv,
) -> None:
    """Criterion 12 - ``_check_eval_suites`` reads ``evals`` and returns the
    same aggregation the legacy ``eval_definitions`` filter would; the row
    delta seen by the new source equals the soft-deleted suite-scoped count."""
    from datetime import UTC, datetime

    from modulo.core.eval_engine import (
        EvalResult as EngineEvalResult,
    )
    from modulo.core.eval_engine import SuiteOutcome, evaluate_suite
    from modulo.core.pipeline_engine.executor import PipelineExecutor

    suite_id = f"suite-parity-{uuid.uuid4().hex[:8]}"

    def_a = uuid.uuid4()
    def_b = uuid.uuid4()
    def_deleted = uuid.uuid4()
    run_id = uuid.uuid4()
    snapshot_id = uuid.uuid4()

    # A real run for eval_results' cross-org FK (eval_results.run_id     runs).
    async with pg_env.engine.begin() as conn:
        await conn.execute(
            sa_text(
            ...
            {
                "id": str(snapshot_id),
                "pid": str(pg_env.pipeline_id),
                "oid": str(pg_env.org_id),
            },
        )
        await conn.execute(
            sa_text(
                ...
                "run_number, status) "
                "VALUES (:id, :oid, :pid, :sid, 'manual', :ih, '{}'::json, :thread, 1, 'complete')"
            ),
            {
                "id": str(run_id),
                "oid": str(pg_env.org_id),
                ...
                "sid": str(snapshot_id),
                "ih": uuid.uuid4().hex,
                "thread": f"{pg_env.org_id}:{run_id}",
            },
        )

    # Seed config: threshold 0.5 exists on BOTH tables (legacy and Eval row).
    # The deleted row exists ONLY as an Eval row (chunk-3's backfill seams are
    # deleted at the "soft-delete" of the new path; historical legacy rows
    # cannot un-delete, so the count delta asserted below is the discriminator.)
    async with pg_env.engine.begin() as conn:
        for eval_id, name in ((def_a, "parity-a"), (def_b, "parity-b")):
            await conn.execute(
                sa_text(
                    "INSERT INTO eval_definitions (id, organisation_id, pipeline_id, name, "
                    "eval_type, config_json, failure_behaviour, pass_threshold, suite_id, "
                    "account_id) VALUES (:id, :oid, :pid, :name, 'regex', '{}'::json, "
                    "'warn', 0.5, :suite, :aid)"
                ),
                {
                    "id": str(eval_id),
                    ...
                    "name": name,
                    "suite": suite_id,
                    "aid": str(pg_env.admin_id),
                },
            )
            # Mirrored Eval row on the new table (the dual-write this chunk
            # is cutting over FROM eval_definitions).
            await conn.execute(
                sa_text(
                    "INSERT INTO evals (id, organisation_id, pipeline_id, node_id, name, "
                    "eval_type, config_json, pass_threshold, suite_id, account_id, "
                    "version) VALUES (:id, :oid, :pid, NULL, :name, 'regex', '{}'::jsonb, "
                    "0.5, :suite, :aid, 1)"
                ),
                {
                    "id": str(eval_id),
        ...
        # Soft-deleted suite-scoped Eval (new-table) WITHOUT a legacy twin.
        await conn.execute(
            sa_text(
                "INSERT INTO evals (id, organisation_id, pipeline_id, node_id, name, "
                "eval_type, config_json, pass_threshold, suite_id, account_id, deleted_at, deleted_by) "
                "VALUES (:id, :oid, :pid, NULL, 'parity-deleted', 'regex', '{}'::jsonb, 0.5, :suite, "
                ":aid, NOW(), :aid)"
            ),
            {
                "id": str(def_deleted),
    ...
    # Run with full coverage of the LIVE evals only: passed results.
    async with pg_env.engine.begin() as conn:
        for eval_id in (def_a, def_b):
            await conn.execute(
                sa_text(
                    "INSERT INTO eval_results (id, organisation_id, run_id, eval_id, "
                    "passed, score) VALUES (:id, :oid, :rid, :eid, true, 0.6)"
                ),
                {
                    "id": str(uuid.uuid4()),
                    "oid": str(pg_env.org_id),
    ...
    async with pg_env.engine.connect() as conn:
        legacy_rows_all = (
            await conn.execute(
                sa_text(
                    "SELECT id, suite_id, pass_threshold, eval_type FROM eval_definitions WHERE suite_id = :s",
                ),
                {"s": suite_id},
            )
        ).fetchall()
        legacy_defs = [
            ...
            for r in legacy_rows_all
        ]
        results_rows = (
            await conn.execute(
                sa_text("SELECT id, run_id, eval_id, passed, score FROM eval_results WHERE run_id = :r"),
                {"r": str(run_id)},
            )
        ).fetchall()
        legacy_eval_results = [
    ...
    # Legacy-path recomputation: the executor's legacy aggregation logic
    # (fully deterministic from the seeded rows) reproduced over the same
    # eval_definitions rows the legacy path would have read.
    expected_eval_ids = {d.id for d in legacy_defs}
    persisted_eval_ids = {r.eval_id for r in legacy_eval_results}
    legacy_total = len(legacy_eval_results)
    legacy_passed = sum(1 for r in legacy_eval_results if r.passed)
    if expected_eval_ids - persisted_eval_ids:
        legacy_outcome = SuiteOutcome.INDETERMINATE
        legacy_aggregate = 0.0
    else:
        threshold = next((d.pass_threshold for d in legacy_defs if d.pass_threshold is not None), None)
        engine_result = evaluate_suite(
            eval_results=[
                EngineEvalResult(
                    id=r.id,
                    run_id=r.run_id,
                ...
                for r in legacy_eval_results
            ],
            suite_id=suite_id,
            pass_threshold=float(threshold) if threshold is not None else None,
        )
        legacy_outcome = engine_result.outcome
        legacy_aggregate = engine_result.aggregate_score

    # The new path: the aggregator reads evals + eval_results.
    executor = PipelineExecutor.__new__(PipelineExecutor)
    factory = async_sessionmaker(pg_env.engine, expire_on_commit=False)
    async with factory() as session:
        suite_results = await executor._check_eval_suites(session, run_id, pg_env.pipeline_id)
    assert len(suite_results) == 1, f"expected one suite result, got {suite_results}"
    actual = suite_results[0]

    # Parity assertion: the new aggregation must match the legacy recompute.
    assert actual.suite_id == suite_id
    assert actual.outcome == legacy_outcome
    assert actual.total_evals == legacy_total
    assert actual.passed_evals == legacy_passed
    assert abs(actual.aggregate_score - legacy_aggregate) < 1e-9

    # Count-difference assertion: soft-deleted suite-scoped rows are the only
    # rows that no longer surface.
    soft_deleted_suite_scoped = int(
        await pg_env.scalar(
            "SELECT COUNT(*) FROM evals WHERE suite_id = :s AND deleted_at IS NOT NULL",
            {"s": suite_id},
        )
    )
    assert soft_deleted_suite_scoped == 1
    # Parity: the new-path total must equal the legacy (eval_definitions)
    # row count; the evals table additionally holds exactly the soft-deleted
    # rows the new filter drops.
    total_evals_rows = int(await pg_env.scalar("SELECT COUNT(*) FROM evals WHERE suite_id = :s", {"s": suite_id}))
    assert total_evals_rows == legacy_total + soft_deleted_suite_scoped
    assert actual.total_evals == legacy_total


# ---------------------------------------------------------------------------
# Criteria 14 / 17 / 24 - schema-shape, gate action, suite-scoped no-gate
# ---------------------------------------------------------------------------
