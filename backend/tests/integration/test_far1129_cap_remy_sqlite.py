"""FAR-1129 cap-remy: SkillLoader against a REAL async DB (SQLite) boundary.

The existing Remy tests mock ``AsyncSession`` end-to-end, so the SQL the
``SkillLoader`` actually executes — the ``RemySkill`` / ``Account`` /
``OrgMembership`` / ``Organisation`` and ``RemyConfigService`` /
``RemyContextSourceService`` reads — is never exercised against a real
...
rows carry explicit ids; the ``RemyRedisRegistry`` (cap-remy's key-value half)
...
# The system_config key RemyConfigService reads for org-level Remy config.
_REMY_CONFIG_KEY_PREFIX = "remy_config:"
...
async def _seed_org_context(session: AsyncSession) -> dict[str, uuid.UUID]:
    ...
    session.add(
        Account(
            id=user_id,
            email=f"remy-{token}@loopback.local",
            display_name=f"Remy Tester {token}",
            password_hash=None,
            auth_provider="local",
            active=True,
    ...
    session.add(
        SystemConfig(
            id=uuid.uuid4(),
            key=f"{_REMY_CONFIG_KEY_PREFIX}{org_id}",
            value={
                "system_prompt": "You are Remy, the Modulo assistant.",
...
async def test_build_system_prompt_reads_real_rows(
    ...
    assert "You are Remy, the Modulo assistant." in prompt
    assert "Answer tersely." in prompt
    assert "Loopback Org" in prompt
    assert "Remy Tester" in prompt
