"""Integration test for migration 0212 (FAR-583 B2b — the DROP migration).

Runs the REAL alembic ``upgrade`` of 0212 against a live Postgres
...
revision just before 0212 so the legacy ``runs`` blob columns still exist.
Covers every stage of the B2b spec:
...
* the INSERT-only repair inserts the absent new-table rows for a TERMINAL
  pre-cutoff run whose legacy blobs have no new-table representation;
...
MIGRATION_REV = "heads"
PREV_REV = "0211_variant_batch_state"
...
async def b2b_db_url(db_url: str, monkeypatch: pytest.MonkeyPatch) -> Any:
    """A private Postgres database stepped up to ``PREV_REV`` (pre-0212).
    ...
    revision BEFORE the B2b drop so the legacy runs blob columns still
    exist, and drop the database again afterwards.
    """
    admin_engine = create_async_engine(db_url, poolclass=NullPool, execution_options={"isolation_level": "AUTOCOMMIT"})
    db_name = f"b2b_iso_{uuid.uuid4().hex[:10]}"
...
async def _seed_b2b_world(db_url: str) -> dict[str, Any]:
        ...
        async with engine.begin() as conn:
            await conn.execute(
                text(
                    ...
                    "VALUES (:id, 'b2b@example.com', 'b2b', 'hash', 'local', true)"
                ),
                {"id": str(aid)},
            )
            await conn.execute(
                text(
                    "INSERT INTO organisations (id, name, slug, settings_json) VALUES (:id, 'b2b', 'b2b', '{}'::json)"
                ),
                {"id": str(oid)},
            )
            await conn.execute(
                text(
                    ...
                    "VALUES (:id, :oid, :aid, 'b2b pipeline', 10, 30, 300, '{}'::json, '[]'::json)"
                ),
    ...
    run_c = uuid.uuid4()  # markers PRESENT-key value divergence
    run_e = uuid.uuid4()  # unknown-status run with an unparseable marker key
    run_f = uuid.uuid4()  # junk scalar legacy blob (structural anomaly gate)
    ...
    try:
        async with engine.begin() as conn:
            for idx, (run_id, status, outputs, telemetry, markers) in enumerate(rows):
                ...
                # (it is parked at PREV_REV, before 0212 drops them).
                blobs_sql = ""
...
async def test_0212_drop_legs_and_abort_gates(b2b_db_url, monkeypatch: pytest.MonkeyPatch) -> None:
    db_url = str(b2b_db_url)
    ids = await _seed_b2b_world(db_url)
    ...
    assert await _alembic_version(db_url) == PREV_REV, "an aborted B2b attempt must roll back fully"
    await _delete_run(db_url, ids["runs"]["run_d"])

    # -- Attempt 2: the structural-anomaly gate aborts on a scalar blob. ------
    with pytest.raises(RuntimeError, match="NOT a jsonb object"):
        await _upgrade(db_url)
    ...
    await _upgrade(db_url)
    assert await _alembic_version(db_url) == "0212_drop_runs_blob_columns"
    ...
    assert not ({*RUN_LEGACY_COLUMNS} & cols), "the three runs blob columns must be GONE post-0212"
