"""Integration tests for migration 0212 (FAR-583 B2b) — Testcontainers Postgres.
        ...
        rows = (
            await conn.execute(
                text(
                    ...
                    "WHERE r.status IN ('running', 'claimed', 'awaiting_human') AND r.created_at < :cutoff"
                ),
                {"cutoff": datetime(2026, 9, 10, 11, 25, 42, tzinfo=UTC)},
            )
        ).scalar_one()
