...
async def test_cross_tenant_pipeline_invisibility(
    ...
    data = resp.json()
    our_pipelines = data if isinstance(data, list) else data.get("items", data.get("pipelines", []))

    for other in tenant_matrix.all():
        if other is None or other.org_id == tenant.org_id:
            continue

        async with httpx.AsyncClient(base_url=other.base_url, verify=False, timeout=60.0) as other_client:
            ...
            other_pipelines = other_data if isinstance(other_data, list) else (
                other_data.get("items") or other_data.get("pipelines", [])
            )
