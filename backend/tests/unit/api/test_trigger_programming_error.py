"""Unit tests: trigger route handlers return 501 on ProgrammingError."""
...
from modulo.auth.dependencies import get_current_tenant_user, get_current_tenant_user_optional, get_current_user
...
def _make_mock_session(exc_side_effect: type[Exception] | None = ProgrammingError("stmt", {}, "mock")) -> AsyncMock:
...
# ---------------------------------------------------------------------------
# triggers.py — all 12 handlers
# ---------------------------------------------------------------------------


def test_list_triggers_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
...
def test_update_cron_config_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_preview_cron_schedule_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_update_polling_config_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with (
...
def test_test_polling_condition_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with (
        patch("modulo.api.routes.triggers.set_rls_org"),
...
def test_create_trigger_programming_error(client: TestClient) -> None:
    session = _make_mock_session(None)
    session.flush = AsyncMock(side_effect=ProgrammingError("stmt", {}, "table not found"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_update_trigger_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_delete_trigger_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_toggle_trigger_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_test_trigger_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_list_trigger_events_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_list_pipeline_triggers_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
# ---------------------------------------------------------------------------
# admin_triggers.py — list_trigger_events handler
# ---------------------------------------------------------------------------


def test_admin_list_trigger_events_programming_error(client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
...
def test_admin_list_trigger_events_count_programming_error(client: TestClient) -> None:
    """Test that the count query outside session.begin() also catches ProgrammingError."""
    session = _make_mock_session(None)
    execute_result = MagicMock()
    execute_result.scalars = MagicMock(return_value=MagicMock(all=MagicMock(return_value=[])))
    session.execute = AsyncMock(return_value=execute_result)
    # Second call (count query) raises ProgrammingError
    session.execute.side_effect = [execute_result, ProgrammingError("stmt", {}, "table not found")]
...
# ---------------------------------------------------------------------------
# webhooks.py — all 3 handlers
# ---------------------------------------------------------------------------


def test_receive_webhook_programming_error(engine_client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with (
...
def test_replay_webhook_programming_error(engine_client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
    with (
        patch("modulo.api.routes.webhooks.set_rls_org"),
...
def test_cleanup_expired_programming_error(engine_client: TestClient) -> None:
    session = _make_mock_session(ProgrammingError("stmt", {}, "table not found"))
