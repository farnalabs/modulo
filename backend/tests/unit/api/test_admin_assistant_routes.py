"""Unit tests for /api/v1/admin/assistant endpoints (config, skills, context sources).

Unit tier: no DB — the SQLAlchemy session is a contract-correct AsyncMock and
the AssistantContextSourceService is patched at its source module.
"""
...
def _make_skill(account_id: uuid.UUID | None = None) -> AssistantSkill:
...
def test_update_config_updates_existing_entry(client: TestClient) -> None:
    ...
    _queue_executes(client.mock_session, _config_result(entry))  # type: ignore[attr-defined]
    resp = client.put("/api/v1/admin/assistant/config", json={"system_prompt": "New prompt"})
    assert resp.status_code == 200, resp.text
...
def test_list_org_skills_db_error_returns_503(client: TestClient) -> None:
    with patch("modulo.api.routes.admin_assistant.set_rls_org", new_callable=AsyncMock, side_effect=SQLAlchemyError()):
        resp = client.get("/api/v1/admin/assistant/skills")
    assert resp.status_code == 503
...
def test_create_org_skill_db_error_returns_503(client: TestClient) -> None:
    with (
        ...
        patch("modulo.api.routes.admin_assistant.AssistantSkill", side_effect=SQLAlchemyError("insert failed")),
    ):
        resp = client.post("/api/v1/admin/assistant/skills", json={"name": "X", "body": "b"})
    assert resp.status_code == 503
...
def test_update_org_skill_applies_partial_fields(client: TestClient) -> None:
    ...
    resp = client.put(
        f"/api/v1/admin/assistant/skills/{skill.id}",
...
def test_update_org_skill_missing_returns_404(client: TestClient) -> None:
    ...
    resp = client.put(f"/api/v1/admin/assistant/skills/{uuid.uuid4()}", json={"name": "X"})
...
def test_update_org_user_level_skill_returns_404(client: TestClient) -> None:
    ...
    resp = client.put(f"/api/v1/admin/assistant/skills/{user_skill.id}", json={"name": "X"})
    assert resp.status_code == 404
...
def test_delete_org_skill_missing_returns_404(client: TestClient) -> None:
    client.mock_session.get = AsyncMock(return_value=None)  # type: ignore[attr-defined]
    resp = client.delete(f"/api/v1/admin/assistant/skills/{uuid.uuid4()}")
    assert resp.status_code == 404
...
def _patch_context_source_service(org_defaults: dict[str, str]) -> Any:
    ...
    return patch("modulo.core.assistant.context_source_service.AssistantContextSourceService", return_value=service)
...
def test_get_org_context_sources_merges_builtin_and_org(client: TestClient) -> None:
    with _patch_context_source_service({"manual_context": "off"}):
        resp = client.get("/api/v1/admin/assistant/context-sources")
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_set_org_context_source_returns_defaults(client: TestClient) -> None:
    with _patch_context_source_service({"page_context": "tool"}):
        resp = client.put(
            "/api/v1/admin/assistant/context-sources/page_context",
...
def test_reset_org_context_sources_deletes_rows(client: TestClient) -> None:
    ...
    _queue_executes(client.mock_session, result)  # type: ignore[attr-defined]
    resp = client.delete("/api/v1/admin/assistant/context-sources")
    assert resp.status_code == 200, resp.text
    assert not resp.json()
...
def test_update_config_programming_error_returns_501(client: TestClient) -> None:
    ...
    resp = client.put("/api/v1/admin/assistant/config", json={"system_prompt": "X"})
    assert resp.status_code == 501
...
def test_list_org_skills_programming_error_returns_501(client: TestClient) -> None:
    with patch(
        "modulo.api.routes.admin_assistant.set_rls_org",
...
def test_update_org_skill_programming_error_returns_501(client: TestClient) -> None:
    ...
    with patch(
        ...
        resp = client.put(f"/api/v1/admin/assistant/skills/{skill.id}", json={"name": "X"})
    assert resp.status_code == 501
...
def test_get_org_context_sources_programming_error_returns_501(client: TestClient) -> None:
    ...
    with patch(
        "modulo.core.assistant.context_source_service.AssistantContextSourceService",
...
def test_get_user_skills_returns_matching_skills() -> None:
    from modulo.api.routes.admin_assistant import get_user_skills
...
def test_get_user_skill_or_404_raises_on_missing() -> None:
    from modulo.api.routes.admin_assistant import get_user_skill_or_404
...
def test_update_config_non_dict_existing_value_treated_as_empty(client: TestClient) -> None:
    ...
    entry.value = "not-a-dict"
    _queue_executes(client.mock_session, _config_result(entry))  # type: ignore[attr-defined]
    resp = client.put("/api/v1/admin/assistant/config", json={"system_prompt": "New"})
    assert resp.status_code == 200
