...
from modulo.api.dependencies import get_settings, get_db_session
...
async def test_create_org_slug_collision(client_admin):
    ...
    existing = Organisation(id=uuid4(), name="Existing", slug="taken", status="active", created_at=datetime.now(timezone.utc))
...
async def test_create_org_duplicate_slug_orig(client_admin):
    ...
    existing_org = Organisation(
        id=uuid4(), name="Existing", slug="dup-slug", status="active", created_at=datetime.now(timezone.utc)
    )
    original = admin_orgs.get_organisation_by_slug
...
async def test_create_org_user_success(client_admin):
    ...
    target_org = Organisation(
        id=target_org_id, name="Target Org", slug="target", status="active", created_at=datetime.now(timezone.utc)
    )

    # Mock get_organisation to return the target org
    original_get_org = admin_orgs.get_organisation
    admin_orgs.get_organisation = AsyncMock(return_value=target_org)

    # Mock get_user_by_email to return None (no duplicate)
