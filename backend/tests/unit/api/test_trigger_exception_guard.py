"""Unit tests: trigger route handlers return 500 on unexpected Exception."""
...
def _make_mock_session(exc: type[Exception] | None = None) -> AsyncMock:
    session = AsyncMock()
    begin_cm = AsyncMock()
...
def _assert_exception_returns_500(resp) -> None:
...
def test_list_triggers_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    ...
    _assert_exception_returns_500(resp)


def test_update_cron_config_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_preview_cron_schedule_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_update_polling_config_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with (
...
def test_test_polling_condition_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with (
        patch("modulo.api.routes.triggers.set_rls_org"),
...
def test_create_trigger_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    session.flush = AsyncMock(side_effect=ValueError("crash"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_update_trigger_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_delete_trigger_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_toggle_trigger_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_test_trigger_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_list_trigger_events_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
def test_list_pipeline_triggers_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with patch("modulo.api.routes.triggers.set_rls_org"):
...
# ---------------------------------------------------------------------------
# admin_triggers.py — list_trigger_events handler (main + count queries)
# ---------------------------------------------------------------------------


def test_admin_list_trigger_events_exception_returns_500(client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
...
# ---------------------------------------------------------------------------
# webhooks.py — receive_webhook and replay_webhook
# ---------------------------------------------------------------------------


def test_receive_webhook_exception_returns_500(engine_client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with (
...
def test_replay_webhook_exception_returns_500(engine_client: TestClient) -> None:
    session = _make_mock_session(ValueError("unexpected"))
    with (
        patch("modulo.api.routes.webhooks.set_rls_org"),
