"""Unit tests: ProgrammingError and SQLAlchemyError on org admin routes."""
...
def _sqlalchemy_error_side_effect(*args: object, **kwargs: object) -> None:
...
def sqlalchemy_error_session() -> AsyncMock:
    mock_session = AsyncMock()
    mock_session.execute.side_effect = _sqlalchemy_error_side_effect
    mock_session.begin = MagicMock(
        return_value=AsyncMock(
            __aenter__=AsyncMock(side_effect=_sqlalchemy_error_side_effect),
            __aexit__=AsyncMock(return_value=False),
        )
    )
    return mock_session
...
@pytest.fixture()
def client_admin_sqla(sqlalchemy_error_session: AsyncMock) -> Generator[TestClient, None, None]:
    async def override_session() -> AsyncGenerator[AsyncMock, None]:
        yield sqlalchemy_error_session
...
@pytest.fixture()
def client_system_admin_sqla(sqlalchemy_error_session: AsyncMock) -> Generator[TestClient, None, None]:
    async def override_session() -> AsyncGenerator[AsyncMock, None]:
        yield sqlalchemy_error_session
...
class TestAdminOrgsSQLAlchemyError:
    def test_create_org_returns_503(self, client_system_admin_sqla: TestClient) -> None:
        resp = client_system_admin_sqla.post(
            "/api/v1/admin/orgs",
            json={"name": "Test Org", "slug": "test-org"},
        )
        assert resp.status_code == 503

    def test_list_orgs_returns_503(self, client_system_admin_sqla: TestClient) -> None:
        resp = client_system_admin_sqla.get("/api/v1/admin/orgs")
        assert resp.status_code == 503

    def test_create_org_user_returns_503(self, client_system_admin_sqla: TestClient, org_id_str: str) -> None:
        resp = client_system_admin_sqla.post(
            f"/api/v1/admin/orgs/{org_id_str}/users",
            json={
                "email": "test@example.com",
    ...
    def test_delete_org_returns_503(self, client_system_admin_sqla: TestClient, org_id_str: str) -> None:
        resp = client_system_admin_sqla.delete(f"/api/v1/admin/orgs/{org_id_str}")
        assert resp.status_code == 503

    def test_get_org_license_returns_503(self, client_system_admin_sqla: TestClient, org_id_str: str) -> None:
        resp = client_system_admin_sqla.get(f"/api/v1/admin/orgs/{org_id_str}/license")
        assert resp.status_code == 503

    def test_set_org_license_returns_503(self, client_system_admin_sqla: TestClient, org_id_str: str) -> None:
        resp = client_system_admin_sqla.put(
            f"/api/v1/admin/orgs/{org_id_str}/license",
            json={"license_key": "test-key"},
        )
        assert resp.status_code == 503

    def test_remove_org_license_returns_503(self, client_system_admin_sqla: TestClient, org_id_str: str) -> None:
        resp = client_system_admin_sqla.delete(f"/api/v1/admin/orgs/{org_id_str}/license")
        assert resp.status_code == 503


class TestAdminOrgProfileSQLAlchemyError:
    def test_get_org_returns_503(self, client_admin_sqla: TestClient) -> None:
        resp = client_admin_sqla.get("/api/v1/admin/org")
        assert resp.status_code == 503

    def test_update_org_returns_503(self, client_admin_sqla: TestClient) -> None:
        resp = client_admin_sqla.put("/api/v1/admin/org", json={"name": "Updated Org"})
        assert resp.status_code == 503

    def test_regenerate_api_key_returns_503(self, client_admin_sqla: TestClient) -> None:
        resp = client_admin_sqla.post("/api/v1/admin/org/regenerate-api-key")
        assert resp.status_code == 503


class TestOrgDeletionSQLAlchemyError:
    def test_request_deletion_returns_503(self, client_admin_sqla: TestClient) -> None:
        resp = client_admin_sqla.post("/api/v1/admin/org/deletion-request")
        assert resp.status_code == 503

    def test_confirm_deletion_returns_503(self, client_admin_sqla: TestClient) -> None:
        resp = client_admin_sqla.post("/api/v1/admin/org/deletion-confirm", json={"token": "test-token"})
        assert resp.status_code == 503

    def test_cancel_deletion_returns_503(self, client_admin_sqla: TestClient) -> None:
        resp = client_admin_sqla.patch("/api/v1/admin/org/deletion-cancel")
        assert resp.status_code == 503

    def test_export_org_data_returns_503(self, client_admin_sqla: TestClient) -> None:
        resp = client_admin_sqla.get("/api/v1/admin/org/export")
        assert resp.status_code == 503

    def test_delete_org_immediate_returns_503(self, client_admin_sqla: TestClient) -> None:
        resp = client_admin_sqla.delete("/api/v1/admin/org")
        assert resp.status_code == 503
