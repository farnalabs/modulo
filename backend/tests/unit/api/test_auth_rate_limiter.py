...
class TestAuthRateLimiterWithMockRedis:
    ...
    @pytest.fixture
    def limiter(self, mock_redis) -> AuthRateLimiter:
    ...
    async def test_check_login_allows_under_max_attempts(self, limiter, mock_redis) -> None:
        mock_redis.ttl = AsyncMock(return_value=-2)
        ...
        pipe.zremrangebyscore = MagicMock(return_value=pipe)
        pipe.zcard = MagicMock(return_value=pipe)
        pipe.execute = AsyncMock(return_value=(None, 9))
        mock_redis.pipeline = MagicMock(return_value=pipe)
    ...
    async def test_record_failure_increments_counter(self, limiter, mock_redis) -> None:
        mock_redis.ttl = AsyncMock(return_value=-2)
        mock_redis.zadd = AsyncMock()
        mock_redis.expire = AsyncMock()
        mock_redis.setex = AsyncMock()
        ...
        pipe.zremrangebyscore = MagicMock(return_value=pipe)
        pipe.zcard = MagicMock(return_value=pipe)
        pipe.execute = AsyncMock(side_effect=[(None, 2), (None, 10)])
        mock_redis.pipeline = MagicMock(return_value=pipe)
    ...
    async def test_record_success_resets_counter(self, limiter, mock_redis) -> None:
        mock_redis.ttl = AsyncMock(return_value=-2)
        mock_redis.setex = AsyncMock()
        mock_redis.zadd = AsyncMock()
        mock_redis.expire = AsyncMock()
        ...
        pipe.zremrangebyscore = MagicMock(return_value=pipe)
        pipe.zcard = MagicMock(return_value=pipe)
        pipe.execute = AsyncMock(side_effect=[(None, 10), (None, 0)])
        mock_redis.pipeline = MagicMock(return_value=pipe)
        ...
        pipe2 = MagicMock()
        pipe2.delete = MagicMock(return_value=pipe2)
        pipe2.execute = AsyncMock(return_value=(True, True))
        mock_redis.pipeline = MagicMock(return_value=pipe2)
    ...
    async def test_ips_are_independent(self, limiter, mock_redis) -> None:
        mock_redis.ttl = AsyncMock(return_value=-2)
        mock_redis.setex = AsyncMock()
        mock_redis.zadd = AsyncMock()
        mock_redis.expire = AsyncMock()
    ...
    async def test_lockout_persists_across_checks(self, limiter, mock_redis) -> None:
        mock_redis.ttl = AsyncMock(side_effect=[-2, 45])
        mock_redis.setex = AsyncMock()
        mock_redis.zadd = AsyncMock()
        mock_redis.expire = AsyncMock()
        pipe = MagicMock()
