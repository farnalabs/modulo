...
class TestNonGatedEndpoints:
    ...
    def test_get_costs_unaffected(self, client: TestClient) -> None:
    ...
    def test_export_unaffected(self, client: TestClient) -> None:
        rows = [{"entity_id": str(_TEAM_ID), "entity_name": "Team A", "total_spend_usd": 100.0, "total_runs": 5}]
        with (
            patch("modulo.api.routes.costs.get_cost_report", return_value=rows),
            patch("modulo.api.routes.costs.set_rls_org"),
    ...
    def test_create_report_unaffected(self, client: TestClient) -> None:
    ...
    def test_anomalies_unaffected(self, client: TestClient) -> None:
