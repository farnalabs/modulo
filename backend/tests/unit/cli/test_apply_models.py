...
class TestPipelineGraphSecretParity:
    ...
    ``server_view()`` is generated by the REAL ``mask_pipeline_graph_node``,
    never hand-built: a hand-built expectation pins only the tiers the sample
    author thought of, which is exactly how the deep VALUE tier slipped
    through — a secret-shaped value under a NON-sensitive deep key."""
        ...
        "nodes": [
            {
                ...
                "env_vars": {
                    ...
                    "LOG_LEVEL": "debug",
                    # Falsy declared secret: the server displays it unmasked.
                    "PYPI_TOKEN": "",
                },
                "context_files": {"runbook.md": "token ghp_00112233445566778899aabbccddeeff00112233"},
                # Deep tier, three shapes: a sensitive LEAF key (whole mask),
                # a falsy secret under a sensitive key (stays unmasked), and a
                # secret-shaped VALUE under a NON-sensitive key (value tier).
                "parameter_overrides": {
                    "nested": {"db_password": "s3cr3t", "signing_secret": ""},
                    "base_url": "https://api.example.com/v1?token=ghp_00112233445566778899aabbccddeeff00112233",
                },
            }
        ],
        "edges": [],
    ...
    @classmethod
    def server_view(cls) -> dict:
        """What the API read of DECLARED shows — produced by the shipped
        server mask so a masker-tier change on either side fails this suite."""
        from modulo.api.middleware.sensitive_mask import mask_pipeline_graph_node

        return {
            "nodes": [mask_pipeline_graph_node(dict(node)) for node in cls.DECLARED["nodes"]],
            "edges": list(cls.DECLARED.get("edges") or []),
        }

    def test_server_display_pins_all_mask_tiers(self) -> None:
        """Hand-pinned expectations against the REAL mask output, so a tier
        changing server-side fails loudly instead of silently re-defining
        'parity'."""
        from modulo.core.secret_patterns import SENSITIVE_VALUE_MASK

        node = self.server_view()["nodes"][0]
        # env tier: sensitive key -> whole mask; other keys value-compiled.
        assert node["env_vars"]["MODULO_USERS"] == SENSITIVE_VALUE_MASK
        assert node["env_vars"]["LOG_LEVEL"] == "debug"
        # falsy declared secret is displayed unmasked (mask_sensitive_value).
        assert not node["env_vars"]["PYPI_TOKEN"]
        # context tier: value redaction in place.
        assert node["context_files"]["runbook.md"] == f"token {SENSITIVE_VALUE_MASK}"
        # deep tier, key shape.
        assert node["parameter_overrides"]["nested"]["db_password"] == SENSITIVE_VALUE_MASK
        assert not node["parameter_overrides"]["nested"]["signing_secret"]
        # deep tier, VALUE shape — the tier the old hand-built SERVER_VIEW missed.
        base_url = node["parameter_overrides"]["base_url"]
        assert "ghp_" not in base_url
        assert base_url == f"https://api.example.com/v1?token={SENSITIVE_VALUE_MASK}"

    def test_declared_view_equals_server_display(self) -> None:
        """The invariant the runtime verification pass checked directly: the
        CLI's desired-side view must be identical to what the server displays.
        Any divergence is phantom drift (perpetual ``updated`` + graph re-PATCH)."""
        from modulo.cli.apply.models import strip_secret_shaped_graph

        assert strip_secret_shaped_graph(self.DECLARED) == self.server_view()

    def test_deep_value_tier_hashes_equal(self) -> None:
        """A secret-shaped value under a NON-sensitive deep key must redact
        identically on both sides — fails if the value tier is dropped from
        ``_mask_graph_deep_config_for_hash``."""
        from modulo.api.middleware.sensitive_mask import mask_pipeline_graph_node
        from modulo.cli.apply.models import strip_secret_shaped_graph
        ...
        server = {"nodes": [mask_pipeline_graph_node(dict(n)) for n in declared["nodes"]], "edges": []}
        cli_view = strip_secret_shaped_graph(declared)
        assert "ghp_" not in cli_view["nodes"][0]["parameter_overrides"]["base_url"]
        assert canonical_hash(cli_view) == canonical_hash(strip_secret_shaped_graph(server))

    def test_empty_string_secret_shows_unmasked_on_both_sides(self) -> None:
        """The falsy-value rule (``mask_sensitive_value`` leaves ``""``
        unmasked) must hold in the CLI hash view too, or the managed view
        shows a sentinel the API never returns."""
        from modulo.cli.apply.models import strip_secret_shaped_graph
        from modulo.cli.apply.plan import canonical_hash

        stripped = strip_secret_shaped_graph(self.DECLARED)
        assert not stripped["nodes"][0]["env_vars"]["PYPI_TOKEN"]
        assert not stripped["nodes"][0]["parameter_overrides"]["nested"]["signing_secret"]
        assert canonical_hash(stripped) == canonical_hash(strip_secret_shaped_graph(self.server_view()))
    ...
    def test_masked_current_view_round_trips(self) -> None:
        ...
        server_view = self.server_view()
        once = strip_secret_shaped_graph(server_view)
