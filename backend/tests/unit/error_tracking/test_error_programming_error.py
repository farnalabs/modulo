...
class TestErrors:
    ...
    def test_ingest_programming_error_returns_501(self):
        app = self._setup_app()
        with patch("modulo.api.routes.errors._service.ingest_batch", AsyncMock(side_effect=_PROGRAMMING_ERROR)):
            ...
            with patch.object(store, "verify_hmac", AsyncMock(return_value=True)):
                ...
                resp = client.post(
                    "/api/v1/errors/ingest", json=INGEST_PAYLOAD, headers={"X-Modulo-Error-Token": "test"}
                )
    ...
    def test_ingest_sqlalchemy_error_returns_503(self):
        app = self._setup_app()
        with patch("modulo.api.routes.errors._service.ingest_batch", AsyncMock(side_effect=_SQLALCHEMY_ERROR)):
            from modulo.api.routes.errors import _get_key_store
    ...
    def test_list_groups_programming_error_returns_501(self):
        app = self._setup_app()
        with patch("modulo.api.routes.errors.get_error_groups", AsyncMock(side_effect=_PROGRAMMING_ERROR)):
            client = TestClient(app)
    ...
    def test_list_groups_sqlalchemy_error_returns_503(self):
        app = self._setup_app()
        with patch("modulo.api.routes.errors.get_error_groups", AsyncMock(side_effect=_SQLALCHEMY_ERROR)):
            client = TestClient(app)
            resp = client.get("/api/v1/errors")
            assert resp.status_code == 503
    ...
    def test_get_group_detail_programming_error_returns_501(self):
        app = self._setup_app()
        with patch("modulo.api.routes.errors.get_error_group", AsyncMock(side_effect=_PROGRAMMING_ERROR)):
            client = TestClient(app)
    ...
    def test_get_group_detail_sqlalchemy_error_returns_503(self):
        app = self._setup_app()
        with patch("modulo.api.routes.errors.get_error_group", AsyncMock(side_effect=_SQLALCHEMY_ERROR)):
            client = TestClient(app)
            resp = client.get(f"/api/v1/errors/{uuid.uuid4()}")
            assert resp.status_code == 503
    ...
    def test_patch_group_programming_error_returns_501(self):
        app = self._setup_app()
        with patch("modulo.api.routes.errors.update_error_group", AsyncMock(side_effect=_PROGRAMMING_ERROR)):
            client = TestClient(app)
    ...
    def test_patch_group_sqlalchemy_error_returns_503(self):
        app = self._setup_app()
        with patch("modulo.api.routes.errors.update_error_group", AsyncMock(side_effect=_SQLALCHEMY_ERROR)):
            client = TestClient(app)
            resp = client.patch(f"/api/v1/errors/{uuid.uuid4()}", json={"status": "resolved"})
            assert resp.status_code == 503
    ...
    def test_list_events_programming_error_returns_501(self):
        ...
        with (
            patch("modulo.api.routes.errors.get_error_group", AsyncMock(return_value=mock_group)),
            patch("modulo.api.routes.errors.get_error_events_by_group", AsyncMock(side_effect=_PROGRAMMING_ERROR)),
        ):
            client = TestClient(app)
    ...
    def test_list_events_sqlalchemy_error_returns_503(self):
        ...
        with (
            patch("modulo.api.routes.errors.get_error_group", AsyncMock(return_value=mock_group)),
            patch("modulo.api.routes.errors.get_error_events_by_group", AsyncMock(side_effect=_SQLALCHEMY_ERROR)),
        ):
            client = TestClient(app)
            resp = client.get(f"/api/v1/errors/{uuid.uuid4()}/events")
            assert resp.status_code == 503
...
def test_list_rules_programming_error_returns_501():
    ...
    with patch("modulo.api.routes.error_notification_rules.select", MagicMock(side_effect=_PROGRAMMING_ERROR)):
        client = TestClient(app)
...
def test_list_rules_sqlalchemy_error_returns_503():
    app, _ = _make_rules_app()
    with patch("modulo.api.routes.error_notification_rules.select", MagicMock(side_effect=_SQLALCHEMY_ERROR)):
        client = TestClient(app)
        resp = client.get("/api/v1/errors/notification-rules")
        assert resp.status_code == 503
...
def test_list_forwarders_programming_error_returns_501():
    app, _ = _make_fwd_app()
    with patch("modulo.api.routes.error_forwarder_config.select", MagicMock(side_effect=_PROGRAMMING_ERROR)):
        client = TestClient(app)
...
def test_list_forwarders_sqlalchemy_error_returns_503():
    app, _ = _make_fwd_app()
    with patch("modulo.api.routes.error_forwarder_config.select", MagicMock(side_effect=_SQLALCHEMY_ERROR)):
        client = TestClient(app)
        resp = client.get("/api/v1/errors/forwarders")
        assert resp.status_code == 503
