...
def backend():
    with patch("modulo.model_backends.jan.ChatOpenAI"):
        return JanBackend(api_key=None, model_id="llama3")
...
def test_backend_id(backend):
    assert backend.backend_id == "jan/llama3"
...
def test_custom_base_url():
    with patch("modulo.model_backends.jan.ChatOpenAI"):
        backend = JanBackend(api_key=None, model_id="llama3", base_url="http://jan:1337/v1")
