...
def backend():
    with patch("modulo.model_backends.lm_studio.ChatOpenAI"):
        return LmStudioBackend(api_key=None, model_id="llama3")
...
def test_backend_id(backend):
    assert backend.backend_id == "lm_studio/llama3"
...
def test_custom_base_url():
    with patch("modulo.model_backends.lm_studio.ChatOpenAI"):
        backend = LmStudioBackend(api_key=None, model_id="llama3", base_url="http://lm-studio:1234/v1")
...
def test_repr_does_not_leak_api_key(backend):
    ...
    assert "lm_studio" in repr(backend)
