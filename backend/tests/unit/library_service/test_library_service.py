...
def test_dogfood_agents_have_environment_capabilities():
    ...
    assert dogfood_agents["test-runner"].content_json["required_environment_capabilities"] == ["shell:exec", "python3.12", "uv"]
...
async def test_list_primitives_type_filter_propagated():
    ...
    with (
        patch("modulo.core.library_service.set_rls_org", new_callable=AsyncMock),
        patch("modulo.core.library_service.list_library_primitives", new_callable=AsyncMock, return_value=org_page) as mock_list,
    ):
