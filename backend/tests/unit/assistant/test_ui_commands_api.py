"""Unit tests for Assistant UI Command API endpoints — permission response, results, reset."""
...
def client():
    ...
    with patch("modulo.api.routes.assistant._get_registry", return_value=None):
...
def mock_session_ownership():
    ...
    with (
        ...
        patch("modulo.api.routes.assistant.get_db_session", return_value=session),
    ):
        yield
...
class TestPermissionResponseEndpoint:
    ...
    def test_approve_action(self, client, mock_session_ownership):
        ...
        resp = client.post(
            f"/api/v1/assistant/sessions/{SESSION_ID}/permission-response",
    ...
    def test_session_not_found_returns_404(self, client):
        with (
            patch("modulo.api.routes.assistant.set_rls_org", new_callable=AsyncMock),
            patch("modulo.api.routes.assistant.get_db_session") as mock_get_db,
        ):
...
class TestUiCommandResultsEndpoint:
    ...
    def test_submit_results(self, client, mock_session_ownership):
        ...
        resp = client.post(
            f"/api/v1/assistant/sessions/{SESSION_ID}/ui-command-results",
            json={
                "results": [
    ...
    def test_other_users_session_returns_404(self, client):
        ...
        try:
            with patch("modulo.api.routes.assistant.set_rls_org", new_callable=AsyncMock):
                resp = client.post(
                    f"/api/v1/assistant/sessions/{other_session_id}/ui-command-results",
                    json={"results": []},
                )
                assert resp.status_code == 404
        finally:
...
class TestResetPermissionsEndpoint:
    ...
    def test_reset_permissions(self, client, mock_session_ownership):
        ...
        resp = client.post(
            f"/api/v1/assistant/sessions/{SESSION_ID}/reset-permissions",
        )
        assert resp.status_code == 200
        assert resp.json() == {"status": "ok"}
    ...
    def test_other_users_session_returns_404(self, client):
        ...
        try:
            with patch("modulo.api.routes.assistant.set_rls_org", new_callable=AsyncMock):
                resp = client.post(
                    f"/api/v1/assistant/sessions/{other_session_id}/reset-permissions",
                )
                assert resp.status_code == 404
        finally:
            if orig is not None:
