"""Unit tests for AssistantConfigService — config CRUD and access control."""
...
class TestAssistantConfigDefaults:
    """Tests for AssistantConfig Pydantic model defaults."""
...
class TestAssistantConfigServiceGetConfig:
    """Tests for AssistantConfigService.get_config."""

    @pytest.fixture
    def service(self, mock_session: AsyncMock) -> AssistantConfigService:
        return AssistantConfigService(mock_session)
...
class TestAssistantConfigServiceUpdateConfig:
    """Tests for AssistantConfigService.update_config."""
    ...
    async def test_update_config_persists(
        ...
        with patch("modulo.core.assistant.config_service.update_config", new_callable=AsyncMock) as mock_set:
            config = AssistantConfig(
            ...
            assert kwargs["key"] == f"assistant_config:{org_id}"
    ...
    async def test_update_config_reraises_on_db_error(
        ...
        with (
            patch(
                "modulo.core.assistant.config_service.update_config",
                new_callable=AsyncMock,
                side_effect=SQLAlchemyError("write failed"),
            ),
            pytest.raises(SQLAlchemyError),
        ):
            await service.update_config(org_id, AssistantConfig(system_prompt="Nope"))


class TestAssistantConfigServiceCheckAccess:
    """Tests for AssistantConfigService.check_access."""
    ...
    async def test_check_access_denies_when_config_fetch_fails(
        ...
        user_id = uuid.uuid4()
        with patch(
            "modulo.core.assistant.config_service.get_config",
            new_callable=AsyncMock,
