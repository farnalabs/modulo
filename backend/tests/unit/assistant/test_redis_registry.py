"""Unit tests for AssistantRedisRegistry — all Redis calls are mocked."""
...
from modulo.core.assistant.redis_registry import (
    AssistantRedisRegistry,
    _json_object,
    _json_object_list,
    _redis_result,
)
...
@pytest.fixture
def registry(redis_client: MagicMock) -> AssistantRedisRegistry:
    """Return a AssistantRedisRegistry whose underlying Redis client is mocked."""
    with patch("redis.asyncio.Redis.from_url", return_value=redis_client) as mock_from_url:
        reg = AssistantRedisRegistry("redis://localhost:6379/0")
...
async def test_close_closes_client(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
...
async def test_set_permission_request_hsets_and_expires(
    registry: AssistantRedisRegistry, redis_client: MagicMock
) -> None:
    ...
    redis_client.hset.assert_awaited_once_with(
        "assistant:permission:req-1",
        mapping={"session_id": "sess-1", "tools": json.dumps(tools)},
    )
    redis_client.expire.assert_awaited_once_with("assistant:permission:req-1", 120)


async def test_get_permission_request_happy_path(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
...
async def test_get_permission_request_invalid_tools_warns_and_defaults_empty(
    registry: AssistantRedisRegistry, redis_client: MagicMock, caplog: pytest.LogCaptureFixture
) -> None:
    redis_client.hgetall.return_value = {"session_id": "sess-1", "tools": "{not json"}
    with caplog.at_level("WARNING", logger="modulo.core.assistant.redis_registry"):
        result = await registry.get_permission_request("req-1")
    assert result == {"session_id": "sess-1", "tools": []}
...
async def test_set_permission_decision_setex(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    decision = {"decision": "allow"}
    await registry.set_permission_decision("req-1", decision, ttl=120)
    redis_client.setex.assert_awaited_once_with("assistant:decision:req-1", 120, json.dumps(decision))
...
async def test_get_and_clear_permission_decision_happy_path(
    ...
    redis_client.delete.assert_awaited_once_with("assistant:decision:req-1")
...
async def test_set_ui_command_results_setex(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    results = [{"result": "ok"}]
    await registry.set_ui_command_results("sess-1", results, ttl=300)
    redis_client.setex.assert_awaited_once_with("assistant:ui_results:sess-1", 300, json.dumps(results))
...
async def test_get_and_clear_ui_command_results_happy_path(
    ...
    redis_client.delete.assert_awaited_once_with("assistant:ui_results:sess-1")
...
async def test_set_session_approval_hsets_and_expires(
    ...
    assert args.args[0] == "assistant:approval:sess-1"
    ...
    redis_client.expire.assert_awaited_once_with("assistant:approval:sess-1", 1800 + 60)


async def test_is_session_approved_happy_path(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
...
async def test_is_session_approved_wrong_page_path(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
...
async def test_is_session_approved_expired(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    payload = json.dumps({"page_path": "/pipelines", "expires_at": time.time() - 1000})
    redis_client.hget.return_value = payload
    assert await registry.is_session_approved("sess-1", "run_pipeline", "/pipelines") is False


async def test_is_session_approved_missing_value(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    redis_client.hget.return_value = None
    assert await registry.is_session_approved("sess-1", "run_pipeline", "/pipelines") is False


async def test_is_session_approved_invalid_json(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    redis_client.hget.return_value = "{not json"
    assert await registry.is_session_approved("sess-1", "run_pipeline", "/pipelines") is False


async def test_is_session_approved_non_object_json(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
...
async def test_clear_session_approvals_deletes_key(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    await registry.clear_session_approvals("sess-1")
    redis_client.delete.assert_awaited_once_with("assistant:approval:sess-1")


async def test_clear_session_deletes_all_keys(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    await registry.clear_session("sess-1")
    redis_client.delete.assert_awaited_once_with("assistant:ui_results:sess-1", "assistant:approval:sess-1")
...
async def test_publish_permission_response(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    decision = {"decision": "allow"}
    await registry.publish_permission_response("req-1", decision)
    redis_client.publish.assert_awaited_once_with("assistant:channel:permission:req-1", json.dumps(decision))
...
async def test_subscribe_permission_response_receives_message(
    ...
    decision = {"decision": "allow"}
    redis_client.pubsub.return_value = _pubsub("assistant:channel:permission:req-1", decision)
    result = await registry.subscribe_permission_response("req-1", timeout=1.0)
    assert result == decision
    redis_client.pubsub.return_value.subscribe.assert_awaited_once_with("assistant:channel:permission:req-1")
...
async def test_subscribe_permission_response_timeout_returns_none(
    ...
    redis_client.pubsub.return_value = _pubsub("assistant:channel:permission:req-1", None)
...
async def test_publish_ui_results(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    await registry.publish_ui_results("sess-1")
    redis_client.publish.assert_awaited_once_with("assistant:channel:ui_results:sess-1", "ready")


async def test_subscribe_ui_results_receives_message(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    redis_client.pubsub.return_value = _pubsub("assistant:channel:ui_results:sess-1", {"x": 1})
...
async def test_subscribe_ui_results_timeout_returns_false(
    ...
    redis_client.pubsub.return_value = _pubsub("assistant:channel:ui_results:sess-1", None)
    assert await registry.subscribe_ui_results("sess-1", timeout=0.01) is False


async def test_publish_resume(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    await registry.publish_resume("sess-1")
    redis_client.publish.assert_awaited_once_with("assistant:channel:resume:sess-1", "resume")


async def test_subscribe_resume_receives_message(registry: AssistantRedisRegistry, redis_client: MagicMock) -> None:
    redis_client.pubsub.return_value = _pubsub("assistant:channel:resume:sess-1", {"x": 1})
...
async def test_subscribe_resume_timeout_returns_false(
    ...
    redis_client.pubsub.return_value = _pubsub("assistant:channel:resume:sess-1", None)
...
async def test_subscribe_permission_response_async_timeout_returns_none(
    ...
    redis_client.pubsub.return_value = _blocking_pubsub("assistant:channel:permission:req-1")
    result = await registry.subscribe_permission_response("req-1", timeout=0.01)
    assert result is None
    redis_client.pubsub.return_value.unsubscribe.assert_awaited_once()
    redis_client.pubsub.return_value.aclose.assert_awaited_once()
...
async def test_subscribe_ui_results_async_timeout_returns_false(
    ...
    redis_client.pubsub.return_value = _blocking_pubsub("assistant:channel:ui_results:sess-1")
    assert await registry.subscribe_ui_results("sess-1", timeout=0.01) is False
    redis_client.pubsub.return_value.unsubscribe.assert_awaited_once()
    redis_client.pubsub.return_value.aclose.assert_awaited_once()
...
async def test_subscribe_resume_async_timeout_returns_false(
    ...
    redis_client.pubsub.return_value = _blocking_pubsub("assistant:channel:resume:sess-1")
    assert await registry.subscribe_resume("sess-1", timeout=0.01) is False
    redis_client.pubsub.return_value.unsubscribe.assert_awaited_once()
    redis_client.pubsub.return_value.aclose.assert_awaited_once()
