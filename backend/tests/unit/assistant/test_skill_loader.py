...
from modulo.core.assistant.skill_loader import SkillEntry, SkillLoader
...
def _mock_skill(
    ...
    **kwargs: Any,
    ...
    skill = MagicMock(spec=AssistantSkill)
    skill.id = uuid.uuid4()
    skill.name = name
...
class _CtxSourceStub:
    ...
    async def get_effective_config(self, org_id: uuid.UUID, user_id: uuid.UUID) -> AssistantConfig:
...
class _ConfigServiceStub:
    """Stub that returns a canned AssistantConfig."""

    def __init__(self, **attrs: Any) -> None:
        self._config = AssistantConfig(**attrs)

    async def get_config(self, org_id: uuid.UUID) -> AssistantConfig:
        return self._config
...
class TestSkillLoaderBuildSystemPrompt:
    ...
    async def _run(
        ...
        with (
            ...
            patch("modulo.core.assistant.skill_loader.AssistantContextSourceService", return_value=ctx_stub),
        ):
...
class TestSkillLoaderSectionBuilders:
    ...
    def test_build_config_section_with_override(self) -> None:
        ...
        config = AssistantConfig(system_prompt="Default prompt.")
    ...
    def test_build_config_section_returns_none_when_no_prompt(self) -> None:
        loader = SkillLoader.__new__(SkillLoader)
        config = AssistantConfig(system_prompt="")
...
class TestSkillLoaderToEntry:
    ...
    def test_converts_orm_to_entry_with_frontmatter(self) -> None:
        loader = SkillLoader.__new__(SkillLoader)
        mock_skill = MagicMock(spec=AssistantSkill)
