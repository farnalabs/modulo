"""Unit tests for Assistant MCP context tools.
...
class TestSensitiveKeyDetection:
    ...
    def test_non_sensitive_keys(self) -> None:
        ...
        assert not _is_sensitive_key("assistant_config:org-1")
...
class TestGetOrgConfig:
    ...
    async def test_assistant_section_filters_and_keeps_safe_keys(self) -> None:
        configs = [
            self._config(f"assistant_config:{ORG_ID}", {"system_prompt": "Helpful."}),
            self._config("feature_flags", {"assistant_enabled": True}),
        ]
        with (
            patch("modulo.api.mcp_server.validate_current_auth", return_value=True),
            patch("modulo.api.mcp_server._session") as mock_session,
    ...
    async def test_sensitive_keys_redacted(self) -> None:
        ...
        assert "assistant_config:org-1" in result["results"]
    ...
    async def test_invalid_section_returns_error(self) -> None:
        ...
        assert "assistant" in result["detail"]
...
class TestGetAvailableFeatures:
    ...
    async def test_returns_feature_table_and_count(self) -> None:
        flags = [
            self._flag("assistant_chat", "core", True),
        ...
        assert "assistant_chat" in result["results"]
        assert "custom_skills" in result["results"]
        assert "| assistant_chat | core | yes |" in result["results"]
