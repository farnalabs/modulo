"""Unit tests for AssistantContextSourceService — merging built-in defaults, org defaults, and user overrides."""
...
from modulo.core.assistant.context_source_service import AssistantContextSourceService
...
def _mock_source_row(
    ...
    row = MagicMock(spec=AssistantContextSource)
    row.id = uuid.uuid4()
...
class TestAssistantContextSourceServiceGetEffectiveConfig:
    """Tests for merging built-in → org defaults → user overrides."""

    @pytest.fixture
    def service(self, mock_session: AsyncMock) -> AssistantContextSourceService:
        return AssistantContextSourceService(mock_session)

    async def test_uses_builtins_when_no_org_or_user_overrides(
        self,
        service: AssistantContextSourceService,
    ...
    async def test_get_effective_config_returns_assistant_config(
        self,
        service: AssistantContextSourceService,
        mock_session: AsyncMock,
        org_id: uuid.UUID,
...
class TestAssistantContextSourceServiceSetUserOverride:
    ...
    async def test_set_user_override_calls_execute(
        ...
        assert f"assistant_context_sources.organisation_id = '{org_id.hex}'" in compiled
        assert "assistant_context_sources.source_key = 'product_docs'" in compiled
        assert f"assistant_context_sources.account_id = '{user_id.hex}'" in compiled
    ...
    async def test_set_user_override_inserts_new_row_when_missing(
        ...
        assert isinstance(added, AssistantContextSource)
...
class TestAssistantContextSourceServiceSetOrgDefault:
    ...
    async def test_set_org_default_calls_execute(
        ...
        assert f"assistant_context_sources.organisation_id = '{org_id.hex}'" in compiled
        assert "assistant_context_sources.source_key = 'product_docs'" in compiled
        assert "assistant_context_sources.account_id IS NULL" in compiled
...
class TestAssistantContextSourceServiceQueries:
    ...
    def test_build_effective_items_marks_overrides(self) -> None:
        service = AssistantContextSourceService.__new__(AssistantContextSourceService)
...
class TestAssistantContextSourceServiceResetUserOverrides:
    ...
    async def test_reset_user_overrides_calls_bulk_delete(
        ...
        stmt = mock_session.execute.call_args[0][0]
        compiled = str(stmt.compile(compile_kwargs={"literal_binds": True}))
        assert "DELETE FROM assistant_context_sources" in compiled
        assert f"assistant_context_sources.organisation_id = '{org_id.hex}'" in compiled
        assert f"assistant_context_sources.account_id = '{user_id.hex}'" in compiled
        mock_session.flush.assert_awaited_once()
