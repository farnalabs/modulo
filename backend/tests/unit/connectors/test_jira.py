...
async def test_query_issue(connector):
    ...
    result = await connector.query(
        ConnectorQuery(resource="issue", filters={"issue_key": "PROJ-123"})
    )
...
async def test_query_search(connector):
    ...
    result = await connector.query(
        ConnectorQuery(resource="search", filters={"jql": "project = PROJ"})
    )
...
async def test_write_update_issue(connector):
    respx.put(f"{_BASE}/issue/PROJ-123").mock(
...
async def test_health_check_token_auth(connector_token):
    respx.get(f"{_BASE}/myself").mock(
        return_value=httpx.Response(200, json={"displayName": "Bob"})
    )
