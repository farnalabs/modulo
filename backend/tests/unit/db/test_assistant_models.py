"""Schema-level tests for Assistant data models: ChatSession, ChatMessage, AssistantSkill."""

from sqlalchemy import CheckConstraint

from modulo.db.models import AssistantSkill, Base, ChatMessage, ChatSession, OrgScoped
...
class TestAssistantSkill:
    def test_table_exists(self) -> None:
        assert "assistant_skills" in Base.metadata.tables

    def test_columns(self) -> None:
        cols = Base.metadata.tables["assistant_skills"].c
        assert "id" in cols
        assert "organisation_id" in cols
        assert "account_id" in cols
        assert "name" in cols
    ...
    def test_is_not_org_scoped(self) -> None:
        assert not issubclass(AssistantSkill, OrgScoped)

    def test_organisation_id_nullable(self) -> None:
        col = Base.metadata.tables["assistant_skills"].c["organisation_id"]
        assert col.nullable

    def test_account_id_nullable(self) -> None:
        col = Base.metadata.tables["assistant_skills"].c["account_id"]
        assert col.nullable

    def test_active_defaults_true(self) -> None:
        col = Base.metadata.tables["assistant_skills"].c["active"]
        assert col.server_default is not None
        assert "true" in str(col.server_default.arg).lower()

    def test_name_not_null(self) -> None:
        col = Base.metadata.tables["assistant_skills"].c["name"]
        assert not col.nullable

    def test_body_not_null(self) -> None:
        col = Base.metadata.tables["assistant_skills"].c["body"]
        assert not col.nullable

    def test_exactly_one_owner_check_constraint(self) -> None:
        table = Base.metadata.tables["assistant_skills"]
        check = next(
            (c for c in table.constraints if isinstance(c, CheckConstraint) and c.name == "ck_assistant_skills_owner"),
            None,
        )
        assert check is not None, "Missing ck_assistant_skills_owner CHECK constraint"
