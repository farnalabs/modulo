...
* ``0095_ongoing_trigger_flag`` registers the flag inactive (default OFF);
  ...
  ``0097_ongoing_trigger_enabled_by_default`` flips it active (default ON) and
...
class TestMigration0095OngoingFlag:
    ...
    def test_single_head_chain(self, migration_0095: ModuleType) -> None:
        ...
        assert heads == ["0097_ongoing_trigger_enabled_by_default"], f"expected a single head, got {heads}"
...
class TestMigration0097OngoingFlagEnabled:
    ...
    def test_single_head_chain(self, migration_0097: ModuleType) -> None:
        ...
        heads = script.get_heads()
        assert heads == ["0097_ongoing_trigger_enabled_by_default"], f"expected a single head, got {heads}"
        assert migration_0097.revision in heads
