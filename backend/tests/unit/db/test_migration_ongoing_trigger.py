...
* ``0089_ongoing_trigger_type`` is loaded and inspected directly: revision
...
* ``0090_ongoing_trigger_flag`` is confirmed as the single head of the chain.
"""
...
_MIGRATION_0089 = "0089_ongoing_trigger_type"
_MIGRATION_0090 = "0090_ongoing_trigger_flag"
...
def migration_0089() -> ModuleType:
    return _load_migration(_MIGRATION_0089)


@pytest.fixture(scope="module")
def migration_0090() -> ModuleType:
    return _load_migration(_MIGRATION_0090)
...
class TestMigration0089OngoingTriggerType:
    def test_revision_chain(self, migration_0089: ModuleType) -> None:
        assert migration_0089.revision == "0089_ongoing_trigger_type"
        assert migration_0089.down_revision == "0088_drop_stages"
        assert migration_0089.branch_labels is None

    def test_upgrade_uses_not_valid_then_validate(self, migration_0089: ModuleType) -> None:
        ...
        source = _source(migration_0089)
        assert "NOT VALID" in source
        assert "VALIDATE CONSTRAINT" in source
        assert "ADD CONSTRAINT" in source

    def test_upgrade_widens_both_checks_with_ongoing(self, migration_0089: ModuleType) -> None:
        source = _source(migration_0089)
        assert _TRIGGERS_VALUES_POST in source
        assert _RUNS_VALUES_POST in source

    def test_partial_ongoing_checks_present(self, migration_0089: ModuleType) -> None:
        source = _source(migration_0089)
        assert _SPEND_PARTIAL in source
        assert _TARGET_PARTIAL in source

    def test_upgrade_creates_trigger_id_indexes(self, migration_0089: ModuleType) -> None:
        source = _source(migration_0089)
    ...
    def test_downgrade_restores_pre_strings_and_drops_indexes(self, migration_0089: ModuleType) -> None:
        source = _source(migration_0089)
...
class TestMigration0090OngoingFlag:
    def test_revision_chain(self, migration_0090: ModuleType) -> None:
        assert migration_0090.revision == "0090_ongoing_trigger_flag"
        assert migration_0090.down_revision == "0089_ongoing_trigger_type"

    def test_single_head_chain(self, migration_0090: ModuleType) -> None:
        script = _script()
        heads = script.get_heads()
        assert heads == ["0090_ongoing_trigger_flag"], f"expected a single head, got {heads}"
        assert migration_0090.revision in heads

    def test_flag_upserts_ongoing_trigger_inactive(self, migration_0090: ModuleType) -> None:
        assert "ongoing_trigger" in migration_0090._FLAGS
        assert migration_0090._FLAGS["ongoing_trigger"][0] == "community"
