...
* ``0087_ongoing_trigger_type`` is loaded and inspected directly: revision
...
* ``0088_ongoing_trigger_flag`` is confirmed as the single head of the chain.
"""
...
_MIGRATION_0087 = "0087_ongoing_trigger_type"
_MIGRATION_0088 = "0088_ongoing_trigger_flag"
...
@pytest.fixture(scope="module")
def migration_0088() -> ModuleType:
    return _load_migration(_MIGRATION_0088)
...
class TestMigration0087OngoingTriggerType:
    def test_revision_chain(self, migration_0087: ModuleType) -> None:
        assert migration_0087.revision == "0087_ongoing_trigger_type"
        assert migration_0087.down_revision == "0086_seeded_alert_rules"
        assert migration_0087.branch_labels is None

    def test_upgrade_uses_not_valid_then_validate(self, migration_0087: ModuleType) -> None:
        ...
        source = _source(migration_0087)
        assert "NOT VALID" in source
        assert "VALIDATE CONSTRAINT" in source
        assert "ADD CONSTRAINT" in source

    def test_upgrade_widens_both_checks_with_ongoing(self, migration_0087: ModuleType) -> None:
        source = _source(migration_0087)
        assert _TRIGGERS_VALUES_POST in source
        assert _RUNS_VALUES_POST in source

    def test_partial_ongoing_checks_present(self, migration_0087: ModuleType) -> None:
        source = _source(migration_0087)
        assert _SPEND_PARTIAL in source
        assert _TARGET_PARTIAL in source

    def test_upgrade_creates_trigger_id_indexes(self, migration_0087: ModuleType) -> None:
        source = _source(migration_0087)
    ...
    def test_downgrade_restores_pre_strings_and_drops_indexes(self, migration_0087: ModuleType) -> None:
        source = _source(migration_0087)
...
class TestMigration0088OngoingFlag:
    def test_revision_chain(self, migration_0088: ModuleType) -> None:
        assert migration_0088.revision == "0088_ongoing_trigger_flag"
        assert migration_0088.down_revision == "0087_ongoing_trigger_type"

    def test_single_head_chain(self, migration_0088: ModuleType) -> None:
        script = _script()
        heads = script.get_heads()
        assert heads == ["0088_ongoing_trigger_flag"], f"expected a single head, got {heads}"
        assert migration_0088.revision in heads

    def test_flag_upserts_ongoing_trigger_inactive(self, migration_0088: ModuleType) -> None:
        assert "ongoing_trigger" in migration_0088._FLAGS
        assert migration_0088._FLAGS["ongoing_trigger"][0] == "community"
