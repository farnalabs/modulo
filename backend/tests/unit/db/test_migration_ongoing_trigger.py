...
* ``0099_run_raw_output_markers`` (FAR-188) is confirmed as the single
  head of the chain, sitting on top of ``0098_slack_app_mention_trigger_type``.
"""
...
_MIGRATION_0100 = "0100_run_classification"
...
class TestMigration0095OngoingFlag:
    ...
    def test_single_head_chain(self, migration_0095: ModuleType, migration_0096: ModuleType) -> None:
        ...
        # which is itself superseded by 0100_run_classification.
        heads = script.get_heads()
        assert migration_0095.revision not in heads
        assert migration_0096.revision not in heads
        assert heads == ["0100_run_classification"], f"expected a single head, got {heads}"
...
class TestMigration0097OngoingFlagEnabled:
    ...
    def test_single_head_chain(self, migration_0097: ModuleType) -> None:
        script = _script()
        heads = script.get_heads()
        # 0097 is superseded as the head by 0100_run_classification.
...
class TestMigration0098SlackAppMention:
    ...
    def test_single_head_chain(self, migration_0098: ModuleType, migration_0099: ModuleType) -> None:
        script = _script()
        heads = script.get_heads()
        # 0098 is superseded as the head by 0100_run_classification.
...
class TestMigration0099RunRawOutputMarkers:
    ...
    def test_single_head_chain(self, migration_0099: ModuleType) -> None:
        script = _script()
        heads = script.get_heads()
        # 0099 is superseded as the head by 0100_run_classification.
        assert heads == ["0100_run_classification"], f"expected a single head, got {heads}"
