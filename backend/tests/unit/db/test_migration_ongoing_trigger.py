"""Migration tests for the ``ongoing`` trigger type (FAR-158, 0086/0087).

No live Postgres here (integration territory) — instead:

* ``0086_ongoing_trigger_type`` is loaded and inspected directly: revision
...
* ``0087_ongoing_trigger_flag`` is confirmed as the single head of the chain.
"""
...
_MIGRATION_0086 = "0086_ongoing_trigger_type"
_MIGRATION_0087 = "0087_ongoing_trigger_flag"

_VERSIONS_DIR = Path(__file__).resolve().parents[3] / "src" / "modulo" / "db" / "migrations" / "versions"

# The pre-feature / post-feature vocabulary strings (hardcoded in 0086).
...
def migration_0086() -> ModuleType:
    return _load_migration(_MIGRATION_0086)


@pytest.fixture(scope="module")
def migration_0087() -> ModuleType:
    return _load_migration(_MIGRATION_0087)
...
class TestMigration0086OngoingTriggerType:
    def test_revision_chain(self, migration_0086: ModuleType) -> None:
        assert migration_0086.revision == "0086_ongoing_trigger_type"
        assert migration_0086.down_revision == "0085_journey_facts"
        assert migration_0086.branch_labels is None

    def test_upgrade_uses_not_valid_then_validate(self, migration_0086: ModuleType) -> None:
        ...
        source = _source(migration_0086)
        assert "NOT VALID" in source
        assert "VALIDATE CONSTRAINT" in source
        assert "ADD CONSTRAINT" in source

    def test_upgrade_widens_both_checks_with_ongoing(self, migration_0086: ModuleType) -> None:
        source = _source(migration_0086)
        assert _TRIGGERS_VALUES_POST in source
        assert _RUNS_VALUES_POST in source

    def test_partial_ongoing_checks_present(self, migration_0086: ModuleType) -> None:
        source = _source(migration_0086)
        assert _SPEND_PARTIAL in source
        assert _TARGET_PARTIAL in source

    def test_upgrade_creates_trigger_id_indexes(self, migration_0086: ModuleType) -> None:
        source = _source(migration_0086)
    ...
    def test_downgrade_restores_pre_strings_and_drops_indexes(self, migration_0086: ModuleType) -> None:
        source = _source(migration_0086)
...
class TestMigration0087OngoingFlag:
    def test_revision_chain(self, migration_0087: ModuleType) -> None:
        assert migration_0087.revision == "0087_ongoing_trigger_flag"
        assert migration_0087.down_revision == "0086_ongoing_trigger_type"

    def test_single_head_chain(self, migration_0087: ModuleType) -> None:
        script = _script()
        heads = script.get_heads()
        assert heads == ["0087_ongoing_trigger_flag"], f"expected a single head, got {heads}"
        assert migration_0087.revision in heads

    def test_flag_upserts_ongoing_trigger_inactive(self, migration_0087: ModuleType) -> None:
        assert "ongoing_trigger" in migration_0087._FLAGS
        assert migration_0087._FLAGS["ongoing_trigger"][0] == "community"
