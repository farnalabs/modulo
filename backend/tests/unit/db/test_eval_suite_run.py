...
def test_single_migration_head() -> None:
    ...
    # 0183_triggers_add_polling_ongoing_agent_signal_indexes (polling / ongoing /
    # agent_signal tick-scan partial-index sweep, on main) chains off 0182.
    ...
    # 0184_trigger_events_indexes_and_type_check (trigger_events composite indexes +
    # trigger_type CHECK, on main) chains off 0183.
    ...
    # 0185_agent_runner_bindings (this PR, FAR-592 / D6) chains off 0184 and is the single head.
    chaining_off_0184 = [p for p in revisions if parents[p] == "0184_trigger_events_indexes_and_type_check"]
    assert [_basename(p) for p in chaining_off_0184] == ["0185_agent_runner_bindings.py"]
    # Nothing chains off 0185_agent_runner_bindings -> it is the single head.
    chaining_off_0185 = [p for p in revisions if parents[p] == "0185_agent_runner_bindings"]
    assert chaining_off_0185 == []
