...
def test_single_migration_head() -> None:
    ...
    # 0151_accounts_must_change_password (this PR, FAR-460) chains off 0150 and is the single head.
    chaining_off_0150 = [p for p in revisions if parents[p] == "0150_add_router_no_match_status"]
    assert [_basename(p) for p in chaining_off_0150] == ["0151_accounts_must_change_password.py"]
    # Nothing chains off 0151 -> it is the single head.
    chaining_off_0151 = [p for p in revisions if parents[p] == "0151_accounts_must_change_password"]
