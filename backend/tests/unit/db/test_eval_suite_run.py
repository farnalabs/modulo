...
def test_single_migration_head() -> None:
    ...
    #    chains off 0154, and 0156_run_idempotency_key (FAR-438) chains off 0155 and is the head.
    ...
    # (FAR-402 P5) chains off 0154; 0156_run_idempotency_key (FAR-438) chains off 0155 and is the head.
    ...
    # 0155_pipeline_retry_compensation (FAR-402 P5) chains off 0154.
    ...
    # 0156_run_idempotency_key (FAR-438) chains off 0155_pipeline_retry_compensation.
    chaining_off_0155 = [p for p in revisions if parents[p] == "0155_pipeline_retry_compensation"]
    assert [_basename(p) for p in chaining_off_0155] == ["0156_run_idempotency_key.py"]
    # Nothing chains off 0156 -> it is the single head.
    chaining_off_0156 = [p for p in revisions if parents[p] == "0156_run_idempotency_key"]
