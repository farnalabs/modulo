...
def test_single_migration_head() -> None:
    ...
    # 0183_triggers_add_polling_ongoing_agent_signal_indexes (polling /
    ...
    # 0184_trigger_events_indexes_and_type_check (trigger_events composite
    # indexes + trigger_type CHECK) chains off 0183.
    ...
    # 0186_bundled_runner_seed_backfill (FAR-590 D4) chains off 0185_error_events_indexes_jsonb.
    chaining_off_0185 = [p for p in revisions if parents[p] == "0185_error_events_indexes_jsonb"]
    assert [_basename(p) for p in chaining_off_0185] == ["0186_bundled_runner_seed_backfill.py"]
    # Nothing chains off 0186_bundled_runner_seed_backfill -> it is the single head.
    chaining_off_0186 = [p for p in revisions if parents[p] == "0186_bundled_runner_seed_backfill"]
