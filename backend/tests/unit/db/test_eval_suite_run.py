...
def test_single_migration_head() -> None:
    ...
    # 0186_agent_runner_bindings (this PR, FAR-592 / D6) chains off 0185 and is the single head.
    chaining_off_0185 = [p for p in revisions if parents[p] == "0185_error_events_indexes_jsonb"]
    assert [_basename(p) for p in chaining_off_0185] == ["0186_agent_runner_bindings.py"]
    # Nothing chains off 0186_agent_runner_bindings -> it is the single head.
    chaining_off_0186 = [p for p in revisions if parents[p] == "0186_agent_runner_bindings"]
