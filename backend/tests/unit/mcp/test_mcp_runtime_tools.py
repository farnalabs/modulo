...
class TestCopyLibraryPrimitive(_AuthContext):
    ...
    @patch("modulo.api.mcp_server.library_copy_to_adapt")
    @patch("modulo.api.mcp_server._session")
    async def test_hosted_fallback_installs_from_community_catalog(
        self,
        mock_session: AsyncMock,
        mock_library_copy: AsyncMock,
        mock_get_entry: AsyncMock,
        ...
        installed = MagicMock()
        installed.id = uuid.uuid4()
        installed.name = "hosted-name"
        installed.slug = "hosted-slug"
        mock_install.return_value = installed
        ...
        assert result["status"] == "installed"
        assert result["primitive_id"] == str(installed.id)
        assert result["name"] == "hosted-name"
        assert result["slug"] == "hosted-slug"
        mock_get_entry.assert_awaited_once_with(mock_sesh, primitive_id)
        mock_install.assert_awaited_once_with(mock_sesh, _PLACEHOLDER_ORG_ID, primitive_id)
