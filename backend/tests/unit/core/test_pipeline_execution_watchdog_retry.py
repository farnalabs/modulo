"""Unit tests for the shared watchdog-kill retry mechanism (FAR-690).

FAR-690: the FAR-369 absolute node-deadline watchdog used to terminal-fail the
run DIRECTLY, bypassing the run-level retry decision. It now consults the
pipeline's ``retry_policy`` through the ONE shared mechanism
(``pipeline_engine.watchdog_retry``) and re-dispatches via the SAME fenced
...
from modulo.core.pipeline_execution import _fail_overdue_node
