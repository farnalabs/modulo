...
def _outputs_by_run(runs: list[Any]) -> dict[Any, Any]:
    """The per-run outputs mapping (FAR-583: the pure evaluator takes the
    reassembled outputs as an explicit parameter, never the legacy column)."""
    return {r.id: r.outputs_json for r in runs}
...
class TestEvaluateCoverageGap:
    ...
    def test_high_divergence_low_differentiation_gap(self) -> None:
        ...
        summary = evaluate_coverage_gap(runs, evals, outputs_by_run=_outputs_by_run(runs), eval_names=names, min_runs=3)
    ...
    def test_pass_flag_used_when_score_absent(self) -> None:
        ...
        summary = evaluate_coverage_gap(
            runs, evals, outputs_by_run=_outputs_by_run(runs), eval_names=_eval_defs(eval_id), min_runs=3
        )
        gap = summary.evals[0]
    ...
    def test_prove_the_fix_gap_requires_both_conditions(self) -> None:
        ...
        no_gap = evaluate_coverage_gap(
            same_runs,
            same_evals,
            outputs_by_run=_outputs_by_run(same_runs),
            eval_names=_eval_defs(eval_id),
            min_runs=3,
        )
        ...
        gap = evaluate_coverage_gap(
            diff_runs,
            diff_evals,
            outputs_by_run=_outputs_by_run(diff_runs),
            eval_names=_eval_defs(eval_id),
            min_runs=3,
        )
    ...
    def test_invalid_min_runs_rejected(self) -> None:
        with pytest.raises(ValueError, match="min_runs must be >= 1"):
            evaluate_coverage_gap([], [], outputs_by_run={}, min_runs=0)
