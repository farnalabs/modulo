...
// org_role: 'admin' so the admin-role sidebar filter shows /admin/assistant.
...
test.describe('Assistant-only mode at /assistant', { tag: '@smoke' }, () => {
  test('sidebar shows the dev-mode /admin/assistant control and hides /assistant; /assistant renders behind auth', { tag: '@smoke' }, async ({ page, env }) => {
    test.setTimeout(120_000)
    test.skip(!isDevModeTarget(env), 'Assistant is dev-mode-gated (private_preview); only runs on a dev-mode target')
    ...
    // so the admin-role sidebar filter keeps /admin/assistant visible.
    ...
    // Positive control: the private_preview /admin/assistant item IS in the sidebar
    ...
    await expect(page.locator('a[href="/admin/assistant"]').first()).toBeVisible()

    // Negative control: /assistant is a bare route, never listed in the sidebar.
    await expect(page.locator('a[href="/assistant"]')).toHaveCount(0)

    // Direct navigation to /assistant renders the view behind the auth guard
    // (dev mode is on, so the private_preview guard passes).
    await page.goto('/assistant')
    await expect(page.locator('[data-testid="assistant-only-view"]')).toBeVisible()
    await expect(page.locator('[data-testid="assistant-only-banner"]')).toBeVisible()
  })
})
