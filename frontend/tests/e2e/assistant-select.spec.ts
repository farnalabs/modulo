...
// Proves the PrimeVue Select contract that the Assistant `select()` executor relies
...
test.describe('Assistant select() contract on migrated Select', { tag: "@regression" }, () => {
  test('filter-bar-status select opens and selects an option via data-value', { tag: "@regression" }, async ({ page, env }) => {
    ...
    // Assistant select() opens the combobox then clicks [data-value="..."].
    await trigger.click()
  ...
  test('migrated Select option renders with a data-value for Assistant to target', { tag: "@regression" }, async ({ page, env }) => {
    test.skip(env.name !== 'local', 'Uses setupLocalMockApi — only runs locally')
    await setupLocalMockApi(page)
    await loginAsAdmin(page, env)
    ...
    // The #option slot must emit data-value so the Assistant executor can target it.
