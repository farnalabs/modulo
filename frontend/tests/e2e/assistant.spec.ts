...
test.describe('Assistant Admin Configuration', { tag: "@regression" }, () => {
  test('page loads with assistant configuration sections', { tag: "@regression" }, async ({ page, env }) => {
    test.skip(!isDevModeTarget(env), 'Route is dev-mode-gated (private_preview); only runs on a dev-mode target')
    await loginAsAdmin(page, env)
    await page.route('**/api/v1/admin/assistant/config', (route) => {
    ...
    await page.route('**/api/v1/admin/assistant/available-providers', (route) => {
    ...
    await page.route('**/api/v1/admin/assistant/skills', (route) => {
    ...
    await page.route('**/api/v1/admin/assistant/context-sources', (route) => {
    ...
    await expect(page.locator('h1')).toContainText('Assistant Configuration')
  })
})
