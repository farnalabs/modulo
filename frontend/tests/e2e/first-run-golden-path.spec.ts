...
test.describe('First-Run Golden Path', () => {
  ...
  test('golden path: login -> browse pipelines -> trigger run -> inspect output', async ({ page, env }) => {
    ...
    await page.route('**/api/v1/pipelines/**', async (route) => {
      ...
      if (url.includes('/trigger')) {
        ...
        return route.fulfill({
          ...
          body: JSON.stringify({ run_id: RUN_ID, status: 'queued' }),
        })
      }
      ...
      return route.fulfill({
        ...
        body: JSON.stringify({
          id: PIPELINE_ID,
          name: 'Demo Pipeline',
          description: 'A demo pipeline to showcase the platform',
          status: 'idle',
    ...
    await page.route('**/api/v1/runs/*', async (route) => {
      ...
      const idx = Math.min(pollCount - 1, statuses.length - 1)
      return route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    // Look for a trigger button or menu action
    const triggerButton = page.getByTestId('pipeline-trigger-run')
    if (await triggerButton.isVisible()) {
      await triggerButton.click()
      await page.waitForLoadState('networkidle')
    } else {
      // Fallback: navigate directly to run detail page
      console.warn('[golden-path] trigger button not found, navigating directly to run')
    }
