    ...
    <template v-else-if="run">
      ...
      <div v-if="runTimestamps" class="flex flex-wrap gap-x-6 gap-y-1 text-xs text-muted-foreground">
        ...
        <div data-testid="run-detail-heartbeat" id="run-detail-heartbeat-anchor">
          <span class="font-medium text-foreground">{{ $t('views.RunDetailView.last_heartbeat') }}</span> <span :class="isHeartbeatStale(heartbeatAge) ? 'font-medium text-warning' : ''">{{ formatHeartbeatAge(heartbeatAge) }}<span v-if="isHeartbeatStale(heartbeatAge)"> ({{ $t('views.RunDetailView.stale') }})</span></span>
        </div>
      </div>
