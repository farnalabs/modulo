<template>
  <div data-theme="agent" class="page-wide"><header class="flex items-center justify-between">
      <PageHeader :title="$t('views.UserAssistantSkillsView.my_assistant_skills')" :subtitle="$t('views.UserAssistantSkillsView.manage_your_personal_skills_for_the_assistant_ai_assistant')" />
      <Button class="border border-primary/30" data-testid="assistant-user-skills-add" @click="skillDialogRef?.openCreate()">
        Add Skill
      </Button>
    </header>
    ...
    <div v-else-if="skills.length === 0" class="rounded-lg border bg-card p-8 text-center">
      <p class="text-lg font-medium">{{ $t('views.UserAssistantSkillsView.no_personal_skills_configured') }}</p>
      <p class="mt-1 text-sm text-muted-foreground">
        Create skills to give Assistant custom instructions and behaviours.
      </p>
    </div>
    <template v-else>
      <div class="overflow-hidden rounded-lg border bg-card shadow-sm">
        <table class="w-full text-left text-sm">
          <thead class="bg-muted/50 text-xs font-medium uppercase text-muted-foreground">
            <tr>
              <th class="px-4 py-3">{{ $t('views.UserAssistantSkillsView.name') }}</th>
              <th class="px-4 py-3">{{ $t('views.UserAssistantSkillsView.description') }}</th>
              <th class="px-4 py-3">{{ $t('views.UserAssistantSkillsView.triggers') }}</th>
              <th class="px-4 py-3">{{ $t('common.active') }}</th>
              <th class="px-4 py-3 text-right">{{ $t('views.UserAssistantSkillsView.actions') }}</th>
            </tr>
          </thead>
    ...
    <AssistantSkillDialog
      ref="skillDialogRef"
      create-description="Create a new personal skill for Assistant."
      edit-description="Update your personal skill."
      create-endpoint="/api/v1/me/assistant/skills"
      update-endpoint="/api/v1/me/assistant/skills/{skill_id}"
      delete-endpoint="/api/v1/me/assistant/skills/{skill_id}"
      @saved="loadSkills"
    /></div>
</template>
...
const { loading, error: loadError, data: skillsResp, load: loadSkills } = useDataFetch<SkillItem[]>(
  () => api.GET('/api/v1/me/assistant/skills'),
  { initialValue: [] },
)
...
watch(skillsResp, () => {
  assistantStore.signalSkillsChanged()
}, { immediate: true })
...
async function toggleSkillActive(skill: SkillItem) {
  ...
  try {
    const { data, error: err } = await api.PUT('/api/v1/me/assistant/skills/{skill_id}', {
