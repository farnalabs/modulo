  ...
  <div class="min-h-screen">
          ...
          <div class="relative" ref="typeFilterRef">
            ...
            >
              ...
              <span
                ...
                class="flex h-5 w-5 items-center justify-center rounded-full bg-primary text-[10px] font-medium text-primary-foreground"
    ...
    <main class="page-wide">
      ...
      <p
        ...
        data-testid="library-community-disclaimer"
      >
        {{ $t("views.LibraryView.community_disclaimer") }}
      </p>

      <div
        v-if="loading || (section === 'hosted' && hostedLoading)"
      ...
      <details
        v-if="section === 'native' && previewPrimitives.length > 0"
        class="rounded-lg border bg-card"
        data-testid="library-preview-section"
      >
        <summary
          class="cursor-pointer px-4 py-3 text-sm font-medium text-muted-foreground hover:text-foreground"
        >
          {{
            $t(
              "views.LibraryView.preview_integrations_count",
              { count: previewPrimitives.length },
              previewPrimitives.length,
            )
          }}
        </summary>
        <LibraryPrimitiveGrid
      ...
      <div
        v-if="section === 'hosted' && hostedCommunityItems.length > 0"
...
const typeOptions = [
  {
    value: "pipeline_template",
    labelKey: "views.LibraryView.type_pipeline_templates",
  },
...
const {
  ...
  async () => {
    ...
    if (selectedTypes.value.length === 1)
      params.set("primitive_type", selectedTypes.value[0]);
    if (selectedTypes.value.length > 1)
      params.set("primitive_types", selectedTypes.value.join(","));
    ...
    if (d) {
      primitives.value =
        section.value === "native"
          ? d.items.filter((p) => p.source !== "community")
          : d.items;
      total.value = d.total;
    }
  },
...
const nativePrimitives = computed(() =>
  applyTypeFilter(
    primitives.value.filter(
      (p) =>
        (p.tier ?? "native") !== "preview" && (p.tier ?? "native") !== "in_dev",
    ),
  ),
);
const previewPrimitives = computed(() =>
  applyTypeFilter(primitives.value.filter((p) => p.tier === "preview")),
);
const communityPrimitives = computed(() =>
  applyTypeFilter(primitives.value.filter((p) => p.source === "community")),
);
...
async function toggleAutoUpdate(prim: LibraryPrimitive) {
  ...
  try {
    ...
    if (idx !== -1 && data)
      primitives.value[idx] = data as unknown as LibraryPrimitive;
  } catch (e) {
    error.value = formatApiError(e);
  } finally {
