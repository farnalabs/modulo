...
import type { ChatSession, ChatMessage, PageContext, ToolResult } from '@/types/assistant'
...
export const useAssistantStore = defineStore('assistant', () => {
  ...
  const activeSessionId = useStorage<string | null>('assistant-active-session', null)
  const messages = ref<ChatMessage[]>([])
  const panelState = useStorage<'closed' | 'floating' | 'docked' | 'maximised'>('assistant-panel-state', 'closed')
  const panelPosition = useStorage('assistant-panel-position', { x: Math.max(8, window.innerWidth - 460), y: 80 })
  const panelSize = useStorage('assistant-panel-size', { width: Math.min(440, window.innerWidth - 16), height: Math.min(600, window.innerHeight - 120) })
  ...
  async function fetchSessions() {
    ...
    error.value = null
    try {
      const resp = await api.GET('/api/v1/assistant/sessions')
      if (resp.error) {
  ...
  async function loadSession(id: string) {
    ...
    messages.value = []
    try {
      const resp = await api.GET('/api/v1/assistant/sessions/{session_id}/messages', {
  ...
  async function renameSession(id: string, name: string): Promise<boolean> {
    error.value = null
    try {
      const resp = await api.PATCH('/api/v1/assistant/sessions/{session_id}', {
        params: { path: { session_id: id } },
        body: { name },
      })
      if (resp.error) {
  ...
  async function deleteSession(id: string) {
    error.value = null
    try {
      const resp = await api.DELETE('/api/v1/assistant/sessions/{session_id}', {
        params: { path: { session_id: id } },
      })
      if (resp.error) {
        error.value = extractErrorMessage(resp.error)
  ...
  async function approvePermission(requestId: string, action: 'approve' | 'reject' | 'approve_for_session') {
    ...
    error.value = null
    try {
      const resp = await api.POST('/api/v1/assistant/sessions/{session_id}/permission-response', {
  ...
  async function resetSessionPermissions() {
    ...
    error.value = null
    try {
      const resp = await api.POST('/api/v1/assistant/sessions/{session_id}/reset-permissions', {
        params: { path: { session_id: activeSessionId.value } },
      })
      if (resp.error) {
        error.value = extractErrorMessage(resp.error)
