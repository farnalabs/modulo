<template>
  ...
  >
    ...
    >
      <div class="flex items-center gap-2 flex-1 min-w-0">
        <template v-if="editingName && store.activeSession">
          <input
            id="assistantpanel-name-input"
            ref="nameInputRef"
            v-model="editNameValue"
            :aria-label="$t('components.assistant.AssistantPanel.session_label')"
            class="assistant-name-input text-sm font-semibold"
        ...
        <template v-else>
          <button type="button"
            ...
            :title="$t('components.assistant.AssistantPanel.click_to_rename')"
          ...
          >
            ...
            <template v-else-if="store.activeSession">
              {{ $t('components.assistant.AssistantPanel.session_label') }} {{ store.activeSession.session_number ? '#' + store.activeSession.session_number : shortId(store.activeSession.id) }}
            </template>
            <template v-else>{{ $t('components.assistant.AssistantPanel.assistant') }}</template>
          </button>
        </template>
        <span v-if="store.isStreaming" class="assistant-pulse-dot" />
      </div>
      <div class="flex items-center gap-1">
        <button
          v-if="store.activeSessionId && store.messages.length > 0"
          type="button"
          class="assistant-titlebar-btn"
          @click="exportTranscript"
          title="Export Transcript"
          :aria-label="$t('components.assistant.export_transcript')"
        >
        ...
        <button
          ...
          :aria-label="$t('components.assistant.reset_permissions')"
        >
          <svg
        ...
        <button type="button"
          ...
          title="Dock"
          :aria-label="$t('components.assistant.dock_panel')"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="14"
        ...
        <button type="button"
          ...
          title="Undock"
          :aria-label="$t('components.assistant.undock_panel')"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <rect x="3" y="3" width="18" height="18" rx="2" />
        ...
        <button type="button"
          v-if="planStore.featureEnabled('assistant_ui_driving')"
          class="assistant-titlebar-btn text-xs font-medium px-1.5"
          @click="cycleSpeed"
        ...
        <button type="button"
          ...
          title="Maximise"
          :aria-label="$t('components.assistant.maximise')"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="14"
        ...
        <button type="button"
          ...
          @click="store.setPanelState('docked')"
          title="Minimise"
          :aria-label="$t('components.assistant.minimise')"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="14"
        ...
        <button type="button"
          ...
          title="Close"
          :aria-label="$t('components.assistant.close_panel')"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="14"
    ...
    >
      ...
      <button type="button"
        ...
        :aria-label="$t('components.assistant.dismiss_error')"
      >
        &times;
      </button>
    </div>
    <div v-if="currentSpeed === 'review' && store.activeSession" class="flex items-center justify-between px-3 py-1.5 text-xs border-b bg-muted/30">
      <span>⏸ {{ $t('components.assistant.AssistantPanel.stops_after_each_navigation') }}</span>
      <button type="button" class="text-xs font-medium underline hover:no-underline" @click="resumeUiCommands">{{ $t('components.assistant.AssistantPanel.resume') }}</button>
    </div>
    <div class="assistant-body">
      <div class="assistant-sidebar" :class="{ open: showSidebar }">
      ...
      <div class="assistant-main">
          <div class="assistant-chat-tabs flex items-center border-b px-2">
            <button type="button"
              class="assistant-tab"
          ...
          <AssistantChat v-show="activeTab === 'chat'" ref="chatRef" />
          <AssistantSkillManager v-if="activeTab === 'skills'" />
          <div
            v-show="activeTab === 'sessions'"
            class="flex-1 overflow-auto p-2"
          >
            <AssistantSessionDrawer @select-session="activeTab = 'chat'" />
          </div>
          <AssistantContextSources v-show="activeTab === 'sources'" />
      </div>
    </div>
  ...
  <button type="button"
    ...
    :aria-label="$t('components.assistant.open_assistant')"
  >
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="20"
...
import AssistantSessionDrawer from "./AssistantSessionDrawer.vue";
import AssistantSkillManager from "./AssistantSkillManager.vue";
import AssistantContextSources from "./AssistantContextSources.vue";
...
const currentSpeed = useStorage('assistant-action-speed', 'normal')
...
const panelClasses = computed(() => ({
  "assistant-floating": store.panelState === "floating",
  "assistant-docked": store.panelState === "docked",
  "assistant-maximised": store.panelState === "maximised",
}));
...
function formatTranscript(): string {
  ...
  const sessionName = session?.name || (session?.session_number ? `Session #${session.session_number}` : "Assistant Chat")
...
function exportTranscript() {
  ...
  const session = store.activeSession
  const filename = `assistant-transcript-${session?.session_number ?? session?.id ?? "chat"}-${new Date().toISOString().split("T")[0]}.md`
...
.assistant-titlebar:active {
  cursor: grabbing;
}
.assistant-titlebar-btn {
  @apply flex items-center justify-center rounded p-1 transition-colors;
  color: hsl(var(--muted-foreground));
}
.assistant-titlebar-btn:hover {
