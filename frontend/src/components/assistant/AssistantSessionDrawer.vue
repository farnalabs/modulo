<template>
  <div class="assistant-sessions">
    <div class="flex items-center justify-between p-3 border-b">
      ...
      <Button severity="secondary" text icon-only @click="handleNewSession" :title="$t('components.assistant.AssistantSessionDrawer.new_session')" :aria-label="$t('components.assistant.new_session')">
        <svg
    ...
    >
      <span class="text-sm text-muted-foreground">{{ $t('components.assistant.AssistantSessionDrawer.loading') }}</span>
    </div>
    ...
    >
      <p class="text-sm text-muted-foreground">{{ $t('components.assistant.AssistantSessionDrawer.no_sessions_yet') }}</p>
      <Button severity="primary" link size="small" @click="handleNewSession" class="mt-2">{{ $t('components.assistant.AssistantSessionDrawer.start_a_new_chat') }}</Button>
    </div>
    ...
    <div v-else class="divide-y">
      <button
        ...
        :key="session.id"
        type="button"
        class="assistant-session-item w-full text-left"
      ...
      >
        <div class="flex items-start justify-between gap-2">
          ...
          <button
            type="button"
            class="assistant-session-delete shrink-0"
            @click.stop="handleDelete(session.id)"
            title="Delete"
            :aria-label="$t('components.assistant.delete_session')"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="12"
