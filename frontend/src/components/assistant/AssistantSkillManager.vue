<template>
  <div class="assistant-skills flex flex-col flex-1 overflow-hidden">
    <div class="flex items-center justify-between p-3 border-b">
      ...
      <Button v-if="!showForm" severity="secondary" text icon-only @click="openCreateForm" :title="$t('components.assistant.AssistantSkillManager.new_skill')" :aria-label="$t('components.assistant.new_skill')">
        <svg
    ...
    <div v-if="showForm" class="assistant-skill-form p-3 space-y-3 border-b">
      <input
        v-for="field in skillFields"
        :key="field.id"
        :id="field.id"
        v-model="form[field.model]"
        class="assistant-skill-input"
        :aria-label="$t(field.labelKey)"
        :placeholder="$t(field.labelKey)"
      />
      <textarea
        v-model="form.body"
        class="assistant-skill-textarea"
        :aria-label="$t('components.assistant.AssistantSkillManager.skill_body_markdown')"
        :placeholder="$t('components.assistant.AssistantSkillManager.skill_body_markdown')"
        rows="4"
      />
    ...
    <div class="flex-1 overflow-auto divide-y">
      ...
      >
        <p class="text-sm text-muted-foreground">{{ $t('components.assistant.AssistantSkillManager.no_skills_yet') }}</p>
      </div>
      <div v-for="skill in skills" :key="skill.id" class="assistant-skill-item p-3">
        <div class="flex items-start justify-between gap-2">
          ...
          <div class="flex gap-1 shrink-0">
            ...
            <button
              type="button"
              class="assistant-skill-action text-destructive hover:bg-destructive/10"
...
const skillFields: { id: string; model: SkillFieldModel; labelKey: string }[] = [
  { id: "assistantskill-name-input", model: "name", labelKey: "components.assistant.AssistantSkillDialog.skill_name" },
  { id: "assistantskill-description-input", model: "description", labelKey: "views.AdminAssistantView.skill_description" },
  { id: "assistantskill-triggers-input", model: "triggersText", labelKey: "components.assistant.AssistantSkillManager.triggers_commaseparated" },
];

async function fetchSkills() {
  skillError.value = null;
  try {
    const { data, error: err } = await api.GET("/api/v1/me/assistant/skills");
    if (err) {
