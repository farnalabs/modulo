<template>
  <div>
    ...
    <div v-else-if="error" class="mb-3 text-sm text-destructive">
      {{ error }}
      <button class="ml-2 underline" data-testid="team-notif-retry" @click="loadEndpoints">Retry</button>
    </div>

    <template v-else>
      <div v-if="teamEndpoints.length === 0 && !showAddForm" class="py-4 text-center text-sm text-muted-foreground">
        No webhook endpoints configured for this team.
      </div>

      <div v-for="ep in teamEndpoints" :key="ep.id" class="mb-2 rounded-lg border p-3">
        <div class="flex items-start justify-between">
          <div class="min-w-0 flex-1">
            <p class="truncate font-mono text-sm" :data-testid="'team-notif-url-' + ep.id">{{ ep.url }}</p>
            <p v-if="ep.description" class="mt-0.5 text-xs text-muted-foreground">{{ ep.description }}</p>
          ...
          <div class="ml-2 flex shrink-0 items-center gap-1">
            <span
              ...
              :class="ep.auto_disabled ? 'bg-destructive/10 text-destructive' : 'bg-success/10 text-success'"
            >
              <span class="h-1.5 w-1.5 rounded-full" :class="ep.auto_disabled ? 'bg-destructive' : 'bg-success'" />
        ...
        <div v-if="testResults[ep.id]" class="mt-2 rounded bg-muted p-2 text-xs" :class="testResults[ep.id].success ? 'text-success' : 'text-destructive'">
          <template v-if="testResults[ep.id].success">
            ✓ Test sent successfully (HTTP {{ testResults[ep.id].status_code }})
          </template>
          <template v-else>
            ✗ Test failed: {{ testResults[ep.id].error || 'HTTP ' + testResults[ep.id].status_code }}
          </template>
        </div>

        <div v-if="deleteConfirmId === ep.id" class="mt-3 rounded-lg border border-destructive/50 bg-destructive/10 p-3">
          <p class="text-sm font-medium text-destructive">Delete this webhook endpoint?</p>
          <p class="mt-1 text-sm text-destructive/80">This will stop all notifications to this URL.</p>
        ...
        <div v-if="editingId === ep.id" class="mt-3 space-y-3 rounded-lg border bg-muted/30 p-3">
          ...
          <div>
            <label class="mb-1 block text-xs font-medium">Secret <span class="text-muted-foreground">(leave blank to keep existing)</span></label>
            <input
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-3">
              <label v-for="evt in availableEvents" :key="evt" class="flex items-center gap-1.5 text-sm">
                <input type="checkbox" :value="evt" v-model="editForm.events" class="rounded border-input" />
                {{ evt }}
              </label>
            </div>
          </div>
      ...
      <div v-if="showAddForm && !editingId" class="mt-3 space-y-3 rounded-lg border bg-muted/30 p-3">
        ...
        <div>
          <label class="mb-1 block text-xs font-medium">Secret <span class="text-muted-foreground">(optional)</span></label>
          <input
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-3">
            <label v-for="evt in availableEvents" :key="evt" class="flex items-center gap-1.5 text-sm">
              <input type="checkbox" :value="evt" v-model="addForm.events" class="rounded border-input" />
              {{ evt }}
            </label>
          </div>
        </div>
...
type NotificationEndpointResponse = components['schemas']['NotificationEndpointResponse']
type TestResult = components['schemas']['TestResult']

const props = defineProps<{ teamId: string }>()

const availableEvents = ['hitl_awaiting', 'run_failed', 'claim_expired', 'hitl_overdue']
...
const addForm = ref({ url: '', secret: '', events: [] as string[], description: '' })
const adding = ref(false)
const addError = ref<string | null>(null)

const editingId = ref<string | null>(null)
const editForm = ref({ url: '', secret: '', events: [] as string[], description: '' })
...
async function saveEdit() {
  ...
  try {
    ...
    if (editForm.value.description) body.description = editForm.value.description
    else body.description = null
    if (editForm.value.secret) body.secret = editForm.value.secret

    const { error: err } = await api.PUT('/api/v1/notifications/{endpoint_id}', {
...
async function deleteEndpoint(id: string) {
  deleting.value = true
  try {
    const { error: err, response } = await api.DELETE('/api/v1/notifications/{endpoint_id}', {
...
async function test(ep: NotificationEndpointResponse) {
  ...
  try {
    const { data, error: err } = await api.POST('/api/v1/admin/notifications/{webhook_id}/test', {
      params: { path: { webhook_id: ep.id } },
    })
    if (err) {
      testResults.value[ep.id] = { success: false, status_code: null, response_body: null, error: String(err) }
    } else if (data) {
      testResults.value[ep.id] = data
    }
  } catch (e: unknown) {
    testResults.value[ep.id] = { success: false, status_code: null, response_body: null, error: e instanceof Error ? e.message : String(e) }
  } finally {
