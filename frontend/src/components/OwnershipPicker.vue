...
const ChevronDown: Component = { template: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>' }
const Earth: Component = { template: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>' }
const Users: Component = { template: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>' }
...
const selectedLabel = computed(() => {
  ...
  const team = teams.value.find(t => t.id === props.modelValue!.owner_team_id)
    ...
    <PopoverRoot v-model:open="open">
      ...
      <PopoverContent
        ...
        <button type="button"
          ...
          :class="{ 'bg-accent text-accent-foreground': modelValue?.visibility === 'org' && !modelValue?.owner_team_id }"
          @click="selectOrg">
        ...
        <div v-if="teams.length > 0" class="my-1 h-px bg-border" role="separator" />
        <template v-if="loading">
          <div class="px-2 py-4 text-center text-sm text-muted-foreground">Loading teams...</div>
        </template>
        <template v-else-if="fetchError">
          <div class="px-2 py-4 text-center text-sm text-destructive">{{ fetchError }}</div>
        </template>
        <template v-else-if="teams.length > 0">
          <div class="px-2 py-1.5 text-xs font-medium text-muted-foreground">Teams</div>
          <button v-for="team in teams" :key="team.id" type="button"
            class="relative flex w-full cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none hover:bg-accent hover:text-accent-foreground focus-visible:bg-accent focus-visible:text-accent-foreground"
            :class="{ 'bg-accent text-accent-foreground': modelValue?.owner_team_id === team.id }"
            @click="selectTeam(team)">
            <Users class="mr-2 h-4 w-4 text-muted-foreground shrink-0" />
            <span class="flex-1">{{ team.name }}</span>
            <span class="text-xs text-muted-foreground">{{ team.member_count }} {{ team.member_count === 1 ? 'member' : 'members' }}</span>
          </button>
        </template>
      </PopoverContent>
    </PopoverRoot>
