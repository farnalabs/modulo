<template>
  <div v-if="store.panelState !== 'closed'" class="remy-panel" :class="panelClasses" :style="panelStyle">
    <div class="remy-titlebar" @mousedown="startDrag">
      ...
      <div class="flex items-center gap-1">
        <button v-if="store.panelState === 'floating'" class="remy-titlebar-btn" @click="store.setPanelState('docked')" title="Dock">
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="3" x2="9" y2="21"/></svg>
        </button>
        <button v-if="store.panelState !== 'maximised'" class="remy-titlebar-btn" @click="store.setPanelState('maximised')" title="Maximise">
        ...
        <button v-else class="remy-titlebar-btn" @click="store.setPanelState('docked')" title="Minimise">
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="12" x2="21" y2="12"/></svg>
        </button>
        <button class="remy-titlebar-btn" @click="store.setPanelState('closed')" title="Close">
    ...
    <div class="remy-body">
      ...
      <div class="remy-main">
        <div v-if="!store.activeSessionId" class="flex flex-col items-center justify-center h-full gap-4 p-8 text-center">
          <div class="flex items-center justify-center rounded-full bg-primary/10 p-4">
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="text-primary"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2z"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>
          </div>
          <h3 class="text-lg font-semibold">Remy</h3>
          <p class="text-sm text-muted-foreground max-w-xs">AI assistant for your Modulo workspace. Start a new conversation or select an existing one.</p>
          <Button @click="handleNewSession">New Session</Button>
        </div>
        <template v-else>
          <div class="remy-chat-tabs flex items-center border-b px-2">
            <button class="remy-tab" :class="{ active: activeTab === 'chat' }" @click="activeTab = 'chat'">Chat</button>
            <button class="remy-tab" :class="{ active: activeTab === 'skills' }" @click="activeTab = 'skills'">Skills</button>
    ...
    <div v-if="store.panelState === 'floating' || store.panelState === 'docked'" class="remy-resize-handle" @mousedown="startResize" />
  </div>
...
function startDrag(e: MouseEvent) {
  ...
  dragging = true
  dragStart = { x: e.clientX, y: e.clientY, posX: store.panelPosition.x, posY: store.panelPosition.y }
...
function startResize(e: MouseEvent) {
  resizing = true
  resizeStart = { x: e.clientX, y: e.clientY, w: store.panelSize.width, h: store.panelSize.height }
...
.remy-titlebar:active { cursor: grabbing; }
.remy-titlebar-btn {
...
.remy-tab:hover { color: hsl(var(--foreground)); }
.remy-tab.active {
