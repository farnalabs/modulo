<template>
  <section class="space-y-6 rounded-lg border bg-card p-6" data-testid="variant-group-builder">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <h2 class="text-base font-semibold tracking-tight">
          {{ $t('views.variantCreator.title') }}
        </h2>
        <p class="mt-1 text-sm text-muted-foreground">
          {{ $t('views.variantCreator.subtitle') }}
        </p>
      </div>
      <Button
        severity="secondary"
        outlined
        type="button"
        data-testid="variant-builder-cancel-build"
        @click="emit('cancel')"
      >
...
watch([() => pipelinesData.value, () => props.initialPipelineId], ([data, initialPipelineId]) => {
  if (data) {
    const listResp = data as unknown as { items: PipelineItem[] }
    pipelines.value = listResp.items ?? []

    const deepLinked = initialPipelineId && pipelines.value.some(p => p.id === initialPipelineId)
    if (deepLinked) {
      selectedPipelineId.value = initialPipelineId as string
