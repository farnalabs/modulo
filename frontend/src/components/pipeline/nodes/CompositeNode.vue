...
const compositeName = computed(() => {
  if (!data.value.compositeRef) return ''
  const c = compositeStore.getCompositeById(data.value.compositeRef)
  return c?.name ?? data.value.compositeRef ?? ''
})

const portIndicator = computed(() => {
  const set = Object.keys(data.value.compositeParameterValues ?? {}).length
  const total = data.value.totalPorts ?? data.value.portCount ?? 0
...
<template>
  ...
  >
    ...
    <div class="mt-1 text-sm font-semibold text-indigo-100">
      {{ compositeName || data.label || 'Composite' }}
    </div>
