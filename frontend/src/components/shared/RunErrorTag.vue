...
import { errorCodeLabel, errorCodeDescription } from '../../utils/runUtils'
...
const description = computed(() => errorCodeDescription(props.code, t))
const pillClass = computed(() => {
  const cls = normalized.value.split('.')[0]
  return CLASS_STYLES[cls] ?? 'bg-muted text-muted-foreground'
})
// Accessible description: the full explanation (from errorCodeDescriptions)
// plus the raw detail if present. Screen readers announce this when the badge
// is focused, and it doubles as the hover tooltip via :title.
const ariaLabel = computed(() =>
  props.detail ? `${description.value}. ${props.detail}` : description.value,
)
</script>
