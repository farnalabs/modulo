...
describe('useAssistantStore API methods', () => {
  beforeEach(() => {
  ...
  it('fetchSessions loads the session list and clears the loading flag', async () => {
    ...
    expect(apiGet).toHaveBeenCalledWith('/api/v1/assistant/sessions')
  ...
  it('createSession prepends the new session, selects it, and clears messages', async () => {
    ...
    expect(apiPost).toHaveBeenCalledWith('/api/v1/assistant/sessions', expect.objectContaining({
  ...
  it('deleteSession removes the session and clears an active selection', async () => {
    ...
    expect(apiDelete).toHaveBeenCalledWith('/api/v1/assistant/sessions/{session_id}', {
      params: { path: { session_id: 's-1' } },
    })
  ...
  it('sendMessage only sends when a session is active', () => {
    ...
    store.activeSessionId = 's-1'
    store.sendMessage('hello assistant')
    expect(store.messages).toHaveLength(1)
    expect(store.messages[0]).toMatchObject({ role: 'user', content: 'hello assistant', session_id: 's-1' })
  ...
  it('pauseAssistant and resumeAssistant toggle the paused flag', () => {
    const store = useAssistantStore()
    store.pauseAssistant()
    expect(store.isPaused).toBe(true)
    store.resumeAssistant()
