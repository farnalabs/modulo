...
vi.mock('../lib/api/client', () => ({
  api: {
    GET: vi.fn().mockImplementation((path: string) => {
      if (path === '/api/v1/admin/assistant/config') {
        return Promise.resolve({ data: mockAssistantConfig, error: undefined })
      }
      if (path === '/api/v1/admin/assistant/available-providers') {
        return Promise.resolve({ data: mockAvailableProviders, error: undefined })
      }
      if (path === '/api/v1/admin/assistant/context-sources') {
        return Promise.resolve({ data: { product_primer: 'always_on', page_context: 'always_on', user_profile: 'always_on' }, error: undefined })
      }
      if (path === '/api/v1/admin/assistant/skills') {
...
describe('AdminAssistantView', () => {
  let wrapper: VueWrapper | null = null

  beforeAll(async () => {
    setActivePinia(createPinia())
    wrapper = mount(AdminAssistantView)
    await vi.waitFor(() => {
...
async function mountAssistant(overrides: {
  ...
  setActivePinia(createPinia())
  const wrapper = mount(AdminAssistantView)
  await flushPromises()
  await nextTick()
  return wrapper
}
...
describe('AdminAssistantView — save flows', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('renders provider status from the model-backends list', async () => {
    ...
    const providers = wrapper.find('[data-testid="assistant-providers"]')
  ...
  it('saves the default model configuration with parsed allowed models', async () => {
    ...
    const nameInput = wrapper.find('[data-testid="assistant-model-name"]')
    await nameInput.setValue('claude-sonnet-4')
    const contextInput = wrapper.find('[data-testid="assistant-model-context"]')
    await contextInput.setValue('128000')
    const allowedModels = wrapper.find('[data-testid="assistant-allowed-models"]')
  ...
  it('activates the allowed-provider chip on toggle (readonly vue-query data fix, FAR-630)', async () => {
    ...
    const chip = wrapper.find('[data-testid="assistant-allowed-provider-openai"]')
    expect(chip.attributes('aria-pressed')).toBe('false')
    await chip.trigger('click')
    await nextTick()
    // the chip activated
    const chipAfter = wrapper.find('[data-testid="assistant-allowed-provider-openai"]')
  ...
  it('saves the system prompt', async () => {
    ...
    const prompt = wrapper.find('[data-testid="assistant-system-prompt"]')
    await prompt.setValue('You are Assistant.')
    ...
    const put = vi.mocked((await import('../lib/api/client')).api.PUT).mock.calls[0]
    expect((put[1] as any).body).toEqual({ system_prompt: 'You are Assistant.' })
    wrapper.unmount()
  })
  ...
  it('saves the additional guidance', async () => {
    ...
    const guidance = wrapper.find('[data-testid="assistant-guidance"]')
  ...
  it('applies the locked-down tool permission preset and saves tool permissions', async () => {
    ...
    const modeSelect = wrapper.find('[data-testid="assistant-tool-perm-mode"]').findComponent({ name: 'Select' })
    ...
    const permsSection = wrapper.find('[data-testid="assistant-tool-permissions"]')
  ...
  it('saves the safety config with split pattern lists', async () => {
    ...
    await wrapper.find('[data-testid="assistant-rate-limit-max-actions"]').setValue('45')
    await wrapper.find('[data-testid="assistant-rate-limit-window"]').setValue('120')
    await wrapper.find('[data-testid="assistant-nogo-page-patterns"]').setValue('/admin/*, /billing')
    await wrapper.find('[data-testid="assistant-allowed-selectors"]').setValue('[data-safe]')

    const safety = wrapper.find('[data-testid="assistant-safety-limits"]')
  ...
  it('renders the knowledge sources table and saves a source mode change', async () => {
    ...
    const put = vi.mocked((await import('../lib/api/client')).api.PUT).mock.calls[0]
    expect(put[0]).toBe('/api/v1/admin/assistant/context-sources/{source_key}')
  ...
  it('renders the skills table with triggers and toggles a skill inactive', async () => {
    const wrapper = await mountAssistant({ skills: [skill()] })

    const skills = wrapper.find('[data-testid="assistant-skills"]')
    expect(skills.text()).toContain('Deploy Helper')
    expect(skills.text()).toContain('deploy')
    expect(skills.text()).toContain('Active')

    const toggle = wrapper.find('[data-testid="assistant-skill-toggle"]')
    await toggle.trigger('click')
    await flushPromises()

    const put = vi.mocked((await import('../lib/api/client')).api.PUT).mock.calls[0]
    expect(put[0]).toBe('/api/v1/admin/assistant/skills/{skill_id}')
  ...
  it('shows a skill toggle failure inline', async () => {
    ...
    await wrapper.find('[data-testid="assistant-skill-toggle"]').trigger('click')
    await flushPromises()
  ...
  it('shows the no-skills empty state and hides the skills-as-knowledge section', async () => {
    const wrapper = await mountAssistant({ skills: [] })

    expect(wrapper.find('[data-testid="assistant-skills"]').text()).toContain('No skills configured yet')
  ...
  it('regenerates the product primer and shows the success message', async () => {
    ...
    await wrapper.find('[data-testid="assistant-primer-regenerate"]').trigger('click')
    await flushPromises()

    const post = vi.mocked((await import('../lib/api/client')).api.POST).mock.calls[0]
    expect(post[0]).toBe('/api/v1/admin/assistant/primer/regenerate')
