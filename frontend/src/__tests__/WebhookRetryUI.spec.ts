...
describe('WebhookRetryUI', () => {
  ...
  describe('Retry All Failed button', () => {
    ...
    it('shows Retrying All… text while retry-all is in progress', async () => {
      ...
      await btn.trigger('click')
      await nextTick()
      const confirmBtn = wrapper.find('[data-testid="admin-notification-log-retry-all-confirm"]')
      expect(confirmBtn.exists()).toBe(true)
      await confirmBtn.trigger('click')
      await nextTick()
      expect(confirmBtn.text()).toBe('Retrying All…')
    })
    ...
    it('calls retry-all endpoint on confirm click', async () => {
