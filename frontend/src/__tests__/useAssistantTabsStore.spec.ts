...
import { useAssistantTabsStore } from '../composables/useAssistantTabsStore'
...
describe('useAssistantTabsStore', () => {
  it('first-mount SEED: live restored activeSessionId with no tabs seeds a tab and does not null it', () => {
    ...
    const tabs = useAssistantTabsStore()
    tabs.reconcile()
  ...
  it('persists seeded tabs to localStorage and restores them on a fresh store instance', async () => {
    ...
    const stored = JSON.parse(localStorage.getItem('assistant-only-tabs') || '[]')
    ...
    const tabs2 = useAssistantTabsStore()
  ...
  it('corrupt JSON in localStorage resets tabs to an empty list', () => {
    localStorage.setItem('assistant-only-tabs', 'not-json{{{')
    const tabs = useAssistantTabsStore()
