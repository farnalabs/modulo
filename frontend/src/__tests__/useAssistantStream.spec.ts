...
vi.mock('../stores/planStore', () => ({
  usePlanStore: vi.fn(() => ({
    featureEnabled: vi.fn((name: string) => {
      if (name === 'assistant_ui_driving') return true
      return false
    }),
  })),
}))
...
describe('useAssistantStream', () => {
  ...
  it('ui_command_batch event executes commands and POSTs results', async () => {
    ...
    global.fetch = vi.fn().mockImplementation((url: string, opts?: any) => {
      if (url === '/api/v1/assistant/sessions/session-1/stream') {
  ...
  it('refuses a second same-session stream while one is in flight', async () => {
    ...
    const first = connectStream('session-1')
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(useAssistantStore().isStreaming).toBe(true)
    ...
    releaseFirst({ ok: true, body: createMockSSEStream([{ event: 'done', data: { message_id: 'm1' } }]) })
    await first
    expect(useAssistantStore().isStreaming).toBe(false)
  })
  ...
  it('assistant-only mode does not execute ui_command_batch (defense-in-depth)', async () => {
    setupStore()
  ...
  it('skips the command batch when the assistant_ui_driving feature flag is off', async () => {
    setupStore()
