...
function mountSources() {
  return mount(RemyContextSources, {
    global: {
      stubs: {
...
describe('RemyContextSources', () => {
  ...
  it('fetches sources on mount and renders them', async () => {
    ...
    expect(apiGet).toHaveBeenCalledWith('/api/v1/me/remy/context-sources')
    expect(wrapper.findAll('.remy-cs-row')).toHaveLength(2)
  ...
  it('shows loading state while fetching', async () => {
    ...
    expect(wrapper.findAll('.remy-cs-row')).toHaveLength(0)
  })
  ...
  it('displays the current source_mode in the select', async () => {
    ...
    const select = wrapper.find('.remy-cs-select')
  ...
  it('updates source mode optimistically and on success', async () => {
    ...
    expect(apiPut).toHaveBeenCalledWith('/api/v1/me/remy/context-sources/{source_key}', {
      params: { path: { source_key: 'web' } },
      body: { source_mode: 'off' },
    })
    // After PUT success, the list is replaced with the API response
    expect(wrapper.findAll('.remy-cs-row')).toHaveLength(1)
  })
  ...
  it('calls DELETE then fetchSources on reset', async () => {
    ...
    expect((wrapper.find('.remy-cs-select').element as HTMLSelectElement).value).toBe('off')

    await wrapper.find('button').trigger('click')
    await flushPromises()

    expect(apiDelete).toHaveBeenCalledWith('/api/v1/me/remy/context-sources')
  ...
  it('shows key tooltip on the info icon span', async () => {
    ...
    const infoSpan = wrapper.find('.remy-cs-row span[title]')
