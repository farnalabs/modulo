...
async def seed() -> None:
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            pipelines_exist = (
                await session.execute(select(Pipeline).where(Pipeline.organisation_id == org.id).limit(1))
            ).first()
            ...
            for email, display_name, org_role in user_defs:
                existing = (
                    await session.execute(select(User).where(User.organisation_id == org.id, User.email == email))
                ).scalar_one_or_none()
            ...
            for name, desc, members in team_defs_data:
                t = (
                    await session.execute(select(Team).where(Team.organisation_id == org.id, Team.name == name))
                ).scalar_one_or_none()
                if t is None:
            ...
            schemas = []
            for sname, _slug_name, desc, schema_json in schema_defs_data:
                s = (
                    await session.execute(select(Schema).where(Schema.organisation_id == org.id, Schema.name == sname))
                ).scalar_one_or_none()
                if s is None:
            ...
            agent_defs = [
                ...
                (
                    ...
                    [{"connector_type": "shell", "capabilities": ["read"]}],
                ),
                (
                    "Changelog Writer",
                    "Writes changelogs from commit data",
                    4,
                    2,
                    1,
                    backends[1].id,
            ...
            for (
                ...
                a = Agent(
                    ...
                    name=name,
                    description=desc,
                    input_schema_id=in_s.id,
                    input_schema_version="1.0",
                    output_schema_id=out_s.id,
            ...
            for pd in pipeline_defs:
                ...
                for i, label in enumerate(pd["stages"]):
                    node = {
                        "id": node_ids[i],
                        "type": "agent" if "Gate" not in label else "manual",
                        "label": label,
                        "position": {"x": 50 + i * 280, "y": 100},
                    }
                ...
                for i in range(len(node_ids) - 1):
                    ...
                    edge = PipelineEdge(
                        ...
                        source_node_id=uuid.UUID(node_ids[i]),
                        target_node_id=uuid.UUID(node_ids[i + 1]),
            ...
            lib_primitives = [
                LibraryPrimitive(
                    ...
                    description="Analyses pull requests for code quality, security, and style",
                    author=str(admin.id),
                    version="1.0.0",
                ...
                LibraryPrimitive(
                    ...
                    author=str(admin.id),
                    version="1.0.0",
                    tags=["workflow", "pr", "github"],
                    content_json={"steps": ["review", "analyze", "approve"], "timeout": 600},
                    visibility="org",
                    created_by=admin.id,
                ),
                LibraryPrimitive(
                    ...
                    author=str(admin.id),
                    version="1.0.0",
                    tags=["workflow", "incident", "ops"],
                    content_json={"steps": ["triage", "respond", "resolve", "postmortem"], "timeout": 3600},
                    visibility="org",
                    created_by=admin.id,
                ),
                LibraryPrimitive(
                    ...
                    description="Connects to GitHub for issue reading and PR creation",
                    author=str(admin.id),
                    version="1.0.0",
                    tags=["integration", "github", "git"],
                    content_json={"type": "github", "capabilities": ["issue_read", "create_pr"], "auth_type": "oauth"},
                    visibility="org",
                    created_by=admin.id,
                ),
                LibraryPrimitive(
                    ...
                    content_json={"type": "slack", "capabilities": ["send_message"], "webhook_required": True},
                    visibility="org",
                    created_by=admin.id,
                ),
                LibraryPrimitive(
                    ...
                    name="PR Review Pipeline Template",
                    slug="pr-review-template",
                    description="Template for creating a PR review pipeline",
                    author=str(admin.id),
                    version="1.0.0",
                    tags=["template", "pr-review"],
                    content_json={"agents": [], "graph_nodes": [], "edges": [], "category": "code-review"},
                    visibility="org",
                    created_by=admin.id,
                ),
                LibraryPrimitive(
                    ...
                    slug="release-template",
                    description="Template for creating a release pipeline",
                    author=str(admin.id),
                    version="1.0.0",
                    tags=["template", "release"],
                    content_json={"agents": [], "graph_nodes": [], "edges": [], "category": "release"},
                    visibility="org",
                    created_by=admin.id,
                ),
                LibraryPrimitive(
                    ...
                    name="PR Review Test Fixture",
                    slug="pr-review-fixture",
                    description="Test fixture for PR review pipeline",
                    author=str(admin.id),
                    version="1.0.0",
                    tags=["test", "fixture", "pr-review"],
                    content_json={"fixture_map": {"Review: Add login endpoint": "Approved with minor comments"}},
                    visibility="org",
                    created_by=admin.id,
                ),
                LibraryPrimitive(
                    ...
                    source="local",
                    primitive_type="test_fixture",
                    name="Incident Test Fixture",
                    slug="incident-fixture",
                    description="Test fixture for incident response pipeline",
                    author=str(admin.id),
                    version="1.0.0",
                    tags=["test", "fixture", "incident"],
                    content_json={
                        "fixture_map": {"Alert: P99 latency spike": "CRITICAL — investigate database query performance"}
                    },
                    visibility="org",
                    created_by=admin.id,
                ),
            ...
            # ── Eval Definitions (4) ──────────────────────────────────────
            eval_defs_list = []
            for ed_name, ed_type, _ed_desc in [
            ...
            for i in range(15):
                ...
                run = Run(
                    ...
                    completed_at=now - timedelta(hours=i * 3 - 0.5)
                    if statuses[i] in ("complete", "failed", "cancelled")
                    else None,
                    ...
                    error_detail="Agent exceeded maximum execution time of 300s"
                    if statuses[i] == "failed" and i == 6
                    else None,
                    outputs_json={"summary": f"Run {i} completed successfully", "findings": []}
            ...
            for ri in range(5):
                session.add(
                    PrimitiveRating(
                        organisation_id=org.id,
                        primitive_id=lib_primitives[ri].id,
                        user_id=users[ri + 1].id,
                        thumbs_up=ri % 2 == 0,
                        comment=[
                            "Great for PR reviews!",
                            "Works well for changelogs",
                            "Good schema for PR data",
                            "Release schema is solid",
                            "Nice integration",
                        ][ri],
                    )
                )
            ...
            session.add(
                OrgApiKey(
                    ...
                    lookup_prefix="mod_cd_",
                    hashed_secret="hash_placeholder_1",
                    role="operator",
                    created_by=admin.id,
                )
            )
            session.add(
                OrgApiKey(
                    ...
                    lookup_prefix="mod_dev_",
                    hashed_secret="hash_placeholder_2",
                    role="runner",
                    created_by=admin.id,
                )
            )
            ...
            for i in range(12):
                er = EvalResult(
                    organisation_id=org.id,
                    eval_id=eval_defs_list[i % 4].id,
                    run_id=runs_list[i % len(runs_list)].id if runs_list else None,
                    passed=i % 3 != 0,
                    score=0.75 + (i % 3) * 0.1,
                    detail=f"eval-result-{i}: {'pass' if i % 3 != 0 else 'fail'}",
            ...
            audit_defs = [
                ...
                (
                    ...
                    "run",
                    None,
                    {"pipeline": pipelines_list[1].name, "error": "AGENT_TIMEOUT"},
                ),
                (
                    "run.awaiting_human",
                    users[5].id,
                    "run",
                    None,
                    {"pipeline": pipelines_list[2].name, "gate": "Verification Gate"},
                ),
                ...
                (
                    "eval.definition_created",
                    admin.id,
                    "eval_definition",
                    eval_defs_list[0].id,
                    {"name": eval_defs_list[0].name},
                ),
                (
                    "eval.result_recorded",
                    admin.id,
                    "eval_result",
                    None,
                    {"eval": eval_defs_list[0].name, "passed": True},
                ),
