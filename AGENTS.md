...
- **Backend**: Python 3.12, uv, FastAPI, LangGraph, SQLAlchemy 2 async + asyncpg, Alembic
...
- All async DB uses `asyncpg` (Postgres) or `aiosqlite` (SQLite dev only). No `psycopg2`/`sqlite3` in async path. Semgrep-enforced.
