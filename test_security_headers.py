...
@pytest.mark.anyio
async def test_content_security_policy(app, async_client) -> None:
    resp = await async_client.get("/test")
    assert resp.headers["Content-Security-Policy"] == EXPECTED_CSP


@pytest.mark.anyio
async def test_x_frame_options(app, async_client) -> None:
    resp = await async_client.get("/test")
    assert resp.headers["X-Frame-Options"] == EXPECTED_XFO


@pytest.mark.anyio
async def test_x_content_type_options(app, async_client) -> None:
    resp = await async_client.get("/test")
    assert resp.headers["X-Content-Type-Options"] == EXPECTED_CTO


@pytest.mark.anyio
async def test_referrer_policy(app, async_client) -> None:
    resp = await async_client.get("/test")
    assert resp.headers["Referrer-Policy"] == EXPECTED_REFERRER


@pytest.mark.anyio
async def test_permissions_policy(app, async_client) -> None:
    resp = await async_client.get("/test")
    assert resp.headers["Permissions-Policy"] == EXPECTED_PERMISSIONS


@pytest.mark.anyio
async def test_all_headers_present(app, async_client) -> None:
    resp = await async_client.get("/test")
    assert resp.headers["Content-Security-Policy"] == EXPECTED_CSP
    assert resp.headers["X-Frame-Options"] == EXPECTED_XFO
    assert resp.headers["X-Content-Type-Options"] == EXPECTED_CTO
...
@pytest.mark.anyio
async def test_hsts_sent_in_production(app, async_client) -> None:
    resp = await async_client.get("/test")
