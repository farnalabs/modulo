...
- **Remediation** is the guardrail-override extension of `recover_node` (adds `eval_failed` to the recoverable statuses). `deliver_manual` is unavailable for terminal runs (no HITL gate → 404). The override re-runs the guardrail pass on the operator-supplied input (re-block safe default), persists the post-redaction payload, flips the run to `pending`, and sets `is_replay=True` so lifecycle-map journeys increment exactly once.
