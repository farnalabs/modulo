...
| `run_concurrency_limit` (planned) | Per organisation | Planned org-level cap on concurrently executing/claimed runs across ALL pipelines in the org (pending runs excluded) — **not yet implemented**. Intended behaviour once it ships: enforced at dispatch time; runs beyond the cap deferred back to `pending` with `error_code='org_capacity_limited'` and re-dispatched by `dispatcher_reconcile` when capacity frees. Default: `null` (unlimited). Stored in `Organisation.settings_json`. |
