...
The "never write a file" / "no egress" promise for a `sandbox_agent` node must be mechanically enforceable at the sandbox boundary, not merely declared. The capability-profile derivation (PR A) + the sandbox policy enforcement surface (PR B) make all three sandbox surfaces first-class, mechanically-certifiable conformance surfaces.

**Capability vocabulary (mechanically derived, `modulo.core.pipeline_engine.sandbox_mode.derive_sandbox_capabilities`):**

- `sandbox.write_files` — False when `read_only` is True (PR B: the workspace is chmodded read-only at runtime, so writes are impossible for the agent's non-root user), True when `read_only` is False (writable), None when the value is unrecognised or absent (unknown — fail-closed). `read_only` is a real validated + enforced `PipelineGraphNode` field.
- `sandbox.egress` — False when `egress_policy` is `deny_all` or `selected` (the dispatch maps both to `allow_internet_access=False`, FAR-296 Phase 3; `selected` additionally enforces the host:port allowlist via a sandbox firewall step in PR B), True when `default`/absent, None when the declared value is unrecognised.
- `sandbox.git_credentials` — True when `git_credentials` is `scoped` (PR B: a credential helper limits the token to the allowlisted github.com host), False when `unscoped`/`none`, None when the value is unrecognised or absent (unknown — fail-closed). `git_credentials` is a real validated + enforced `PipelineGraphNode` field.

The derivation is MECHANICAL — it reads the node's actual enforced configuration, never a declared claim — so the conformance hard-block CERTIFIES what is genuinely enforced (egress, read-only workspace, scoped git credentials) and fails CLOSED (unknown) for anything not a real, validated, enforced config. It never certifies from unvalidated/unenforced dict keys (a smuggled key in an imported workflow must not certify a deny-guarantee nothing enforces).

**Conformance wiring (PR A):** the live-manifest reader (`conformance.build_live_manifest`) derives the sandbox surface from the `sandbox_agent` node's config at node start and stamps it into the capability manifest. A block-action guardrail's `required_capabilities` on the sandbox write/egress surface is a DENY/negative guarantee: `["sandbox.write_files"]` certifies writes are impossible, `["sandbox.egress"]` certifies no egress. The manifest inverts the raw mechanical polarity for write/egress (confirmed-absent write/egress is the certified guarantee), so the existing three-state derivation reads it correctly — confirmed-absent → `present` (certified), present → `absent` (claim violated → fail-closed block), unknown → `unknown` (fail-closed block). `sandbox.git_credentials` is a POSITIVE guarantee (scoped = certified) and is NOT inverted. The enforcement surface (read-only mounts, network-egress allowlist, git-credential scope) is shipped in PR B (`modulo.core.pipeline_engine.sandbox_policy`).
