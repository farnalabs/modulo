...
- **Reject→correction edge** — a HITL gate's `hitl_gate_config` MAY declare a `correction_target` (accepted + persisted through the graph contract, `HitlGateConfig`). The reject→correction DISPATCH seam is NOT yet wired: no correction node type exists in `node_runner` and no production path dispatches `FeedbackManager.run_single_node_correction` from the graph — a rejection currently kicks back to the plain `reject_target` (MAJOR-5 Option B). Wiring the seam is a tracked follow-up; T1's `recover_node` override stays the break-glass path. Correction runs (`trigger_type='correction'`) are EXCLUDED from the pipeline `retry_policy` re-dispatch — no chained corrections; the single retry budget is owned by the correction path.
