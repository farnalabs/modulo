...
| `run_concurrency_limit` | Per organisation | Max concurrently executing/claimed runs across ALL pipelines in the org (pending runs excluded). Enforced at dispatch time — new runs beyond the cap are deferred back to `pending` with `error_code='org_capacity_limited'` and re-dispatched by `dispatcher_reconcile` when capacity frees. Default: `null` (unlimited). Stored in `Organisation.settings_json`. |
