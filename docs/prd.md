...
- **Convergence check** — prior states are fingerprinted (canonical SHA-256 over sorted-key JSON). A previously-seen or strictly-worse input/output state escalates `converged` to HITL immediately — no oscillation burn.
