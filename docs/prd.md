...
#### Guardrail trust model PR C (shipped — FAR-309)

The guardrail trust-model migrations are **down-migration rollback-safe** (FAR-309 PR C, the final FAR-223 item 8 follow-up). `0113_guardrail_summary` and `0116_guardrail_trust_pr_b` are plain additive migrations chained onto the linear revision graph, and their DOWNGRADE drops exactly the columns the upgrade added — an org rolling the schema back can do so cleanly, losing no data beyond the feature's own columns. Verified by unit round-trip tests that execute both migrations against an in-memory SQLite engine (up → down → up) and assert: the trust surface (`runs.guardrail_summary_json`, `pipeline_snapshots.guardrail_pins_fingerprint`, `eval_definitions.deleted_at`/`deleted_by`, `ix_eval_definitions_deleted_at`) appears, disappears, and re-appears at each step; the downgrade drops ONLY its own columns — never the owning tables or their rows (ordinary eval definitions and soft-deleted guardrails survive a downgrade); and the ORM models (`Run`, `EvalDefinition`, `PipelineSnapshot`) stay consistent with the schema at every revision, enforced by a model/migration drift guard that fails if a trust column exists on one side but not the other.
...
**Pack content shipped (FAR-216 PR B):** the SOC 2 pack ships as `modulo.core.guardrails.packs.soc2` (`SOC2_PACK`, built by `build_soc2_pack()`). It carries the concrete TSC controls and their guardrail mappings, all deterministic (regex/json_schema only — no LLM judges): CC6.1 (logical & physical access control — BLOCK AWS access-key identifiers in payloads), CC6.6 (malware prevention — BLOCK executable/script content markers: `<script>`, `javascript:`, base64 MZ/PE markers), CC7.2 (anomaly monitoring — OBSERVE SQL-injection / path-traversal markers, warn-mode-first by design), CC8.1 (change management — WARN on payloads that are not a well-formed non-empty object), A1.2 (availability — WARN on payloads exceeding size bounds via json_schema `maxProperties`/`maxLength`), and P4.1 (personal-information handling — REDACT PII fields: SSN, credit card, email, via STATIC field-path redaction). Every control is mapped and the pack passes `assert_pack_ci_ready` (zero unmapped, zero uninstantiable).
...
- `sandbox.write_files` — ALWAYS `None` (unknown) today. The declared read-only workspace flag (`read_only`) is NOT a `PipelineGraphNode` field and node_runner/e2b do not enforce a read-only filesystem, so the capability cannot be certified. It resolves unknown (fail-closed block) until the PR B enforcement surface (read-only mounts) lands together with the `PipelineGraphNode` / GraphValidator / node_runner surface. The derivation deliberately never certifies from the unvalidated/unenforced `read_only` key.
- `sandbox.egress` — False when `egress_policy` is `deny_all` or `selected` (the dispatch maps both to `allow_internet_access=False`, FAR-296 Phase 3), True when `default`/absent, None when the declared value is unrecognised. This is the ONLY currently mechanical/enforced capability — `egress_policy` is a validated `PipelineGraphNode` field that node_runner enforces.
- `sandbox.git_credentials` — ALWAYS `None` (unknown) today. The declared git-credential scope (`git_credentials`) is NOT a `PipelineGraphNode` field and node_runner/e2b never scope git credentials, so the capability cannot be certified. It resolves unknown (fail-closed block) until the PR B enforcement surface (git-credential scope) lands together with the `PipelineGraphNode` / GraphValidator / node_runner surface. The derivation deliberately never certifies from the unvalidated/unenforced `git_credentials` key.

The derivation is MECHANICAL — it reads the node's actual configuration, never a declared claim — so the conformance hard-block CERTIFIES egress is impossible rather than trusting a declaration. `sandbox.write_files` / `sandbox.git_credentials` are deliberately UNKNOWN: a conformance claim on them must NEVER be certified from dict keys that are neither validated by `PipelineGraphNode` nor enforced by node_runner — a smuggled `read_only: true` / `git_credentials: "scoped"` in an imported workflow (which stores node dicts raw without model validation) would otherwise certify a deny-guarantee nothing enforces (the e2b sandbox is still writable with full git access). This violates the "MECHANICAL — never a declared claim" core claim, so those capabilities fail CLOSED until PR B.

**Conformance wiring (PR A):** the live-manifest reader (`conformance.build_live_manifest`) derives the sandbox surface from the `sandbox_agent` node's config at node start and stamps it into the capability manifest. A block-action guardrail's `required_capabilities` on the sandbox surface is a DENY/negative guarantee: `["sandbox.write_files"]` certifies writes are impossible, `["sandbox.egress"]` certifies no egress. The manifest inverts the raw mechanical polarity for write/egress (confirmed-absent write/egress is the certified guarantee), so the existing three-state derivation reads it correctly — confirmed-absent → `present` (certified), present → `absent` (claim violated → fail-closed block), unknown → `unknown` (fail-closed block). Because `sandbox.write_files` always derives unknown today, a claim on it always resolves `unknown` (fail-closed block) — it can only become `present` once PR B makes it mechanical/enforced. `sandbox.git_credentials` is a POSITIVE guarantee and is NOT inverted: `["sandbox.git_credentials"]` certifies the node's git credentials are scoped (limited), so scoped → `present` (certified), unscoped/absent → `absent` (fail-closed block), undeclared → `unknown` (fail-closed block). It too always derives unknown today and resolves `unknown` (fail-closed block) until PR B. Enforcement of the policy surface itself (read-only mounts, network-egress allowlist, git-credential scope) is PR B.
