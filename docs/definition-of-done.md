...
| Integration | `backend/tests/integration/` | real Postgres via testcontainers, Alembic migrations applied first, Factory Boy. Migrations include `test_migrations` round-trips (e.g. `0092_ongoing_trigger_type` NOT VALID/VALIDATE + partial CHECKs + indexes) | Docker | `uv run pytest tests/integration/<file> -q -m integration` (usually CI / merge-queue only) |
