...
- `claim_expiry_minutes` decreased (non-retroactive against outstanding claims — `HitlClaim.expires_at` set once at claim creation).
...
- Unit tests for the diff primitive, covering all five weakening-capable fields.
...
5. **New this iteration**: should this plan's permission key (`"pipeline.graph.weakening"` or similar) actually be registered in ADR-017's `PERMISSIONS` dict for route-layer defense-in-depth, in addition to the service-layer backstop? Recommend yes, as a small addition once `task-authz-centralized-enforcement` merges — not blocking this plan's core (the service-layer backstop is the load-bearing control either way).
