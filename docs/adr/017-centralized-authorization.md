...
- **MCP** has a per-tool scope matrix (`core/mcp/scope_validator.py`): `TOOL_SCOPE_REQUIREMENTS` maps tool → minimum org role; `check_tool_scope()` compares against `ORG_ROLE_HIERARCHY` (viewer 0 < runner 1 < operator 2 < admin 3).
- **REST** uses `get_current_tenant_user` (`auth/dependencies.py`), which verifies authentication + org membership only — **no role comparison**. Any org member, including `viewer`, can call every mutating endpoint.
...
5. **API-key role escalation**: any org member can create an `operator`-role key; MCP middleware trusts the stored role (`mcp_server.py:293`).
...
10. ~~**THREE deprecated MCP tool aliases**: `get_trigger_events` (`mcp_server.py:1410-1423`), `browse_library` (`:1338-1351`), `get_documentation` (`:2092-2100`) — plus module-level alias assignments at `:2592-2594`. **Remy's runtime prompt advertises `get_documentation`** (`core/remy/skill_loader.py:34`); frontend references exist (`AdminRemyView.vue:834`, `en-US.js:949`); PRD documents the alias-preservation contract (~12 refs incl. `prd.md:540-542`).~~ **[RESOLVED — REMOVED 2026-07-31 in PR #434 (A1a implementation). Audit 2026-08-03 confirms none of the three names remain anywhere in `backend/src` — the tools are registered as `search_library`/`list_trigger_events`/`search_documentation` (see `tests/unit/test_mcp_structural_coverage.py` `_EXPECTED_TOOLS`); `core/remy/skill_loader.py:34` now advertises `search_documentation`; the frontend and PRD alias references no longer exist.]**
11. **`review_hitl:deliver_manual` is impossible via MCP today**: action accepted (`mcp_server.py:1192`) but missing from `TOOL_SCOPE_REQUIREMENTS` → "Unknown action" → `insufficient_scope` for all roles.
12. **`organisations.settings_json` is `sa.JSON`, not JSONB** (`organisation.py:26`) — `jsonb_set` cannot target it without a cast; `get_effective_setting` (`settings_resolver.py:13-35`) falls through to process-global `SystemConfig` (no `organisation_id`); there is **no parent-app auth middleware** (only the MCP sub-app's `McpAuthMiddleware`); `validate_api_key` flushes `last_used_at` (`api_key.py:144-145`); `validate_current_auth` never re-sets `_ctx_role`.
...
- **Migration-failure handling**: entrypoint stays non-fatal; `_run_migrations` (currently a no-op) becomes the **single authoritative runner** with a bounded retry loop and **FATAL exhaustion INSIDE the function** (it is called bare at `main.py:684` — the bounds must not depend on the caller), guarded by the Postgres advisory-lock pattern. **Hard-fail owner-rows assertion runs AFTER the retry loop** (actionable message listing offending `account_id`s + prescribed UPDATE); `SQLAlchemyError` from the assertion query → log + continue (invariant guaranteed by the migration transaction). **Avoid the double-run crash-loop**: `_run_migrations` must tolerate the entrypoint's prior `alembic upgrade heads` (advisory-lock wait sized against concurrent-machine lock waits; dedicated test). **`infra_blocked` producer defined** (iteration-7: no consumer pipeline exists today — `list_pipelines` shows no Deploy Agent; wiring a consumer + Branch Fixer prompt branch for `infra_blocked` is a follow-up task, not a blocker for this ADR).
- **JWT live-role re-read**: centralized `resolve_role_from_membership(session, account_id, organisation_id)` in **`auth/dependencies.py`** (beside `_verify_identity`; lazy-import pattern). Used by: `get_current_tenant_user`, MCP JWT middleware + SSE revalidation (TTL-bounded ONLY on SSE), `/refresh` (same transaction as `advance_sequence`), OAuth token/refresh/per-call clamp, API-key per-call clamp. Filters **`deactivated_at IS NULL`**. Failure modes: missing/deactivated  **401**; SQLAlchemyError  **DENY (503)** on BOTH REST and MCP (review decision 2026-07-31: no degrade-to-claim - a DB blip must not restore a removed user's stale role; no production users, so fail-closed is the safe choice); other  500. **Breaking-change note**: accounts without an active org_memberships row are 401'd on tenant routes; system admins use get_current_user (not tenant) so they are unaffected; no production accounts exist to break. `get_current_tenant_user_optional` **stays claim-only** (documented webhook impact). Query budget: piggyback the kill-switch read onto the membership read.
- **No deprecation window**: bare `owner` JWT claim denied at -1 everywhere.
- **8-site cleanup + ORM + docs** (verified complete): `pipelines.py:1419`, `admin.py:1644` (preserve `is_system_admin` bypass), `contributions.py:176`, `cli/migrate.py:80,101` (live-check-only), `alert_dispatcher.py:139`, `pipelines.py:942` log, ORM model `org_membership.py:14` + assertion `"owner"` NOT in the model check, docs (PRD §9.2 intro + §9.3.1, product-map org-entity/pipeline-diff-rollback, penetration-test-plan). Semgrep/pattern check: no route checks the literal string `"owner"` as a role.
...
**Single-source-of-truth**: `TOOL_SCOPE_REQUIREMENTS` becomes `tool -> permission-key` referencing `PERMISSIONS`. **`_VALID_ROLES` loop rewritten**: resolve each tool's permission key through `PERMISSIONS` (raise on unknown), assert resolved role in `ORG_ROLE_HIERARCHY`. **Delete dead key + remove ALL THREE deprecated aliases** — including the module-level alias assignments at `mcp_server.py:2592-2594` and the `:2978` docstring. **Complete consumer inventory (iteration-7)** **[IMPLEMENTED — PR #434, 2026-07-31: dead key deleted, aliases removed, all consumers below migrated]**:
- Production: `core/remy/skill_loader.py:34` (prompt advertises `get_documentation` → switch to `search_documentation`), `frontend/src/views/AdminRemyView.vue:834` (`toolCall: 'get_documentation()'`), `frontend/src/locales/en-US.js:949` (`browse_library` key).
...
- Docs: PRD ~12 refs (`:530,:532,:540-542,:1947,:2639,:2664,:2681,:2688,:2766,:2812,:2822` — incl. the alias-preservation contract that the removal overturns), `alpha.md:144`.
...
3. `require_system_or_org_admin(permission)` — the one true hybrid: `admin.py:1641` (org deletion).
...
| `admin.py` org deletion (5 routes) | sys-admin OR org admin | `require_system_or_org_admin` |
...
| `PUT .../email-settings`, `POST .../email-settings/test`, `admin_orgs.py:419,506` | same, no role min today | mutations variant: `org.email.manage` @ admin |
...
| SCIM (`scim.py`) | `MODULO_SCIM_TOKEN` | Exempt at phase 1. `scim_update_user` has a *functional* role-UPDATE — keep unwired (or delete); test no SCIM route sets an org role; document runner-default grant for Phase 3 |
