...
| Max concurrent runs per org (all pipelines) | `Organisation.settings_json["run_concurrency_limit"]` (via `GET`/`PUT /api/v1/admin/org/run-concurrency`) | Org-wide safeguard against flooding the shared worker pool across all pipelines; `null` = unlimited |
