...
| Storage | environment variable or K8s Secret — never in code |
...
- Do not commit `MODULO_USERS` to any configuration file — inject it via the
  runtime environment (K8s Secret, Docker secret, or vault).
- Use bcrypt-hashed passwords. The format is `$2b$12$<hash>` (generated by
  the `modulo init` CLI or any bcrypt tool).
...
For Kubernetes, set `securityContext` to enforce this at the pod level:
...
state is in Postgres/Redis). Enable read-only root filesystem:
...
If a component needs temporary disk (e.g., file uploads, export staging),
...
### 5.5 Image Pull Policy

```yaml
imagePullPolicy: Always
```

In production, pin to a specific digest instead of a tag:

```yaml
image: ghcr.io/farnalabs/modulo-backend@sha256:<digest>
```

This prevents tag-mutation attacks on the registry.
...
**Configuration:**
- Audit log retention is configured via `MODULO_AUDIT_RETENTION_DAYS` (default:
  365).
- Export to JSONL or CSV via the admin UI or `modulo audit export`.
...
| Audit log | 365 days (configurable) | Database (`audit_log` table) |
...
3. **Deploy with rolling update** (Kubernetes):
   ```bash
   kubectl set image deployment/modulo-backend backend=ghcr.io/farnalabs/modulo-backend:v2.0.0 -n modulo-prod
...
kubectl exec deploy/modulo-backend -n modulo-prod -- uv run alembic downgrade --sql -1  # preview SQL
...
- **Minimum replicas:** 2 for backend, 2 for frontend.
- **Readiness probe:** Must return 200 before the pod receives traffic. Backend
  readiness checks DB connectivity and migration status.
- **Pod Disruption Budget:** Prevent all replicas from being terminated
  simultaneously:
  ```yaml
  apiVersion: policy/v1
  kind: PodDisruptionBudget
  metadata:
    name: modulo-backend-pdb
  spec:
    minAvailable: 1
    selector:
      matchLabels:
  ...
  ```
- **WebSocket connections:** In-flight agent runs are lost on pod termination.
  Use the RuntimeProvider's reconnect mechanism or design pipelines to be
  idempotent at the step level.
...
- [ ] cert-manager configured with Let's Encrypt production issuer (K8s)
...
- [ ] Rolling update strategy configured (maxUnavailable <= 25%)
- [ ] PodDisruptionBudget applied (minAvailable >= 1)
- [ ] Rollback tested in staging before production
- [ ] At least 2 backend replicas running (zero-downtime requirement)
