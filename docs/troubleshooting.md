...
| `capacity_timeout` | A capacity-blocked run (org or pipeline cap) sat `pending` past its TTL without a slot freeing. | The stale-run sweep terminal-fails it after the TTL once its per-run capacity-retry budget is exhausted (`SAQ_CAPACITY_RETRY_BUDGET`, default 3, compared against `claim_count` — which counts ALL claims, not just capacity cycles; FAR-705). Within budget AND showing a claim heartbeat within the TTL window, the run stays `pending` and the background re-dispatch machinery (60s `dispatcher_reconcile` capacity branch) retries it when a slot frees. A backstop terminal-fails the run regardless of the budget when it shows no claim heartbeat at all (never claimed — the dominant capacity path is dispatch-time deferral WITHOUT a claim, so `claim_count` stays 0 and the budget gate can never fire for it) or its last claim heartbeat is itself older than the TTL. The backstop also covers a budget set at/above `SAQ_RUN_CLAIM_CAP` (20), which makes the budget gate unsatisfiable. | Raise the relevant concurrency cap (`max_concurrent_runs`, `sandbox_concurrency_limit`, `run_concurrency_limit`) or wait for active runs to complete. A terminal fail means the run waited out the TTL past its budget (or was never claimed, or its claim heartbeat went stale past the TTL) – sustained saturation, not a transient blip. |
