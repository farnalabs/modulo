...
| `capacity_timeout` | A capacity-blocked run (org or pipeline cap) sat `pending` past its TTL without a slot freeing. | The stale-run sweep terminal-fails it after the TTL — but only once the run's per-run capacity-retry budget is exhausted (`SAQ_CAPACITY_RETRY_BUDGET`, default 3, counted via `claim_count`; FAR-705). Within budget the run stays `pending` and the background re-dispatch machinery (60s `dispatcher_reconcile` capacity branch) retries it when a slot frees; each claim/demote cycle consumes one budget unit. | Raise the relevant concurrency cap (`max_concurrent_runs`, `sandbox_concurrency_limit`, `run_concurrency_limit`) or wait for active runs to complete. A terminal fail means the run waited out the TTL on more attempts than the budget allows – sustained saturation, not a transient blip. |
