...
| Retry enabled but a failed run was never retried | The stored `retry_policy` has an explicit empty `on: []` list (no retry – the FAR-525 editor's no-op save shape), or `max_retries` is 0, or the failing outcome's event is not in a granular `on` list. Absent `on` (key missing or null) with a budget > 0 means ALL retryable events; an explicit list is granular; an explicit empty list means no retry | Inspect `pipelines.retry_policy`: remove the `on: []` key (or set a real event list / a budget >= 1). In the pipeline editor, the retry panel shows the coverage mode and warns when nothing can retry | No log pattern – check the stored policy and the run's terminal `error_code` against the `on` list |
...
|---|---|---|---|
| `executor_stalled` | A claimed SAQ run dispatched **no node** within the setup grace (the `execute_run` zombie watchdog at `SAQ_SETUP_GRACE_SECONDS`), or `dispatcher_reconcile` found a claimed-but-nodeless zombie (fresh heartbeat, zero LangGraph checkpoints after `SAQ_CLAIMED_NODELESS_MINUTES`). | Three paths behave differently. The setup-grace zombie watchdog consults the pipeline's `retry_policy` first (FAR-693): "stall" in `on` (or an absent-`on` all-events policy) with budget remaining → the run is re-dispatched via the SAME mechanism the run-level retry uses (fenced pending-reset + SAQ job retry), bounded by `max_retries` (the budget counter is `max(node_attempt_count, claim_count - 1)` - node attempts AND prior SAQ claims - so a PRE-node hang that never started a node is bounded too, not just mid-graph hangs); no/insufficient coverage → terminal-fail as before. The `dispatcher_reconcile` nodeless path re-dispatches first (safe – zero nodes executed, so nothing can double-execute): bounded by the pipeline's `retry_policy` ("stall" in `on` → `max_retries`; a non-empty policy without "stall" → terminal-fail; no/empty policy → `SAQ_NODELESS_REDISPATCH_BUDGET`, default 4 since FAR-812), throttled to at most one re-dispatch per `SAQ_CLAIMED_NODELESS_MINUTES` window per run, then terminal-failed once the budget is exhausted. Each reconcile terminal-fail bumps the `claimed_but_never_dispatched` healthz counter (FAR-714) and ingests a `source='saq'` error event, so bursts are alertable without grepping run error_details. Resume runs (`resume_run` job) keep the unconditional terminal fail on the setup-grace path. | Investigate the worker machine that claimed the run (wedged worker? pre-node hang in checkpointer/graph compile/connector init?). Check `saq:runs:stats` heartbeats for that host. If the run terminal-failed, its policy did not cover the stall or the budget was gone; a re-dispatched run retries automatically. |
| `node_deadline_exceeded` | A node dispatched but did not **complete** within its configured `timeout_seconds` – the FAR-369 absolute node-deadline watchdog killed the run (independent of idle/activity; catches half-alive SSE stalls the idle-watchdog misses). | The watchdog consults the pipeline's `retry_policy` (FAR-690): "timeout" in `on` (or an absent-`on` all-events policy) with budget remaining → the run is re-dispatched via the SAME mechanism the run-level retry uses (fenced pending-reset + SAQ job retry), bounded by `max_retries` (the budget counter is `max(node_attempt_count, claim_count - 1)` - node attempts AND prior SAQ claims - so the bound holds for every hang site, including ones that never started a node); no/insufficient coverage, a correction run, a non-idempotent graph, or a live script-mode lease → terminal-fail as before. Resume runs keep the unconditional terminal fail. | For chronic deadline deaths, raise the node's `timeout_seconds` on the pipeline; a terminal-failed run means the policy did not cover "timeout" or the budget was exhausted. |
...
| `capacity_timeout` | A capacity-blocked run (org or pipeline cap) sat `pending` past its TTL without a slot freeing. | The stale-run sweep terminal-fails it at the TTL. A NEVER-claimed run (`claim_count` 0, the dominant capacity path is dispatch-time deferral WITHOUT a claim) terminal-fails at the TTL regardless of the budget: the stranded-refresh loop restamps its heartbeat while the run is inside the TTL window, so a heartbeat-age test alone would wait a further full TTL (~2x the documented TTL; FAR-705). A CLAIMED run gets the per-run capacity-retry budget grace (`SAQ_CAPACITY_RETRY_BUDGET`, default 3, compared against `claim_count`, which counts ALL claims, not just capacity cycles): within budget AND showing a claim heartbeat within the TTL window, it stays `pending` and the background re-dispatch machinery (60s `dispatcher_reconcile` capacity branch) retries it when a slot frees. A claimed run is terminal-failed regardless of the budget once `claim_count` exceeds the budget or its last claim heartbeat is itself older than the TTL: the heartbeat backstop also covers a budget set at/above `SAQ_RUN_CLAIM_CAP` (20), which makes the budget gate unsatisfiable. | Raise the relevant concurrency cap (`max_concurrent_runs`, `sandbox_concurrency_limit`, `run_concurrency_limit`) or wait for active runs to complete. A terminal fail means the run waited out the TTL (never claimed), or waited out the TTL past its budget / with a claim heartbeat stale past the TTL (claimed) – sustained saturation, not a transient blip. |
...
Monitoring surfaces for all of the above: `/healthz/ready` exposes the `dispatcher_reconcile` counters (`claim_cap_terminalized`, `age_terminalized`, `nodeless_failed`, `claimed_but_never_dispatched`, `dispatch_failed_terminalized`, `enqueue_failed_*`, ...) and the `stale_run_recovery` advisory check; when telemetry is enabled the OTel metrics `runs_running_count`, `runs_oldest_running_age_seconds`, `runs_stall_reason_total`, and `runs_claim_count_total` are updated every reconcile tick. A non-zero `claimed_but_never_dispatched` (FAR-714) means the zombie repair terminal-failed runs that SAQ claimed but which never dispatched a node: a burst is an alert-grade signal of degraded workers/sandboxes, not queue saturation.
