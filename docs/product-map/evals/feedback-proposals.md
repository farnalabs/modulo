...
depends-on: [task-nv2-eval-definition, task-nv4-feedback-routing]
status: partial
---
...
- [ ] System runs pipeline eval suite against rejected output as standalone evaluation (EvalEngine.standalone_evaluate) — Stub: `detect_eval_gap()` always returns False
- [ ] FeedbackRecord is tagged `eval_gap=True` when no eval scored the output as failing — Stub: never actually set
...
- **`detect_eval_gap()` is a stub** — always returns `False`. Eval gap detection cannot function until this is implemented with real `EvalEngine.standalone_evaluate()` calls.
