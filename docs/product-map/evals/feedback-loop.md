...
- 2026-08-15 (distribute partial-model-backends, round 3): Audit pass. All remaining unchecked behaviours are genuine cross-cutting gaps in the feedback loop (unconsumed pipeline-level `default_feedback_handler`, unimplemented gate-level `feedback_handler`, `reject_routing_conflict` validation, `feedback_handler`-supersedes-`reject_target`, correction-proposal accept/reject actions, correction-run-through-full-eval-suite not end-to-end verified, eval-proposal agent drafting new cases, proposals queue UI, human publish of proposed evals, library contribution v2, `ConcurrentModificationError` auto-retry, advisory lock, sequential `detect_eval_gap` without timeout/circuit-breaker, per-call RLS round-trip, `producing_node_id`/empty-`rejected_output` validation, deleted-run/deleted-pipeline handling, 255-char `gate_id` model validation). None are affected by this session's model-backend deletion-protection change; they require `feedback.py`/feedback-manager changes beyond this delivery's allowlist. No tests deleted or disabled. Status: partial.
