...
- [ ] `EvalSuiteBlockedError` dead code: the `for sr in suite_results: if not sr.passed:` loop at executor.py:557 is effectively unreachable for non-passing results because `_check_eval_suites` raises `EvalSuiteBlockedError` before returning. The loop only iterates when all suites pass or no suites exist — in both cases `if not sr.passed` is never True. One path should be cleaned up.
- [ ] `replace_pipeline_graph_endpoint` in pipelines.py does not catch `ProgrammingError` — if the pipeline_edges table doesn't exist (migrations not yet run), a 500 error is returned instead of 501.
