...
- 2026-08-15: Coverage completion (FAR-231/FAR-233 distribute batch). Re-verified all behaviour checkboxes are accurate. Confirmed the three remaining Known Gaps: (1) PRD 8.17 specifies the HITL gate `condition` as `{eval_id, threshold, operator}` while the code implements `eval_condition` with `{eval_name, ...}` — a documented schema difference, functionally equivalent; (2) no end-to-end integration test for the full eval-before-interrupt → suite check → eval_failed chain; (3) no timeout plumbed through to eval evaluation inside the HITL gate (the node-level timeout does not bound `EvalEngine.evaluate`). Added two unit tests for the failure-semantics edge cases (JMESPath condition syntax error → fail-closed ValueError; eval_condition referencing a nonexistent eval_name → graceful skip), closing the eval-system.md items those cover. Status: partial.
