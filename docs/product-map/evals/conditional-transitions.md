...
- [ ] `EvalSuiteBlockedError` class defined but not raised in code — post-run suite check uses direct status string
...
- [ ] `EvalSuiteBlockedError` class is defined in `eval_engine/__init__.py` but never raised — the post-run suite check in executor.py sets `error_code = "eval_suite_blocked"` manually instead.
