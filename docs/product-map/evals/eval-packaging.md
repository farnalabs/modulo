...
- [x] Migration adds suite_id and pass_threshold columns (0023_eval_suite_threshold) ### Suite-Level Pass Threshold
...
- [x] threshold = 1 — only perfect scores pass ### Post-Run Suite Enforcement
...
- [x] SuiteEvalResult exposes suite_id, aggregate_score, passed, blocking_failures ### Suite-Level OKR Tracking
...
- [x] Admin endpoint: GET /evals/okr-progress/{suite_id} ### Edge Cases
...
- [ ] Reactor: suite_id on eval is just a string — no FK, no orphan protection ### BDD
- [ ] eval_suite_crud.feature — placeholder only (scenarios not implemented)
- [ ] eval_run.feature — "Eval run below threshold fails" scenario tests suite threshold ## Known Gaps
