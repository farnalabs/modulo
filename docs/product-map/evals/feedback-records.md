...
status: partial
---
# Feedback Records Discovered from 1 completed delivery tasks. ## Behaviours ### Record Creation & Retrieval
...
- [ ] Empty list returns total=0 with empty items array ### Status Transitions
...
- [ ] API updates status via PATCH /feedback/{id}/status ### Handler Types & Auto-Correction
...
- [ ] Validation error when both reject_target and feedback_handler set on same gate (reject_routing_conflict) ### Correction Run Mechanics
...
- [ ] link_correction_run raises ValueError if record not in a state allowing "correcting" transition ### Post-Correction Evaluation
...
- [ ] run_post_correction_eval raises ValueError if correction run not found in DB ### Eval Gap Detection
...
- [x] detect_eval_gap returns 404 if record not found ### Feedback Inbox & Review
...
- [ ] API serialises all FeedbackRecord fields in responses ### Integration / Persistence
...
- [ ] Records created with different handler types persist correctly ## Known Gaps
