...
bdd: backend/tests/bdd/features/pipelines/run_variants.feature
code:
...
- [ ] Feedback system: `detect_eval_gap()` flags when human rejection is not caught by existing evals
...
- Feedback system `detect_eval_gap()` is a stub — real eval suite population not connected
