...
- [x] Store integration wired in `planStore` and `dashboard` — `planStore.ts` registers `team`/`license`/`plan` handlers, `dashboard.ts` registers `run`/`pipeline`; other stores (agents, schemas, connectors, triggers, model_backends, evals, feedback, library) still need `registerHandler()` calls (see Known Gaps)
...
- **2026-08-15**: Coverage sweep (partial-small-b). Verified store-integration wiring: `planStore.ts:180-182` calls `registerHandler("team"/"license"/"plan")` and `dashboard.ts:303-304` calls `registerHandler("run"/"pipeline")`; `NotificationBell.vue`/`DashboardNotificationsPanel.vue` register `notification`. `dispatchToStore` routing covered by `useEventStream.test.ts` (`dispatchToStore` describe block) and `syncRegistry.test.ts`. Marked the store-integration behaviour `[ ]`→`[x]`; the remaining-store gap stays in Known Gaps. 83/83 behaviours covered.
