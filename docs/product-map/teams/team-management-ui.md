...
status: partial
---
# Team Management UI Discovered from 1 completed delivery tasks. ## Behaviours ### Team list
...
- [x] Empty state shown when no teams exist ### Create team
...
- [x] Create form shows error feedback ### Rename team
...
- [ ] Rename collisions (existing name) not tested in unit tests ### Delete team
...
- [ ] Team deletion writes `team_deleted` to AuditEvent ### Members list
...
- [x] Members error state shown with retry button ### Add member
...
- [ ] Email-based invitation flow per PRD v1 spec (direct add implemented instead) ### Change member role
...
- [ ] No guard against removing the last admin/operator from a team ### Remove member
...
- [x] Admin deactivation flow removes all team memberships ### RBAC and access control
...
- [ ] "My Teams" section in user profile panel ### Notification endpoints
- [ ] Team notification endpoint config in UI (PRD requirement)
- [ ] Notification endpoints stored on team entity ### Team-scoped visibility (cross-feature)
...
- [ ] SSO group-to-team mapping via claims (JIT provisioning) ### Concurrency and edge cases
...
- [ ] Self-removal from team not tested ## Known Gaps
