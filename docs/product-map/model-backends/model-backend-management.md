...
- [x] `health_check` test-inference call on save — create/update run the provider health check and persist `last_health_check_at` / `last_health_check_error` on the entity (non-blocking; the graph validator surfaces failures as `MODEL_BACKEND_UNHEALTHY`)
...
- [ ] **No health check API route**: no standalone `/api/v1/model-backends/{id}/health` endpoint for manual revalidation (health is checked on save and by the graph validator)
...
- [ ] **Hard delete not blocked by non-terminal run snapshots**: delete protection covers agent FK (`ON DELETE RESTRICT`) and fallback references, but a `PipelineSnapshot` whose `model_backend_pins_json` references a backend with a non-terminal run is not checked
