...
- 2026-08-15 (distribute partial-model-backends, round 3): Re-audit pass on the hub entry alongside the model-backend management deletion-protection delivery. Confirmed every unchecked behaviour remains a genuine cross-cutting gap, not “implemented but untested” — org-scoped cache with TTL/invalidation (hub is run-scoped only, cleared in `__aexit__`), typed health-check error codes, 5-minute health-check staleness bound, retry-with-backoff, mid-run monitoring, concurrent-unregistration during `get_with_rotation()` scan (accepted not-thread-safe), and the pinned-`model_id`-at-runtime PRD §8.1 deviation. No code change was possible for the hub within this delivery's allowlist (these require changes to `pipeline_engine`/executor, `graph_validator`, `core/pricing`, or the cost controller); no tests deleted or disabled. Status: partial.

- 2026-08-15 (distribute partial-model-backends, round 1): Coverage drive on the two model-backend entries. **Hub**: verified + checked off 2 edge-case behaviours — primary-in-own-fallback-list skipped (`test_get_skips_primary_in_own_fallback_list` + only-fallback variant raising `BackendUnavailableError`) and scan-all returning unrelated backends (`test_get_with_rotation_scan_returns_unrelated_backend`, distinct provider/model backend_ids). **CORRECTED 3 stale `[x]` claims** under Runtime Resolution: the pinned `model_id` is stored and validated but never applied at runtime — the run-scoped hub builds backends from the CURRENT entity row, so paused→resumed runs pick up operator `model_id`/provider changes and re-runs from older snapshots use the current entity (documented as a PRD §8.1 deviation in Known Gaps). Documented that `core/pricing.get_pricing()` exists but is unwired (cost not computed against pinned `model_id`). Status: partial.

- 2026-08-15 (distribute partial-model-backends, round 2): Coverage-drive pass on the model-backend entries. **Hub**: re-audited every unchecked behaviour against `ModelBackendHub` (`core/model_backend_hub/__init__.py`) and the hub unit/BDD suites. Confirmed ALL remaining unchecked behaviours are genuine cross-cutting gaps, not "implemented but untested" — each already has a Known Gaps note: org-scoped caching with TTL/invalidation (hub is run-scoped only, `__aexit__` clears), typed health-check error codes (`credential_expired`/`endpoint_unreachable`/`quota_exceeded`/`model_not_found`), 5-minute health-check staleness bound, retry-with-backoff, mid-run monitoring, concurrent-unregistration during `get_with_rotation()` scan (accepted not-thread-safe design), and the pinned-`model_id`-at-runtime PRD §8.1 deviation (run-scoped hub builds from the current entity row; resume re-inits from current rows). No code changes were possible within this delivery's allowlist (these require changes to `pipeline_engine`/executor, `graph_validator`, `core/pricing`, cost controller, or DB layers); no tests deleted or disabled. Status: partial.
