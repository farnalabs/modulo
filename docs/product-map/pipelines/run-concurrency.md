...
## QA History
- **2026-08-15 (coverage sweep partial-small-b): Verified the 4 unchecked items (per-connector advisory write lock, `waiting_for_lock` sub-state, cancel-from-`waiting_for_lock`, cancellation-while-waiting) remain genuine gaps — the `waiting_for_lock` sub-state was excised in migration 0074/0075, and the connector-level advisory locks in `core/connector_hub/locking.py` are not wired into run admission. 16/20 behaviours covered.**
