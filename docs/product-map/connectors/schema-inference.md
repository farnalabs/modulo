...
- [x] Reads are limited to a configurable sample size (PRD §8.16 default: 200 — API default `limit=200`, max 200, `_SAMPLE_LIMIT` in ConnectorHub raised to 200; the LLM prompt carries up to 200 sanitised records)
...
- [x] Issue-tracker connectors (Jira, Linear) propose fields from issue metadata: summary, description, priority, status, assignee, labels
- [x] Git-host connectors (GitHub, GitLab) propose fields from repository/PR metadata: repo name, branch, PR title, PR description, file paths
- [x] CI-runner connectors propose fields from pipeline/job metadata: pipeline ID, status, branch, commit SHA, duration
- [x] Chat connectors (Slack) propose fields derived from message/channel metadata
- [x] Generic connectors with no specialised schema return a minimal field set (id, name, type)

The connector-type-aware guidance lives in `schema_registry/inference.py` (`_CONNECTOR_TYPE_CATEGORY` maps 25+ connector type ids to a category; `connector_category()` falls back to `generic` for unlisted types). The infer route passes `ci.connector_type_id` into `SchemaInferenceService`, which appends the category's field-extraction guidance to the system prompt. The route's `supported_inference_types` set now admits every connector type that has a category entry and a `query()` implementation. Covered by 9 unit tests in `test_schema_inference.py` (one per category + generic fallback + case-insensitivity + `connector_category()` matrix) plus an endpoint test asserting `connector_type` reaches the service.
...
- [x] ~~**Sampled record default (200)**: PRD says default 200 records, code caps at 50 in prompt builder, API default limit is 10, max is 100~~ **RESOLVED 2026-08-15**: API default limit is 200, max 200; `ConnectorHub._SAMPLE_LIMIT` is 200; the prompt builder default is 200 sanitised records
...
- [x] ~~**Connector-type-aware field extraction not implemented**: all connector types use the same generic prompt~~ **RESOLVED 2026-08-15**: `inference.py` maps connector type ids to extraction categories (issue-tracker, git-host, ci-runner, chat, document-store, generic) and appends category-specific field guidance to the system prompt; the route passes `connector_type_id` into the service; `supported_inference_types` expanded to all 25+ categorised connector types
- [x] ~~**No dedicated BDD feature file for connector-type-aware inference (only generic schema_inference.feature exists)**~~ **RESOLVED 2026-08-15**: the connector-type-aware guidance is unit-tested (9 prompt/guidance tests in `test_schema_inference.py` + a route test asserting `connector_type` reaches the service); the BDD schema_inference.feature remains generic by design
...
- 2026-08-15: connector-type-aware extraction + 200-record sample default (this Worker). (1) Implemented connector-type-aware field-extraction guidance in `schema_registry/inference.py`: `_CONNECTOR_TYPE_CATEGORY` maps 25+ connector type ids (jira/linear/trello/asana/monday/shortcut/youtrack → issue-tracker; github/gitlab/bitbucket/gitea/azure_repos → git-host; ci-runner/circleci/buildkite/jenkins/teamcity/azure_pipelines → ci-runner; slack/discord/microsoft_teams → chat; notion/confluence/sharepoint/dropbox_paper → document-store; unlisted → generic minimal id/name/type). The infer route passes `ci.connector_type_id` into `SchemaInferenceService`, which appends the category guidance to the system prompt. `supported_inference_types` in the route expanded to all categorised connector types that implement `query()`. (2) Raised the sample-size default to match PRD §8.16's 200: `SchemaSampleQuery.limit` default 200 / max 200, `ConnectorHub._SAMPLE_LIMIT` 100→200, prompt-builder default `_MAX_SAMPLE_RECORDS` 50→200. (3) Fixed in place a pre-existing BDD fixture gap in `test_schema_inference.py` (mock schema objects lacked `folder_id`/`account_id`, breaking the publish-inferred-schema scenario with a 500). Tests added: 9 prompt/guidance unit tests (category coverage + generic fallback + case-insensitivity + `connector_category()` matrix), `test_infer_schema_passes_connector_type_to_service`, `test_infer_schema_default_limit_is_200`; BDD default-limit step/feature updated 10→200. Verified: 80 unit + 13 BDD schema-inference/connector tests, otel_bridge + observability + environments + error_handling packages, integration env-profile CRUD — all pass. Status: partial (genuine gaps: registry draft state, field-level editor, rare-field exclusion, abstract_name, concurrency guard, feature flag, 504 mapping, sandboxed-environment).
