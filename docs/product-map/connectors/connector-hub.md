...
bdd:
  ...
  - backend/tests/bdd/features/connectors/monday.feature
  - backend/tests/bdd/features/connectors/trello.feature
  - backend/tests/bdd/features/connectors/asana.feature
  - backend/tests/bdd/features/connectors/shortcut.feature
  - backend/tests/bdd/features/connectors/datadog.feature
  - backend/tests/bdd/features/connectors/filesystem.feature
unit-tests:
  - backend/tests/unit/connector_hub/test_connector_hub.py
  - backend/tests/unit/connector_hub/test_connector_hub_e2e.py
...
- [x] Multiple `ConnectorHub` instances can coexist in the same process (one per concurrent run) — per-instance registries + per-instance `asyncio.Lock`; covered by `test_multiple_hubs_coexist` and `test_multiple_hubs_concurrent_initialise`
...
- [x] `type_id = "asana"` → `AsanaConnector(personal_access_token=creds.personal_access_token)`; BDD feature file exists (asana.feature: 10 scenarios); ConnectorType.ASANA; enum entry exists; unit tests exist (test_asana_connector.py)
- [x] `type_id = "monday"` → `MondayConnector(api_key=creds.api_key)`; BDD feature file exists (monday.feature: 13 scenarios); ConnectorType.MONDAY; enum entry exists; unit tests exist (test_monday_connector.py)
- [x] `type_id = "youtrack"` → `YouTrackConnector(token=creds.token, base_url=config.base_url)` — `base_url` required (`ValueError` if missing); BDD feature file exists (youtrack_connector.feature: issues, issue, projects); ConnectorType.YOUTRACK; enum entry exists; unit tests exist (test_youtrack_connector.py); list endpoints (`issues`, `projects`, `users`) guard against corrupt/hostile non-list bodies via `_safe_top_level_records` (empty page instead of crash); list endpoints (`issues`, `projects`, `users`) guard against corrupt/hostile non-list bodies via `_safe_top_level_records` (empty page instead of crash)
...
- [x] `type_id = "datadog"` → `DatadogConnector(api_key=creds.api_key, app_key=creds.app_key, site=config.site)` — `api_key` + `app_key` required; site defaults to `"us"`; BDD feature file exists (datadog.feature: 10 scenarios); ConnectorType.DATADOG; enum entry exists; unit tests exist (test_datadog_connector.py)
...
- [x] `type_id = "filesystem"` → `FilesystemConnector(base_path=config.base_path)` — `base_path` is required; `ValueError` if missing; BDD feature file exists (filesystem.feature: 5 scenarios, wired via test_alpha_connectors.py); ConnectorType.FILESYSTEM; enum entry exists
...
- **No BDD for 16 connector types** that have feature files but are not wired to step definitions (azure_key_vault, azure_pipelines, azure_repos, buildkite, circleci, discord, dropbox_paper, grafana, jenkins, microsoft_teams, opsgenie_connector, pagerduty, sentry, sharepoint, swappable_binding, teamcity_connector). Feature files exist but are not testable — production-mocking gap. (codeclimate, confluence, gitea, n8n, notion, npm, onepassword, pypi, snyk, sonarqube, trivy, youtrack are wired via test_connectors.py / dedicated step modules.)
- **No BDD at all** for: bitbucket, shell, and the CI runner connectors (github_actions_ci, gitlab_ci).
...
- [x] ~~**No end-to-end integration test**: `ConnectorHub.initialise()` → connector method → OTel span → credential cleanup~~ — Added `test_connector_hub_e2e.py` (4 tests) walking the full lifecycle: real Fernet decryption through the secrets backend → connector built with the decrypted credential → `health_check`/`query`/`write` on a real filesystem connector → OTel spans verified via an InMemorySpanExporter (span names, `connector.type`/`connector.operation`/`connector.org_id`/`connector.healthy`/`connector.result_total` attributes, ERROR spans with `connector.error_type` + exception event, and no credential/payload leakage in any span attribute) → `close()`/`__aexit__` releases every credential-bearing connector (registry empty, `get()`/`acl()` raise `ConnectorNotFoundError`, `_initialised` reset, re-initialise works). Also added a named `ConnectorHub.close()` lifecycle method (`__aexit__` delegates to it) so shutdown is callable without the async context manager and is idempotent.
...
### QA History
- 2026-08-17: improve-architecture: **HARDENED YouTrack list parsing against corrupt payloads** (`connectors/youtrack/__init__.py`). The three list endpoints — `query("issues")` (`GET /issues`), `query("projects")` (`GET /admin/projects`), and `query("users")` (`GET /users`) — read the response body straight into `ConnectorResult.records` as a top-level array, so a corrupt/hostile non-list body (object, string, number, `null`) would crash downstream list iteration with a `TypeError`/`ValueError` instead of degrading to an empty page. Added `_safe_top_level_records()` helper that treats only a real list (of dict items) as records and coerces anything else to an empty page, applied to all three endpoints (completing the corrosive-payload hardening lens; YouTrack was the last array-returning list connector not yet guarded). **Tests:** 5 new unit tests in `test_youtrack_connector.py` (issues/projects/users non-list bodies → empty records + total 0; mixed-array items (str/int/null) filtered out, valid dicts preserved). 28/28 youtrack unit tests pass, ruff lint + format clean. Status: partial (youtrack BDD feature file remains unwired).
- 2026-08-16: improve-architecture: **HARDENED PagerDuty list parsing against corrupt payloads** (`connectors/pagerduty/__init__.py`). The 7 list endpoints (`incidents`, `services`, `teams`, `users`, `escalation_policies`, `schedules`, `oncalls`) read records straight off `body.get(key, [])` — a corrupt or hostile non-dict body (list, string, number) crashed with `AttributeError`, and a non-list page field returned a bare string as the records list. Each endpoint now degrades to an empty page via the shared `_safe_records` helper (completing the connector-hardening lens: Azure Repos/Pipelines/Key Vault, Bitbucket, Teams, SharePoint, TeamCity, CircleCI, n8n, Opsgenie, Datadog). The adjacent paging reads are guarded the same way — new `_body_field(body, key, default)` guards offset/`more` extraction, and `_paging_total` now treats a non-dict body as absent. **Tests:** 8 corrupt-body endpoint tests (non-dict body + non-list page field across all 7 endpoints) + `_body_field` matrix + non-dict `_paging_total` cases in `test_pagerduty.py`. 89/89 pagerduty + safe_page unit tests pass, ruff check + format clean, mypy --strict clean. Status: partial (16 unwired BDD feature files; shell connector partial-init; broad `except Exception` in `initialise()`).
- 2026-08-14 (improve-tests): QA lens pass on the shared `safe_int` coercion helper (`connectors/_safe_int.py`, used by Jira, Slack, Opsgenie, Snyk, SonarQube and the dashboard route). Previously the helper had **zero direct test coverage** — it was only exercised indirectly through per-connector resilience suites, so its contract could drift (and did). Added a dedicated 56-test unit suite (`tests/unit/connectors/test_safe_int.py`) locking the full bool/non-finite/unparseable matrix: booleans and non-numeric types → default; `inf`/`-inf`/`nan` floats and `Infinity`/`NaN` Decimals → default; int passthrough; float/Decimal truncation toward zero; numeric-string parsing (whitespace, `+`/`-`, underscores); bytes/bytearray parsing; and rejection of `1.5`/`1e3`/`0x2a`/`inf`/empty/`12,5` strings. The new suite immediately exposed a **real latent crash**: `Decimal("sNaN")` (signaling NaN) makes `math.isfinite()` raise `ValueError`, escaping the non-finite guard and crashing the caller — exactly what the helper exists to prevent. Fixed `safe_int` to treat signaling NaNs as non-finite (fall back to the default) instead of propagating. All 56 new tests + the 8 existing indirect `safe_int` tests in `test_jira_resilience.py`/`test_slack.py` pass; ruff check + format clean; the 21 QA-lens architecture tests still pass.
- 2026-08-14: improve-architecture (product-map walk): **HARDENED connector pagination against corrupt/non-finite values** (Opsgenie, Snyk, Datadog) — completing the cross-connector QA lens begun with Jira (#1246), Slack (#1251), and SonarQube (#1254). Python's json parser produces `inf` for overflowing literals like `1e999`, so a corrupt/hostile response must not be able to crash or poison pagination. **(1) Opsgenie `_query_alerts`** (`connectors/opsgenie/__init__.py`): `next_cursor` was computed as `int(params.get("offset", 0)) + len(records)` whenever `paging` was truthy — a garbage/bool/non-finite offset crashed with `ValueError`/`OverflowError`, and a non-dict `paging` block (or a final page whose `paging` carries no `next` key) still emitted a cursor, risking an unbounded pagination loop. New `_next_offset_cursor(offset, records, paging)` emits a cursor only when `paging` is a dict advertising `next` and coerces the offset through the shared `safe_int`; new `_paging_total_count(body)` sanitises `totalCount` (non-finite/bool/garbage → 0, absent → `None`) and is wired into all 6 list endpoints (`alerts`, `alert_notes`, `alert_logs`, `teams`, `schedules`, `escalations`). **(2) Snyk list endpoints** (`projects`/`issues`/`orgs`/`tests`): `total=body.get("meta", {}).get("count", len(data))` crashed with `AttributeError` on a non-dict `meta` and passed a non-finite/garbage `count` straight through. New `_meta_total(body, fallback)` treats non-dict `meta` as absent and coerces `count` through `safe_int`. **(3) Datadog `_search_logs`**: `body.get("meta", {}).get("page", {}).get("after")` crashed with `AttributeError` on non-dict `meta`/`page`. Now each block is type-guarded and only a string `after` is emitted as `next_cursor`. **Tests:** 18 new unit tests (`test_opsgenie.py` ×9 incl. direct coverage of both helpers, `test_snyk.py` ×5 incl. `_meta_total` matrix, `test_datadog.py` ×4) proving `totalCount: 1e999`, `meta.count: 1e999`, garbage cursors/offsets, bool totals, and non-dict `paging`/`meta`/`page` blocks fall back gracefully instead of crashing or looping. 133/133 direct connector tests + 75/75 `test_opsgenie_connector.py`/`test_snyk_connector.py`/`test_datadog_connector.py` hub tests pass, ruff check + format clean, mypy --strict clean (3 connector modules).
- 2026-08-11: improve-architecture: **Wired 6 previously-undocumented connector BDD feature files into the product map.** Verified `backend/tests/bdd/steps/test_connectors.py` registers `scenarios()` for `monday.feature` (13), `trello.feature` (9), `asana.feature` (10), `shortcut.feature` (10), `datadog.feature` (10), and `test_alpha_connectors.py` registers `filesystem.feature` (5). Updated the `bdd:` frontmatter and fixed the per-type notes that wrongly claimed "no BDD" for shortcut/trello/asana/monday/datadog/filesystem. Also corrected the stale "Remaining Gaps" claim that 15+ feature files were unwired — re-audited step-definition wiring: 16 connector feature files remain unwired (azure_key_vault, azure_pipelines, azure_repos, buildkite, circleci, discord, dropbox_paper, grafana, jenkins, microsoft_teams, opsgenie_connector, pagerduty, sentry, sharepoint, swappable_binding, teamcity_connector); codeclimate, confluence, gitea, n8n, notion, npm, onepassword, pypi, snyk, sonarqube, trivy, youtrack are wired via test_connectors.py / dedicated step modules). Status: partial (16 unwired feature files; shell connector partial-init; broad `except Exception` in `initialise()`).
- 2026-08-11: improve-architecture: **RESOLVED the "No end-to-end integration test" known gap** (`core/connector_hub/__init__.py` + `test_connector_hub_e2e.py`). Added a named `ConnectorHub.close()` lifecycle method (idempotent, callable without the async context manager; `__aexit__` now delegates to it) so shutdown is an explicit, testable contract. Added 4 E2E integration tests walking the full lifecycle in one flow: real Fernet decryption through the secrets backend → connector built with the decrypted credential → `health_check`/`query`/`write` on a real filesystem connector → OTel spans verified via an InMemorySpanExporter (span names, `connector.type`/`connector.operation`/`connector.org_id`/`connector.healthy`/`connector.result_total` attributes, ERROR spans with `connector.error_type` + exception event, no credential/payload leakage in span attributes) → `close()`/`__aexit__` releases every credential-bearing connector (registry empty, `get()`/`acl()` raise `ConnectorNotFoundError`, `_initialised` reset, re-initialise works). Also verified the decrypted GitHub token reaches the built connector and that double-`close()` is a no-op. Updated product map `connectors/connector-hub.md` (unit-tests frontmatter + Known Gap → RESOLVED, Remaining Gaps line removed, QA History). 941/941 connector_hub unit tests pass (937 + 4 new), ruff lint + format clean, mypy --strict clean. Status: partial (BDD step definitions for 15+ feature files still unwired; shell connector partial-init; broad `except Exception` in `initialise()`).
