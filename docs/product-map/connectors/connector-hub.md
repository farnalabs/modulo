...
- **No BDD for 16 connector types** that have feature files but are not wired to step definitions (azure_key_vault, azure_pipelines, azure_repos, buildkite, circleci, discord, dropbox_paper, grafana, jenkins, microsoft_teams, opsgenie, pagerduty, sentry, sharepoint, swappable_binding, teamcity). Feature files exist but are not testable — production-mocking gap. (codeclimate, confluence, gitea, n8n, notion, npm, onepassword, pypi, snyk, sonarqube, trivy, youtrack are wired via test_connectors.py / dedicated step modules.)
