...
- **No BDD for 3 connector types** that have feature files but are not wired to step definitions (azure_pipelines, opsgenie, teamcity). Feature files exist but are not testable — production-mocking gap. (codeclimate, confluence, gitea, n8n, notion, npm, onepassword, pypi, snyk, sonarqube, trivy, youtrack were wired after this entry's last QA pass.)
- **No BDD at all** for: azure_repos, bitbucket, sharepoint, buildkite, circleci, jenkins, dropbox_paper, sentry, pagerduty, grafana, microsoft_teams, discord, azure_key_vault, shell, CI runner connectors (github_actions_ci, gitlab_ci).
