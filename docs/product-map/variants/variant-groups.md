...
- [ ] RLS org context set on every CRUD endpoint via `set_rls_org`
...
- [ ] Selection strategy constrained to `'weighted'` or `'single'` (DB CHECK constraint)
...
- [ ] 429 if no variant selected or quota exceeded in `run_variant_weighted`
...
- [ ] Check constraint enforces `selection_strategy IN ('weighted', 'single')`

### Pipeline limits
- [ ] Concurrent run limit enforced per pipeline per variant group via `check_pipeline_run_quota` (`active < max_concurrent_runs`)
...
- [ ] BDD feature file: placeholders only — no real Gherkin scenarios
