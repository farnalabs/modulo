...
depends-on: [task-nv2-eval-llm-judge, task-nv3-variant-run]
status: partial
---
...
- [ ] Variant group scoped to organisation (RLS enforced)
...
- [ ] Variants stored as JSON column on VariantGroup
...
- [ ] weighted selection dominates over low-weight variants (100:1 ratio heavily favours higher weight)
...
- [ ] Each run increments variant group run_count
...
- [ ] N variants creates N runs, each counted individually against pipeline limits
...
- [ ] Coverage gap endpoint accessible via API
...
- [ ] Prompt diff endpoint accessible via API
