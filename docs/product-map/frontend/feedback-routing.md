...
status: partial
---
# Feedback System — Routing ## Behaviours ### FeedbackRecord lifecycle
...
- [x] FeedbackRecord is immutable after creation ### Handler types and routing strategy
...
- [ ] Setting both `feedback_handler` and `reject_target` is a validation error (`reject_routing_conflict`) ### Correction run mechanics
...
- [x] `ai_correction_with_human_review` marks `needs_human_review=True` on eval pass ### AI correction agent
...
- [ ] Correction proposal injected as `feedback_correction` in run_context ### Eval suite growth
...
- [ ] Library contribution of curated evals (v2) ### Feedback inbox UI
...
- [x] Paginated results ### API endpoints
...
- [x] `GET /api/v1/feedback/proposals` — eval proposals queue ### Testing
...
- [ ] BDD: Feedback inbox review actions complete the lifecycle ## Known Gaps - **Correction run checkpoint seeding**: Correction run creates a fresh run rather than seeding from original LangGraph checkpoint at target_node_id (per PRD 8.20: "pre-seeded with checkpoint state")
