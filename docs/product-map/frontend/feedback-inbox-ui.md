...
depends-on: [task-nv4-feedback-record]
status: partial
---
...
- [ ] Displays pipeline name, rejection reason/summary, status badge, created date, handler_type per row
...
- [ ] Caches loaded detail per record (no re-fetch on collapse/re-expand)
...
- [ ] On failure: shows error message, button re-enabled
...
- [ ] On failure: shows red error message, buttons re-enabled

### Feedback review request schema
- [ ] Review POST body supports optional `annotation` field
- [ ] Review POST body supports optional `status` field (e.g. `resolved`, `correcting`)
...
- [ ] Feature file is a placeholder with no real scenarios (gap)
