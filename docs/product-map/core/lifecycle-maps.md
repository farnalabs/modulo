...
- [x] Version entries report the account that last saved them (`updated_by` — the map row IS the active version in v1, so the version entry's `created_by` reflects the last-saver; falls back to the original creator `account_id` for pre-stamping rows)
...
- [x] Legacy map content written before FAR-175 (dangling edges / duplicate stage/edge ids / transition cycles) — repaired by the one-off backfill script `backend/scripts/backfill_lifecycle_map_legacy_content.py` (dry-run default, `--apply` to write, exits 1 if any map is left unrepaired), so such maps become editable again including graduation; `clean_legacy_content` (`core/lifecycle_map/validation.py`) dedupes ids, drops dangling edges, and breaks cycles, and the script rewrites `content_json` and rebuilds the junction projection
...
- [x] Map CRUD operations are written to the org audit chain (`append_audit_event`, fail-open) — create/import, metadata update, delete, and stage graduation each record `lifecycle_map.*` events; the editor's version save (POST/PUT `/versions`) records the dedicated `lifecycle_map.version_saved` event, and restore (`POST /{id}/restore`) records the dedicated `lifecycle_map.restored` event, keyed by map id and actor
...
- Legacy maps blocked by FAR-175 strict validation are unblocked via the one-off backfill script `backend/scripts/backfill_lifecycle_map_legacy_content.py` (FAR-203). This is a deliberate one-off decision: `normalize_content` stays strict for NEW content (422 on dangling edges / duplicate ids / cycles), and only pre-existing legacy rows are repaired in place (dedupe ids, drop dangling edges, break cycles) so graduation is not permanently blocked. The script is not wired to any automation — an operator runs it once per environment (dry-run by default, `--apply` to write).

## QA History

- 2026-08-15 — FAR version-actor stamping: added `lifecycle_maps.updated_by` (nullable UUID, migration 0103) stamped on every version-producing write path (create, update, save version, update version, graduate, restore). The version entry's `created_by` (version list/GET, map detail `versions[]`) now reflects the account that last saved the current version, falling back to the original creator's `account_id` for pre-stamping rows — it was always `null` before. Added unit tests (create stamps account_id; save/update/graduate/restore stamp updated_by; serialiser reports updated_by and falls back to account_id; POST/PUT version routes pass `updated_by=principal.account_id`) + 1 BDD scenario.
...
- 2026-08-14 — FAR-203: audit follow-ups to FAR-175. (1) Version-save endpoints (POST/PUT `/versions`) now emit their own `lifecycle_map.version_saved` audit event instead of the generic `lifecycle_map.updated`; (2) restore (`POST /{id}/restore`) now emits a `lifecycle_map.restored` audit event (it was the only map mutation without one, fail-open, savepoint-nested like the others); (3) legacy dangling-edge/duplicate-id unblock decision: one-off backfill script `backend/scripts/backfill_lifecycle_map_legacy_content.py` repairs pre-FAR-175 content via the pure `clean_legacy_content` helper (dedupe stage/edge ids, drop dangling edges, greedily break cycles), rewriting `content_json` + rebuilding the `lifecycle_map_stages` junction projection (dry-run default, `--apply`, exits 1 if any map is left unrepaired). Unit tests: 15 `clean_legacy_content` cases + 3 real-DB script tests (apply repairs content+junction, dry-run writes nothing, uncleanable maps left untouched); updated the 2 version-save audit route tests to pin `lifecycle_map.version_saved` and added 2 restore-audit route tests; BDD: version-save audit, restore audit, and legacy-backfill scenarios.
