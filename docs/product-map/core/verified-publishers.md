...
- [ ] Admin deletes a publisher ### Registry verify / trust lookup
...
- [ ] Registry verify returns `publisher_status` (verified/community/revoked) from in-memory model ### Library primitive verified field
...
- [ ] Local library primitives always have `verified: null` (enforced by check constraint) ### Trust tier display (amber warning on copy)
...
- [ ] Copy-to-adapt of a green-tier primitive shows no warning, no extra confirmation ### Error states
...
- [ ] Registry verify with non-existent slug returns 404 ### Edge cases
...
- [ ] Built-in Modulo publisher is always `verified` in the in-memory model ## Known Gaps
