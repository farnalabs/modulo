...
- [x] Event timestamp set to UTC now ### Append-Only Enforcement (two layers) **Postgres trigger:**
...
- [x] `register_append_only_guard()` is idempotent ### Chain Verification
...
- [x] Respects max_events limit (default 10000) ### Event Listing — Cursor Pagination
...
- [x] Returns limit+1 items internally to detect has_more ### Event Export & Batch Detail
...
- [x] RLS-scoped: only caller's org events returned ### API Endpoints
...
- [x] All endpoints require auth and are RLS-scoped to caller's org ### Event Types (PRD 8.12 table — 18 documented event types)
...
- [x] Org migration → events stay in source org (no cross-org visibility, enforced by RLS) ### Error Handling
...
- [x] Missing event_type → model validation error before DB write (event_type is NOT nullable in DB — String(100), no default, but SQLAlchemy model has no explicit nullable=False) ### Security
...
- [x] payload_json is arbitrary JSON — sensitive data can be embedded (caller's responsibility) ### Backward Compatibility
...
- [x] AuditChainHead.last_event_id FK uses ON DELETE SET NULL — deleting an event (if trigger removed) doesn't break chain ## Known Gaps
