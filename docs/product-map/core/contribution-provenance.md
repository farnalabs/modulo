...
- [ ] Registry primitives have `ed25519_signature` not null; CHECK constraint enforces this ### Registry protocol (publish / pull / verify)
...
- [ ] `compute_popularity_score()` and `list_registry_primitives_ranked()` for search ranking ### Signature verification & trust tiers
...
- [ ] Warning text: "This primitive has not been verified by Modulo. Review the prompt template and schema before use." ### Copy-to-adapt flow
...
- [ ] Browser POST to `/api/v1/libraries/{id}/adapt` succeeds (201) and creates local copy ### Rating system
