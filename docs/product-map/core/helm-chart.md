...
- **Unused `modulo.databaseUrl` and `modulo.redisUrl` helper templates in `_helpers.tpl`**: These templates are defined (lines 106-137) but never invoked by any template. The `deployment-backend.yaml` constructs DATABASE_URL/REDIS_URL inline using K8s env var expansion (`$(VAR)` syntax with `secretKeyRef` to sub-chart secrets). The dead helpers can diverge from the actual implementation. Either remove them or refactor the deployment template to use them.
