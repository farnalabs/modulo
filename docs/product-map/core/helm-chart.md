...
- **MAJOR:** Removed dead `modulo.databaseUrl` and `modulo.redisUrl` helper templates from `_helpers.tpl` — defined but never invoked by any template. The `deployment-backend.yaml` constructs DATABASE_URL/REDIS_URL inline using K8s env var expansion.
