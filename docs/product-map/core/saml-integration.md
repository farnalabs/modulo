...
bdd: backend/tests/bdd/features/auth/sso_saml.feature
code:
...
- [x] AssertionConsumerServiceURL points to configured `MODULO_PUBLIC_URL + /api/v1/auth/saml/acs` ### ACS endpoint
...
- [x] Calls `apply_group_mappings` when groups attribute present and group mappings configured ### SP metadata endpoint
...
- [x] Metadata contains correct ACS Location URL ### Enterprise license gating
...
- [x] `/api/v1/auth/sso/providers` exposes `saml: true` only when license present + configured ### IdP metadata handling
...
- [x] Raises error when IDPSSODescriptor is missing from metadata ### SSO provider admin CRUD
...
- [x] Non-admin users get 403 on all admin SSO endpoints ### Connection testing
...
- [x] Test returns failure details on invalid XML or missing descriptor ### Group-to-team mapping
...
- [x] Duplicate membership is not re-added ### JIT provisioning edge cases
