...
- [x] `saml_metadata` route catches unhandled `Exception` → returns 500 (consistent with login/ACS patterns)
- [x] `saml_metadata` route returns 400 when `modulo_saml_enabled` is False (consistent with login/ACS)
...
- 2026-07-09: Cross-cutting QA (index 356). Fixed CRITICAL — `GET /api/v1/admin/sso/providers` called `get_provider(session, provider_id)` with undefined `provider_id` (NameError crash on every call). Fixed MAJOR — `saml_metadata` route now checks `modulo_saml_enabled` before serving XML (consistent with login/ACS). Fixed MAJOR — `saml_metadata` route now has Exception→500 catch (consistent with login/ACS). Confirmed all [x] entries are still accurate. Added 2 new error-handling checkboxes. Status: partial (14 known gaps + 3 unchecked edge cases + 4 unchecked resilience items).
...
- [x] BDD `sso_saml.feature` ACS scenarios fixed (2026-07-12) — `jit_provision_user` mock now returns `(Account, UUID, str)` tuple matching the function's return type; all 22 SAML unit + BDD step tests pass
