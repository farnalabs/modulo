...
- [ ] AssertionConsumerServiceURL points to configured `MODULO_PUBLIC_URL + /api/v1/auth/saml/acs`
...
- [ ] Calls `apply_group_mappings` when groups attribute present and group mappings configured
...
- [ ] Metadata contains correct ACS Location URL
...
- [ ] `/api/v1/auth/sso/providers` exposes `saml: true` only when license present + configured
...
- [ ] Raises error when IDPSSODescriptor is missing from metadata
...
- [ ] Test returns failure details on invalid XML or missing descriptor
