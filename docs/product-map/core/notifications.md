...
- PRD §8.11 specifies 5 consecutive failures within 24h triggers auto-disable; code uses 10 consecutive with no time window — documented gap (see Known Gaps)
...
- [x] Admin can create a team-scoped webhook by passing `team_id` — POST /webhooks stores the reference; response echoes `team_id`, and org admins can manage it on create/update
- [x] Admin can update an endpoint's team scope via `team_id` on PUT /webhooks/{id} — re-assigns the endpoint to another team; omitting `team_id` leaves the current scope untouched
- [x] Creating/updating a webhook with an unknown `team_id` (missing, soft-deleted, or other-org team) returns 422 — `_validate_team_exists` runs inside the RLS-scoped transaction, so a dangling team reference can no longer fall through to an FK `IntegrityError` (previously surfaced as 503)
- [x] `team_id` is echoed on WebhookResponse across list/get/create/update/re-enable — org admins can distinguish team-scoped endpoints from org-wide ones
...
- [x] Team notification endpoint configuration via admin API is v1 per PRD — admin create/update accept `team_id` (must reference a live org team, else 422) and responses echo it; the non-admin `/api/v1/notifications` CRUD exposes the same field
...
- [x] Multi-worker advisory lock for the claim-expiry job — `expire_stale_claims` in `core/hitl_manager/expiry_job.py` takes a per-org `pg_try_advisory_xact_lock` (`_EXPIRY_LOCK_KEY`) so the SAQ `claim_expiry` cron and the in-process loop never sweep the same org concurrently; covered by `test_claim_expiry_job.py`
...
- ~~Multi-worker advisory lock for expiry job not yet implemented~~ **RESOLVED (2026-08-15)**: `expire_stale_claims` (`core/hitl_manager/expiry_job.py`) acquires a per-org `pg_try_advisory_xact_lock` (`_EXPIRY_LOCK_KEY`), shared by the SAQ `claim_expiry` cron and the in-process loop — only one expiry writer per org at a time.
- ~~Team notification endpoint configuration (team_id field) not exposed in admin API create/update routes~~ — RESOLVED 2026-08-17: admin `POST /webhooks` / `PUT /webhooks/{id}` now accept `team_id` (RLS-scoped existence validation → 422 for unknown teams), `WebhookResponse` echoes `team_id`, and the non-admin `/api/v1/notifications` CRUD already surfaced it
...
## QA History (index 407)
- 2026-08-17: improve-architecture — **RESOLVED the "Team notification endpoint configuration (team_id field) not exposed in admin API create/update routes" known gap** (`api/routes/admin_notifications.py`). The `NotificationEndpoint` model and the notifier's `_get_subscribed_endpoints` already supported team-scoped dispatch, and the non-admin `/api/v1/notifications` CRUD already surfaced `team_id`, but the admin API (`/api/v1/admin/notifications`) never accepted or echoed it — org admins could not manage team-scoped webhooks. (1) `WebhookCreate.team_id` / `WebhookUpdate.team_id` (`uuid.UUID | None`, invalid format → 422). (2) New `_validate_team_exists(session, org_id, team_id)` — an RLS-scoped, org-filtered, non-deleted `teams` lookup that raises 422 `Unknown team id: <id>` before any write, so a dangling team reference can no longer fall through to the FK `IntegrityError` (previously surfaced as 503). (3) `create_webhook`/`update_webhook` persist `ep.team_id`; update only re-assigns when `team_id` is provided (omitting it leaves the current scope). (4) `WebhookResponse.team_id` echoed on list/get/create/update/re-enable. (5) Regenerated `frontend/src/lib/api/schema.ts` (additive `team_id` on WebhookCreate/WebhookUpdate/WebhookResponse). **Tests** — 9 new endpoint unit tests in `test_admin_notifications_webhooks.py` (create team-scoped 201 + persisted + echoed, create org-wide, unknown-team 422 + no persist, invalid UUID 422, update re-assign 200, update unknown-team 422 keeps scope, update omitting team_id keeps scope, list echoes team_id, get echoes team_id). Updated product map (`Admin API (CRUD)` + `Team-scoped dispatch` behaviour `[ ]`→`[x]`, 4 new behaviour rows, Known Gap → RESOLVED, QA History). Verification: 9/9 new tests + 40/40 existing notification endpoint/delivery-log tests pass, ruff check + format clean. Status: partial (Slack native integration, 5-in-24h auto-disable threshold, `X-Modulo-Timestamp` replay protection, dispatcher isolation, multi-wo... (line truncated to 2000 chars)

## QA History (2026-08-15 coverage sweep)
- Converted the remaining unchecked behaviour checkboxes to Known Gap bullets (both already documented): auto-disable threshold mismatch (PRD §8.11 5/24h vs code 10 consecutive, no window) and team notification endpoint configuration. The team_id surface was subsequently RESOLVED by index 407 (2026-08-17) — admin create/update now accept and echo `team_id`; the auto-disable threshold mismatch remains an open gap. No code change in this sweep. Status: partial (88/89 — the only remaining unchecked item is the auto-disable threshold gap, documented in Known Gaps).
