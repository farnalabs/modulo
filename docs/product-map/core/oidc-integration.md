...
prd: §9.4, §6.2 (sso enterprise feature gate), §9.2 (SSO + JIT provisioning with teams)
...
- [ ] Seeded providers get scopes `["openid", "profile", "email"]` and default role from `modulo_sso_default_role`
...
- [ ] SAML is reported as enabled only when license present + SAML configured
...
- [ ] Fetches discovery document from provider's `discovery_url` via HTTPS
...
- [ ] Token exchange uses `grant_type=authorization_code` with client credentials
...
- [ ] Falls back to `preferred_username` then email prefix as display name
...
- [ ] Frontend base URL derived from first `CORS_ORIGINS` origin
...
- [ ] `GET /api/v1/admin/sso/providers` lists all SSO providers
...
- [ ] Test returns failure details on network error or invalid document
