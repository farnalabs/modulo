...
- [ ] Nodes without explicit node_type default to "agent" ### Convert Manual → Agent
...
- [ ] Removes output_schema_id from the node after conversion ### Revert Agent → Manual
...
- [ ] Falls back to snapshot node label when current label is empty ### Frontend — Pipeline Editor
...
- [ ] Pipeline saves successfully after reversion ### Error States
...
- [ ] Concurrent graph replacement uses row-level locking (SELECT FOR UPDATE) ### Alice Persona Scenarios
...
- [ ] Restore a previous pipeline snapshot to roll back the replacement entirely ## Known Gaps - BDD feature files (`node_types.feature`, `manual_node.feature`, `pipeline_builder.feature`) are placeholders with no scenarios
