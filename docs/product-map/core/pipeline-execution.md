...
- [x] WebSocket reconnect mid-run → `replay_since()` ring-buffer replay is unit-tested (`TestRunEventBrokerReplay` in `test_pipeline_engine.py` covers events-after-seq, zero-seq, after-latest, empty buffer, stale-seq); no BDD scenario yet.
...
- [x] Oversized pipeline graph (>500 nodes or >1000 edges) → rejected at Pydantic validation layer in `PipelineGraphUpdate.reject_database_conflicts`, returns 422 with descriptive message before any DB work (verified by `test_replace_pipeline_graph_rejects_excessive_node_count` / `_edge_count` in `test_pipelines_endpoint.py`)
...
- [x] Manual node resume with invalid output schema → `_validate_against_schema` raises `OutputSchemaValidationError` (ValueError subclass), which the executor maps to the domain-specific `schema_validation_failure` error code (`contract.schema` on read surfaces) — no longer a confusing `error_code="ValueError"`. Fixed 2026-08-15 (node_runner.py + executor.py + error_codes.py).
...
- **Per-node retry policy (max_retries, retry_on, backoff) not implemented**: the PIPELINE-level `retry_policy` (auto re-dispatch on `stall`/`timeout`/`failure`, §8.9) IS implemented in the executor (`_retry_after_policy`, jittered capped backoff, GraphValidator `check_retry_policy`). The per-NODE retry loop (`retry_on` error classification + linear/exponential backoff at the node execution level, PRD §8.9) is still not implemented — a node failure terminal-fails the run (subject only to pipeline-level re-dispatch).
...
- **Missing BDD for conditional gate — RESOLVED**: The JMESPath-based conditional gate feature now has a BDD scenario (`error_recovery.feature:68-73` — "Conditional gate skips on falsy condition").
- **Missing BDD for eval-before-interrupt**: The eval-before-interrupt feature in `node_runner.py` has no BDD scenario.
- **Missing BDD for node timeout — RESOLVED**: BDD scenario exists at `error_recovery.feature:62-66` ("Node timeout marks run failed").
- **Empty pipeline (no nodes) produces raw 500**: `graph_cache.py` raises `ValueError` which becomes HTTP 500 instead of a structured validation error (defensive only — pre-run `validate_for_run` catches it with `TOPOLOGY_NO_NODES` before execution).
...
- **Manual node schema validation error produces `error_code="ValueError"` — FIXED**: `_validate_against_schema` now raises `OutputSchemaValidationError`, which the executor maps to `schema_validation_failure` (read surfaces present `contract.schema`). Fixed 2026-08-15.
...
- **WebSocket reconnect replay — now unit-tested**: `replay_since()` ring-buffer replay has unit coverage (`TestRunEventBrokerReplay` in `test_pipeline_engine.py`); no BDD scenario exists.
...
|---|---|---|---|
| 2026-08-15 | distribute — coverage sweep toward covered | VERIFIED — WebSocket reconnect replay (`replay_since()`, 100-event ring buffer) now has unit coverage via `TestRunEventBrokerReplay` in `test_pipeline_engine.py` (events-after-seq, zero-seq, after-latest, empty, stale-seq). Marked the "no unit test coverage" checkbox `[x]`; no BDD scenario yet. | [x] 1 behaviour verified [x] WS-replay gap resolved (unit-level) |
| 2026-08-15 | distribute — coverage sweep toward covered | Fixed MAJOR — manual-node resume with invalid output now produces the domain-specific `schema_validation_failure` error code (`contract.schema` on read surfaces) instead of `error_code="ValueError"`: `OutputSchemaValidationError` added in `node_runner.py`, mapped in `executor._stream_graph`, aliased in `error_codes.LEGACY_ALIASES`. Added unit coverage — `TestOutputSchemaValidation` (domain error + mapping) and `TestRunEventBrokerReplay` (WebSocket reconnect `replay_since()` ring-buffer replay, previously untested) in `test_pipeline_engine.py`. VERIFIED — oversized graph (>500 nodes / >1000 edges) rejection is implemented at the Pydantic layer and tested (`test_replace_pipeline_graph_rejects_excessive_node_count` / `_edge_count`). Updated Known Gaps — clarified that the PIPELINE-level retry_policy is implemented while the per-NODE retry loop remains; marked manual-node error-code and WS-replay gaps resolved. | [x] 2 behaviours verified/implemented [x] 12 new unit tests [x] 3 gaps resolved |
...
| 2026-07-08 | Cross-cutting QA (index 333) | Fixed CRITICAL — removed duplicate dead `except IntegrityError` handlers in runs.py (trigger_run, observe_run_node). Fixed MAJOR — resume() RuntimeError now produces domain-specific `configuration_error` instead of `error_code="RuntimeError"`. Fixed MAJOR — added `exc_info=True` to all 15 `_log.warning("route.db_error")` calls for proper exception context in SQLAlchemyError handlers. Fixed MINOR — marked conditional gate BDD and node timeout BDD Known Gaps as RESOLVED (scenarios exist in error_recovery.feature). | [x] 2 dead code blocks removed [x] 1 domain-specific error code added [x] 15 log calls improved [x] 2 stale gaps marked resolved |
