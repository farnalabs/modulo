...
- [ ] Tests exist for `RepositoryHub` dispatch logic — **no test coverage**
...
- [ ] Tests exist for `set_rls_user_context` on generic backend — **no unit test for the session.info path**
...
- No unit tests for `RepositoryHub` dispatch logic
