...
## Known Gaps
- Non-Postgres backends (MariaDB, SQLite) — `with_for_update()` is a no-op, so concurrent appends could create chain forks (only Postgres serializes appends via `SELECT ... FOR UPDATE`)
