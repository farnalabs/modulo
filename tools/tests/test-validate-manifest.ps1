...
BeforeAll {
    ...
    $script:ValidatorPath = Join-Path $toolsDir "validate-manifest.ps1"
...
'@
    ...
    # Run the validator against the isolated fixture repository, not this checkout.
    $script:TestValidator = Join-Path $TestDir "validate-manifest-test.ps1"
    $validatorContent = Get-Content -Raw -LiteralPath $ValidatorPath
    $testValidatorContent = $validatorContent -replace [regex]::Escape('$repoRoot = Resolve-Path (Join-Path $PSScriptRoot "..")'), "`$repoRoot='$TestDir'"
    Set-Content -Path $TestValidator -Value $testValidatorContent
